...
use meaning_analysis::project::PackageKind;
...
fn main_pkg(main_text: &str, pkg_text: &str) -> (IncrementalDatabase, SourceId, SourceId) {
...
fn scope_signatures_pulls_in_local_package_definition() {
    ...
    let scope = db.scope_signatures(main);
...
fn scope_signatures_prefers_sty_over_dtx() {
    ...
    assert_eq!(
        db.package_graph().loads(&fpath(&db, main))[0].to,
        fpath(&db, sty)
    );
    let scope = db.scope_signatures(main);
...
fn scope_signatures_backdates_on_prose_edit() {
    ...
    let _ = db.scope_signatures(main);
...
fn resolved_package_options_maps_a_member_sty() {
    ...
    let resolved = db.resolve_package_options();
    let facts = resolved
        .get(&fpath(&db, pkg))
...
fn unknown_option_fires_through_the_salsa_path() {
    ...
    let model = meaning_parser::semantic::SemanticModel::build(&root);
...
fn package_option_model_backdates_on_body_edit() {
    ...
    let _ = db.resolve_package_options();
