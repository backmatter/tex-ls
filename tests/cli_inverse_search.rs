//! CLI-level tests for `meaning inverse-search`, the command a PDF viewer runs
//! to hand a source position back to the editor.
//!
//! These run the real binary (`CARGO_BIN_EXE_meaning`) so they cover the clap
...
fn inverse_search(ipc_dir: &Path, args: &[&str]) -> Output {
    Command::new(env!("CARGO_BIN_EXE_meaning"))
        ...
        .arg(ipc_dir)
        .args(args)
        .env_remove("MEANING_IPC_DIR")
        .output()
        .expect("run meaning inverse-search")
}
...
fn reports_that_no_server_is_listening() {
    ...
    let err = stderr(&out);
    assert!(
        err.contains("no meaning language server is listening"),
...
fn unlinks_a_stale_advertisement() {
    ...
    let advertisement = meaning::ipc::Advertisement {
        pid: 999_999,
        transport: if cfg!(unix) {
            meaning::ipc::Transport::Unix
        } else {
            meaning::ipc::Transport::Tcp
        },
        address: if cfg!(unix) {
...
fn accepts_the_texlab_line1_spelling() {
    ...
    assert!(
        stderr(&out).contains("no meaning language server is listening"),
        "{}",
        stderr(&out)
    );
}
