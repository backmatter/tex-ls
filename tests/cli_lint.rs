//! CLI-level tests for `tex-ls lint --output json`: the machine-readable
//! findings contract consumed by external tools.
//!
//! These run the real binary (`CARGO_BIN_EXE_tex-ls`) so they cover the
...
fn lint_honors_declared_environments() {
    ...
    std::fs::write(dir.path().join("tex-ls.toml"), DECLARES_VERBATIM).unwrap();
