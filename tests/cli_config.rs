//! CLI-level tests for `$TEX_LS_CONFIG` (issue #40): the environment variable
//! naming a config file used when no project `tex-ls.toml` is discovered.
//!
//! These run the real binary (`CARGO_BIN_EXE_tex-ls`) so they cover reading
...
//! ancestor walk stops there and a developer's own `tex-ls.toml` cannot leak
//! in.
...
fn format_stdin(dir: &Path, env_config: Option<&str>, extra_args: &[&str]) -> Output {
    let mut cmd = Command::new(env!("CARGO_BIN_EXE_tex-ls"));
    cmd.arg("format")
        .args(extra_args)
        .current_dir(dir)
        .env_remove("TEX_LS_CONFIG")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped());
    if let Some(value) = env_config {
        cmd.env("TEX_LS_CONFIG", value);
    }
    let mut child = cmd.spawn().expect("run tex-ls");
    child
        .stdin
        .take()
        .unwrap()
        ...
        .unwrap();
    child.wait_with_output().expect("wait for tex-ls")
}
...
fn env_config_applies_when_no_project_config() {
    let dir = repo_dir();
    let config = dir.path().join("elsewhere").join("tex-ls.toml");
...
fn project_config_beats_env_config() {
    let dir = repo_dir();
    let env_config = dir.path().join("elsewhere").join("tex-ls.toml");
...
fn dangling_env_config_is_a_hard_error() {
    let dir = repo_dir();
    let missing = dir.path().join("nowhere").join("tex-ls.toml");

    let output = format_stdin(dir.path(), Some(missing.to_str().unwrap()), &[]);

    assert_eq!(output.status.code(), Some(2));
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("tex-ls:"), "stderr: {stderr}");
}
...
fn parse_honors_a_discovered_declaration() {
    let dir = repo_dir();
    std::fs::write(dir.path().join("tex-ls.toml"), ALIAS_CONFIG).unwrap();
...
fn parse_honors_an_explicit_config_path() {
    ...
    let config = elsewhere.path().join("tex-ls.toml");
