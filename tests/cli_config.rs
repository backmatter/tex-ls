//! CLI-level tests for `$MEANING_CONFIG` (issue #40): the environment variable
//! naming a config file used when no project `meaning.toml` is discovered.
//!
//! These run the real binary (`CARGO_BIN_EXE_meaning`) so they cover reading
...
//! ancestor walk stops there and a developer's own `meaning.toml` cannot leak
//! in.
...
fn env_config_applies_when_no_project_config() {
    let dir = repo_dir();
    let config = dir.path().join("elsewhere").join("meaning.toml");
...
fn project_config_beats_env_config() {
    let dir = repo_dir();
    let env_config = dir.path().join("elsewhere").join("meaning.toml");
...
fn dangling_env_config_is_a_hard_error() {
    let dir = repo_dir();
    let missing = dir.path().join("nowhere").join("meaning.toml");

    let output = format_stdin(dir.path(), Some(missing.to_str().unwrap()), &[]);

    assert_eq!(output.status.code(), Some(2));
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("meaning:"), "stderr: {stderr}");
}
...
fn parse_doc(dir: &Path, args: &[&str]) -> Output {
    Command::new(env!("CARGO_BIN_EXE_meaning"))
        .arg("parse")
        .args(args)
        .arg("doc.tex")
        .current_dir(dir)
...
fn parse_honors_a_discovered_declaration() {
    let dir = repo_dir();
    std::fs::write(dir.path().join("meaning.toml"), ALIAS_CONFIG).unwrap();
...
fn parse_honors_an_explicit_config_path() {
    ...
    let config = elsewhere.path().join("meaning.toml");
