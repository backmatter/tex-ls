...
use tex_ls_formatter::formatter::{FormatStyle, format_with_style_flavored};
...
fn format_never_reads_the_aux_file() {
    ...
    // symbols only. `tex-ls format` output must be byte-identical whether or not
...
use tex_ls::format::format_file_with_packages;
