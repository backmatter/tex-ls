...
use meaning_analysis::bib::format;
use meaning_analysis::bib::linter::{Severity, apply_fixes, check_document};
...
fn comment_directive_suppresses_following_entry() {
    // A `@comment{meaning-lint skip …}` carrier suppresses the named rule on the next
    // entry only.
    let src = "\
...
fn formatting_preserves_comment_directive_target_during_sort() {
    let src = "\
@comment{meaning-lint skip unknown-field: intentional}
...
fn file_directive_suppresses_all() {
    let src = "\
@comment{meaning-lint skip-file: quiet}
@string{a = {A}}
@misc{k, title = {DNA}}
";
...
fn inert_suppression_cannot_be_hidden_by_a_blanket_directive() {
    let src = "@comment{meaning-lint skip-file: vendored}\n\
