//! JSON Schema for `meaning.toml`.
//!
//! Generates the schema from the host [`Config`] type, keeps the checked-in
//! `meaning.schema.json` artifact synchronized, and checks representative
...
const SCHEMA_ID: &str = "https://backmatter.github.io/meaning/meaning.schema.json";
const SCHEMA_PATH: &str = "meaning.schema.json";
...
fn generate_schema_json() -> Value {
    ...
    if let Value::Object(map) = &mut json {
        ...
        map.insert(
            "title".to_string(),
            Value::String("Meaning configuration".to_string()),
        );
        map.insert(
            "description".to_string(),
            Value::String(
                "Schema for meaning.toml. Generated from the host Config types; \
...
fn schema_accepts_a_complete_runtime_valid_config() {
    ...
    let path = directory.path().join("meaning.toml");
    fs::write(&path, COMPLETE_CONFIG).expect("write config");
    meaning::config::ConfigLoader::load_from(&path).expect("runtime accepts config");
