//! JSON Schema for `tex-ls.toml`.
//!
//! Generates the schema from the host [`Config`] type, keeps the checked-in
//! `tex-ls.schema.json` artifact synchronized, and checks representative
...
const SCHEMA_ID: &str =
    "https://raw.githubusercontent.com/backmatter/tex-ls/main/tex-ls.schema.json";
const SCHEMA_PATH: &str = "tex-ls.schema.json";
...
fn generate_schema_json() -> Value {
    ...
    if let Value::Object(map) = &mut json {
        ...
        map.insert(
            "title".to_string(),
            Value::String("tex-ls configuration".to_string()),
        );
        map.insert(
            "description".to_string(),
            Value::String(
                "Schema for tex-ls.toml. Generated from the host Config types; \
...
fn schema_accepts_a_complete_runtime_valid_config() {
    ...
    let path = directory.path().join("tex-ls.toml");
    fs::write(&path, COMPLETE_CONFIG).expect("write config");
    tex_ls::config::ConfigLoader::load_from(&path).expect("runtime accepts config");
