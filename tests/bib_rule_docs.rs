...
//! (`docs/reference/bib-linter-rules.md`) is rendered from each bib rule's
...
//! output to the Markdown reference directory.

use tex_ls_analysis::bib::linter::docs::{
    demo_diagnostics, explain_rule, render_reference_page, render_rule_doc,
};
use tex_ls_analysis::bib::linter::{ALL_BIB_RULE_IDS, all_rules};
...
fn bib_reference_page_is_committed() {
    let committed = std::fs::read_to_string("docs/reference/bib-linter-rules.md")
        .expect("bib-linter-rules.md should exist");
    assert_eq!(
        committed,
        render_reference_page(),
        "docs/reference/bib-linter-rules.md is stale; run `cargo run --example docgen`",
    );
}
