...
use tex_ls_analysis::incremental::{IncrementalDatabase, QueryKind};
...
fn parse_count(db: &tex_ls_analysis::incremental::Analysis) -> usize {
    db.query_log()
        .iter()
...
// --- declarations (`tex-ls.toml`; AGENTS.md decision #12) -------------------
...
fn the_reparse_base_holds_a_lossless_tree() {
    ...
    let base = db.reparse_prev(file).expect("a base");
    let from_base = tex_ls_parser::syntax::SyntaxNode::new_root(base.green.clone());
...
fn unread_edits_preserve_the_complete_transform() {
    ...
    let full = tex_ls_parser::parser::parse(&text);
...
fn checked_edits_are_atomic_and_reject_stale_source_incarnations() {
    use tex_ls_analysis::incremental::ChangeError;
    let mut db = IncrementalDatabase::default();
    let path = Path::new("a.tex");
...
/// later parse can tell whether it is usable. Editing `tex-ls.toml` changes what a
...
fn external_batch_removal_keeps_surviving_observations_and_last_duplicate_value() {
    ...
    assert_eq!(
        db.file_alias(Path::new("/project/refs.bib")),
