...
//!
//! The reference page (`docs/reference/linter-rules.md`) is rendered from
...
//! `render_reference_page` output to the Markdown reference directory.

use std::path::Path;

use tex_ls_analysis::linter::docs::{
    demo_diagnostics_with, explain_rule, render_reference_page, render_rule_doc,
};
use tex_ls_analysis::linter::rules::{ALL_RULE_IDS, RETIRED_RULE_IDS, all_rules};
...
fn reference_page_is_committed() {
    let committed = std::fs::read_to_string("docs/reference/linter-rules.md")
        .expect("linter-rules.md should exist");
    assert_eq!(
        committed,
        render_reference_page(),
        "docs/reference/linter-rules.md is stale; run `cargo run --example docgen`",
    );
}
