...
//! cross-rule ordering, and `% tex-ls-lint` suppression over realistic
//! multi-line documents — complementing the focused per-rule unit tests in
//! `src/linter/`.

use std::path::{Path, PathBuf};

use tex_ls_analysis::linter::{Severity, lint_document};
...
use tex_ls_parser::parser::{parse, parse_with_flavor, reconstruct};
...
fn lint_project_full(files: &[(&str, &str)]) -> Vec<(String, tex_ls_analysis::linter::Diagnostic)> {
    let parsed: Vec<(PathBuf, SyntaxNode, SemanticModel)> = files
        .iter()
        .map(|(path, src)| {
            let kind = file_kind_or_tex(Path::new(path));
...
fn lint_with_bib(tex: &str, bibs: &[(&str, &str)]) -> Vec<&'static str> {
    ...
    let bib_keys: HashMap<PathBuf, Vec<SmolStr>> = bibs
        .iter()
        .map(|(path, src)| {
            let bib_model = tex_ls_analysis::bib::semantic::Model::build(
                &tex_ls_analysis::bib::parse(src).syntax(),
            );
            (
                PathBuf::from(path),
...
fn citation_gating_holds_for_fragment_and_wildcard() {
    ...
    // Wildcard usage does not define explicitly missing keys.
...
fn node_ignore_suppresses_only_the_next_block() {
    let src = "\
% tex-ls-lint skip deprecated-command: legacy macro
{\\bf one}
...
fn file_ignore_silences_a_rule_everywhere() {
    let src = "\
% tex-ls-lint skip-file deprecated-command: legacy file
{\\bf one}
{\\it two}
\\label{a}\\label{a}
";
...
fn file_ignore_all_silences_everything() {
    let src = "\
% tex-ls-lint skip-file: vendored
{\\bf one}
\\label{a}\\label{a}
";
    assert!(lint(src).is_empty());
...
fn retired_suppression_reports_and_fixes_end_to_end() {
    let src = "% tex-ls-ignore deprecated-command: legacy\n{\\bf one}\n";
    assert_eq!(
        lint(src),
...
fn retired_file_suppression_cannot_hide_its_own_deprecation() {
    let src = "% tex-ls-ignore-file: vendored\n{\\bf one}\n";
    assert_eq!(
        lint(src),
        vec![("deprecated-suppression-syntax", Severity::Warning)]
    );
...
fn inert_suppression_cannot_be_hidden_by_a_blanket_directive() {
    let src = "% tex-ls-lint skip-file: vendored\n% tex-ls-lint on deprecated-command\n";
...
fn node_ignore_silences_a_stylistic_rule() {
    let src = "\
% tex-ls-lint skip dollar-display-math: legacy snippet
$$x = y$$
";
    assert!(lint(src).is_empty(), "got: {:?}", lint(src));
}
...
use tex_ls_analysis::linter::{apply_fixes, check_document};
