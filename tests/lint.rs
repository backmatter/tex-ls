...
//! cross-rule ordering, and `% meaning-lint` suppression over realistic
//! multi-line documents — complementing the focused per-rule unit tests in
//! `src/linter/`.

use std::path::{Path, PathBuf};

use meaning_analysis::linter::{Severity, lint_document};
...
use meaning_analysis::source::file_kind_or_tex;
use meaning_parser::parser::{parse, parse_with_flavor, reconstruct};
...
fn lint_project_full(
    files: &[(&str, &str)],
) -> Vec<(String, meaning_analysis::linter::Diagnostic)> {
    let parsed: Vec<(PathBuf, SyntaxNode, SemanticModel)> = files
        .iter()
        .map(|(path, src)| {
            let kind = file_kind_or_tex(Path::new(path));
...
fn lint_with_bib(tex: &str, bibs: &[(&str, &str)]) -> Vec<&'static str> {
    use meaning_analysis::project::{
        CiteFileFacts, ResolvedCitations, collect_bib_resource_targets,
    };
    ...
    let bib_keys: HashMap<PathBuf, Vec<SmolStr>> = bibs
        .iter()
        .map(|(path, src)| {
            let bib_model = meaning_analysis::bib::semantic::Model::build(
                &meaning_analysis::bib::parse(src).syntax(),
            );
            (
                PathBuf::from(path),
...
fn node_ignore_suppresses_only_the_next_block() {
    let src = "\
% meaning-lint skip deprecated-command: legacy macro
{\\bf one}
...
fn file_ignore_silences_a_rule_everywhere() {
    let src = "\
% meaning-lint skip-file deprecated-command: legacy file
{\\bf one}
{\\it two}
\\label{a}\\label{a}
";
...
fn file_ignore_all_silences_everything() {
    let src = "\
% meaning-lint skip-file: vendored
{\\bf one}
\\label{a}\\label{a}
";
    assert!(lint(src).is_empty());
...
fn retired_suppression_reports_and_fixes_end_to_end() {
    let src = "% meaning-ignore deprecated-command: legacy\n{\\bf one}\n";
    assert_eq!(
        lint(src),
...
fn retired_file_suppression_cannot_hide_its_own_deprecation() {
    let src = "% meaning-ignore-file: vendored\n{\\bf one}\n";
    assert_eq!(
        lint(src),
        vec![("deprecated-suppression-syntax", Severity::Warning)]
    );
...
fn inert_suppression_cannot_be_hidden_by_a_blanket_directive() {
    let src = "% meaning-lint skip-file: vendored\n% meaning-lint on deprecated-command\n";
...
fn node_ignore_silences_a_stylistic_rule() {
    let src = "\
% meaning-lint skip dollar-display-math: legacy snippet
$$x = y$$
";
    assert!(lint(src).is_empty(), "got: {:?}", lint(src));
}
...
use meaning_analysis::linter::{apply_fixes, check_document};
