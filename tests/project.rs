...
use tex_ls_analysis::incremental::{IncrementalDatabase, QueryKind, QueryLogEntry, SourceId};
use tex_ls_analysis::project::IncludeKind;
...
/// The path a `SourceId` is actually tracked under. `upsert_file` lexically
...
fn fpath(db: &IncrementalDatabase, file: SourceId) -> PathBuf {
    db.file_path(file).to_path_buf()
}

fn main_bib(main_text: &str, bib_text: &str) -> (IncrementalDatabase, SourceId, SourceId) {
...
fn main_part(main_text: &str, part_text: &str) -> (IncrementalDatabase, SourceId, SourceId) {
    let mut db = IncrementalDatabase::default();
    let main = db.upsert_file(Path::new("/proj/main.tex"), main_text.to_string());
...
fn resolved_labels_unions_across_the_include_graph() {
    ...
    let resolved = db.resolve_labels();
...
fn resolved_citations_unions_referenced_bib_keys() {
    ...
    let resolved = db.resolve_citations();
...
#[test]
fn resolved_citations_uses_published_file_alias() {
    let mut db = IncrementalDatabase::default();
    ...
    assert!(db.set_file_alias(Path::new("/proj/refs.bib"), Path::new("/shared/refs.bib")));
