//! CLI-level tests for `meaning debug format`, the invariant-check command the
...
//! running the real binary (`CARGO_BIN_EXE_meaning`). Every invocation passes
//! `--no-config` so a developer's own `meaning.toml` cannot leak in.
...
fn meaning(dir: &Path, args: &[&str]) -> Output {
...
fn report_on_passing_file_has_no_failure_headings() {
    ...
    let output = meaning(dir.path(), &["debug", "format", "--report", "ok.tex"]);
...
fn dump_passes_requires_dump_dir() {
    ...
    let output = meaning(dir.path(), &["debug", "format", "--dump-passes", "ok.tex"]);
...
fn trivia_check_counts_skipped_bib_files_separately() {
    ...
    let output = meaning(dir.path(), &["debug", "format", "--checks", "trivia", "."]);
    assert!(output.status.success(), "stderr: {}", stderr(&output));
    assert!(
...
fn format_error_never_reads_as_an_invariant_failure() {
    ...
    let output = meaning(dir.path(), &["debug", "format", "bad.tex"]);
    ...
    let output = meaning(dir.path(), &["debug", "format", "--report", "bad.tex"]);
    assert_eq!(output.status.code(), Some(1));
    let report = stdout(&output);
