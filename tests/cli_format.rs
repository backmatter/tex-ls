//! CLI-level tests for `tex-ls format --check`: the report a CI step or a
...
//! (`CARGO_BIN_EXE_tex-ls`) so they cover the stdout-vs-stderr split and exit
...
//! there and a developer's own `tex-ls.toml` cannot leak in.
...
/// Run `tex-ls format` with `input` on stdin (`None` closes it: `/dev/null` is
...
fn dash_cannot_be_mixed_with_paths() {
    ...
    assert!(
        stderr.contains("Usage: tex-ls format"),
...
/// `tex-ls format doc.tex` and `tex-ls format < doc.tex` differently is a trap.
#[test]
