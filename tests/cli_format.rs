//! CLI-level tests for `meaning format --check`: the report a CI step or a
...
//! (`CARGO_BIN_EXE_meaning`) so they cover the stdout-vs-stderr split and exit
...
//! there and a developer's own `meaning.toml` cannot leak in.
...
/// Run `meaning format` with `input` on stdin (`None` closes it: `/dev/null` is
...
fn dash_cannot_be_mixed_with_paths() {
    ...
    assert!(
        stderr.contains("Usage: meaning format"),
...
/// `meaning format doc.tex` and `meaning format < doc.tex` differently is a trap.
#[test]
