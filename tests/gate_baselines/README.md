...
Recorded at meaning commit `268c5d8` (S0); re-recorded after S2, which removed
...
- **92 `content-change` additions to the `all` sets** (latex2e 69, latex3 11,
  ...
  11) plus latexindent's `figureValign` family (12), which is what the
      ...
      any perturbation via `meaning format --wrap reflow`". The gate now says
      so.
...
The gate compares invariants on corpus inputs. Upstream `*-mod1.tex` and
`*-output.tex` files reflect latexindent configurations, so their bytes are not
expected Meaning output. For fixture authoring, follow
[CONTRIBUTING.md](../../CONTRIBUTING.md#formatter-fixtures).

The corpus is GPL-3.0 and Meaning is MIT. Fetch it into the ignored `corpora/`
directory and write original fixtures rather than copying corpus source.
...
```sh
meaning --no-config debug format --checks all --report .     # -> <corpus>.all.txt
meaning --no-config debug format --checks trivia --report .  # -> <corpus>.trivia.txt
```
...
- `content-change` — `fmt` violated the whitespace-only invariant on a perturbed
  ...
  **pre-existing reflow-on-`.dtx` doc-layer bugs**, reachable without any
  perturbation via `meaning format --wrap reflow`:
