...
use meaning_formatter::formatter::{FormatStyle, format_with_style};

use lsp_types::{
    ApplyWorkspaceEditParams, ClientCapabilities, Code, CodeActionContext, CodeActionKind,
    CodeActionParams, CodeActionProvider, CodeActionResponse, CompletionItem, CompletionItemKind,
    CompletionParams, CompletionResponse, Contents, DefinitionParams, DefinitionResponse,
    ...
    FoldingRange, FoldingRangeKind, FoldingRangeParams, FoldingRangeProvider, FormattingOptions,
...
fn start_server(
    ...
    let server_thread = std::thread::spawn(move || meaning::lsp::serve(server).unwrap());
...
fn assert_server_capabilities(init: &InitializeResult) {
    ...
    assert!(
        matches!(
            ...
            Some(CodeActionProvider::Bool(true))
        ),
    ...
    assert!(
        init.capabilities
            ...
            .is_some_and(|opts| {
                opts.commands
                    .contains(&"meaning.changeEnvironment".to_owned())
                    && !opts
                        .commands
...
fn lsp_bib_diagnostics_formatting_and_symbols() {
    ...
    let edits: Vec<TextEdit> =
        serde_json::from_value(resp.response_result.clone().unwrap()).unwrap();
    assert_eq!(edits.len(), 1, "expected one whole-document edit");
    let expected = meaning_analysis::bib::format_with_style(
        doc,
        FormatStyle {
            line_width: 80,
            indent_width: 2,
...
fn formatting_reloads_changed_config_without_watcher_support() {
    ...
    let dir = tempfile::tempdir().expect("temp dir");
    let config_path = dir.path().join("meaning.toml");
...
/// `meaning.…` id or its `texlab.…` alias), with the texlab-shaped single
        ...
        match recv(client) {
            Message::Response(resp) => {
                ...
                assert!(
                    ...
                    resp.response_result.clone().err()
                );
                break;
            }
            Message::Notification(_) => continue,
...
fn lsp_change_environment_rewrites_pair() {
    ...
    // The command, invoked from the outer `\end` name, targets the outer pair.
    change_environment(
        &client,
        3,
        "meaning.changeEnvironment",
...
fn lsp_change_environment_declines_without_environment() {
    ...
    send_request(
        ...
        "workspace/executeCommand",
        serde_json::to_value(ExecuteCommandParams {
            command: "meaning.doesNotExist".to_owned(),
            arguments: Some(vec![]),
            work_done_progress_params: Default::default(),
        })
        .unwrap(),
...
fn apply_edits(text: &str, edits: &[TextEdit]) -> String {
    let index = meaning_analysis::text::LineIndex::new(text);
...
fn lsp_registers_file_watchers_on_initialized() {
    ...
    let globs: Vec<String> = opts["watchers"]
        ...
        .collect();
    assert!(
        globs.contains(&"**/*.{tex,bib,sty,cls,dtx,ins}".to_owned()),
        "expected the supported-source glob, got {globs:?}"
    );
    assert!(
        globs.contains(&"**/meaning.toml".to_owned()),
...
fn lsp_watched_config_change_reanalyzes_open_doc() {
    // Dropping a `meaning.toml` beside an open doc (out of editor) and signalling the
...
fn lsp_a_request_is_answered_under_its_own_workspace_declarations() {
    ...
    std::fs::write(
        declaring.path().join("meaning.toml"),
        "[environments.mycode]\nlike = 'lstlisting'\n",
    )
    .unwrap();
...
//
// The "viewer" throughout is `meaning debug echo-args`, which records its argv
// to a file: the one program guaranteed to exist wherever CI runs, and it shows
// exactly what `%f`/`%p`/`%l` expanded to.
// ---------------------------------------------------------------------------

/// `initializationOptions` configuring `meaning debug echo-args --out <record>`
...
fn viewer_options(record: &std::path::Path, args: &[&str]) -> serde_json::Value {
    ...
    serde_json::json!({
        "forwardSearch": {
            "executable": env!("CARGO_BIN_EXE_meaning"),
            "args": argv,
        }
    })
}
...
fn forward_search_honors_build_root_for_an_unseeded_parent() {
    ...
    // directory — and `meaning::config::ConfigLoader::discover` reaches that directory by canonicalizing.
...
fn inverse_search_reveals_the_position_via_show_document() {
    ...
    let viewer = {
        ...
        std::thread::spawn(move || meaning::ipc::send_inverse_search_in(&ipc_dir, &main_path, 3, 0))
    };
...
fn inverse_search_declines_a_file_outside_the_workspace() {
    ...
    let err = meaning::ipc::send_inverse_search_in(&ipc_dir, &stranger, 1, 0)
        .expect_err("no server owns this file");
    assert!(
        matches!(err, meaning::ipc::IpcError::NoServerForFile(_)),
        "{err:?}"
    );
...
fn a_client_without_show_document_support_never_advertises() {
    ...
    let err = meaning::ipc::send_inverse_search_in(&ipc_dir, &dir.path().join("main.tex"), 1, 0)
        .expect_err("nothing should be listening");
    assert!(matches!(err, meaning::ipc::IpcError::NoServer), "{err:?}");
