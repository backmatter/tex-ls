...
class SmokeScanProjectTests(unittest.TestCase):
    def test_scans_nested_files_and_records_skips_and_failures(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            fake_meaning = root / "meaning"
            fake_meaning.write_text(
                "#!/usr/bin/env bash\n"
                'if [[ "$*" == *bad.tex* ]]; then\n'
            ...
            fake_meaning.chmod(0o755)
            results = root / "results"
            environment = {
                **os.environ,
                "MEANING_BIN": str(fake_meaning),
                "MEANING_SHA": "meaning-sha",
                "MEANING_VERSION": "meaning test",
                "ALLOWLIST": "",
            }
...
class SmokeWorkflowTests(unittest.TestCase):
    def test_repository_clones_stay_outside_the_uploaded_results(self) -> None:
        ...
        self.assertIn(
            'REPOS_DIR="$RUNNER_TEMP/meaning-debug-format-repos/github"',
            workflow,
        )
