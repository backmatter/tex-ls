...
use tex_ls_analysis::source::{FileKind, lint_file_kind};
...
/// directory containing `tex-ls.toml`, or the working directory when there is no
...
/// as arguments.
///
/// There is no `use_defaults` flag: tex-ls
...
impl ExcludeFilter {
    ...
    pub fn force_excludes(&self, path: &Path) -> bool {
        self.force
            && match &self.matcher {
                Some(matcher) => relative_match_path(matcher, path).is_some_and(|relative| {
                    matcher
    ...
    fn is_excluded(&self, path: &Path, is_dir: bool) -> bool {
        match &self.matcher {
            Some(matcher) => relative_match_path(matcher, path)
...
// Match relative paths explicitly. A canonical configuration root may use a
// Windows verbatim prefix or long names while watcher paths use short names.
// Deleted files still resolve through their nearest existing ancestor.
fn relative_match_path(matcher: &Gitignore, path: &Path) -> Option<PathBuf> {
    let path = std::path::absolute(path).ok()?;
    if let Ok(relative) = path.strip_prefix(matcher.path()) {
        return Some(relative.to_path_buf());
    }
    let root = matcher.path().canonicalize().ok()?;
...
#[cfg(test)]
use tex_ls_parser::parser::{LatexFlavor, LexConfig};

#[cfg(test)]
use tex_ls_analysis::source::file_kind_or_tex;
...
mod tests {
    ...
    #[test]
    fn force_exclude_handles_canonical_roots_and_deleted_paths() {
        let dir = tempfile::tempdir().unwrap();
        let root = dir.path().canonicalize().unwrap();
        let filter = ExcludeFilter::new(&root, &["/vendor/".into()])
            .unwrap()
            .with_force_exclude(true);
        let missing = dir.path().join("vendor/missing/deleted.tex");
        assert!(filter.force_excludes(&missing));
        assert!(filter.is_excluded(&dir.path().join("vendor"), true));
        assert!(!filter.force_excludes(&dir.path().join("main.tex")));
        let outside = tempfile::tempdir().unwrap();
        assert!(!filter.force_excludes(&outside.path().join("vendor/file.tex")));
    }
