...
use tex_ls_analysis::linter::RuleSelection;
pub use tex_ls_analysis::linter::settings::LintConfig;
pub use tex_ls_formatter::settings::FormatConfig;
use tex_ls_parser::declarations::{
    CommandDecl, CommandDecls, DeclarationError, Declarations, EnvironmentDecl, EnvironmentDecls,
    ResolvedDeclarations,
};

pub const CONFIG_FILE_NAME: &str = "tex-ls.toml";

/// Environment variable naming a config file to use when no project
/// `tex-ls.toml` is discovered. Sits between project discovery and the
/// global user config in `Config::resolve`'s
/// precedence, and shadows the global file entirely when set.
pub const CONFIG_ENV_VAR: &str = "TEX_LS_CONFIG";

/// Built-in exclude patterns applied when `exclude` is unset (a present `exclude`
/// replaces them). Kept deliberately small: tex-ls only ever processes
...
pub struct Config {
    ...
    /// relative to the directory containing this `tex-ls.toml`. Applies to *both*
    ...
    /// Completion-only command/environment option schemas.
    #[serde(default)]
    pub options: BTreeMap<String, BTreeMap<String, Vec<String>>>,
    ...
    /// delimiters (`\bea`/`\eea`).
    ///
    /// These explicit declarations reach the parser. The shared parser type owns
    /// their wire format, so native and browser inputs use identical spellings.
    /// A keyed table lets configuration layers merge declarations by name.
    #[serde(default)]
...
impl Config {
    ...
    /// whole vocabulary at once, and because the other front ends (the browser
    ...
    pub fn validate(&self, path: Option<&Path>) -> Result<(), ConfigError> {
                ...
                || matches!(name.as_str(), "." | ".."))
        {
            return Err(ConfigError::InvalidValue {
                path: path.map(Path::to_path_buf),
                field: "job-name",
                message: "must be a nonempty bare job name".into(),
            });
        }
        Ok(())
    }
...
impl ValidatedConfig {
    pub fn formatter_settings(&self) -> &tex_ls_formatter::settings::ResolvedFormatSettings {
...
impl Config {
    ...
    fn validate_values(
        ...
        (
            ResolvedDeclarations,
            RuleSelection,
            tex_ls_formatter::settings::ResolvedFormatSettings,
        ),
        ConfigError,
    > {
