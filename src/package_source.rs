...
//! `scope_signatures` query: the CLI
...
pub trait PackageSource {
    ...
    /// `parsed_document`, which reads the
...
/// the precedence rules; mirrors `scope_signatures`.
...
/// equivalent of `scope_signatures`.
...
mod tests {
    ...
    fn formatter_scope_never_reaches_the_texmf_tree() {
        ...
        // disk scope that feeds `tex-ls format` must not pull its definitions in —
        ...
        let index = crate::texmf::build_from_roots(&[tree.path().to_path_buf()]);
