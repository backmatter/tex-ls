//! The `meaning` command-line surface.
//!
...
pub struct Cli {
    #[command(subcommand)]
    pub command: Command,
    /// Path to a `meaning.toml` to use instead of discovering one. Applies to
    /// `format` and `lint`; ignored by `parse`, `lsp`, and `init`.
    #[arg(long, value_name = "PATH", global = true, conflicts_with = "no_config")]
    pub config: Option<PathBuf>,
    /// Ignore any `meaning.toml` (project, `$MEANING_CONFIG`, or global) and
...
pub enum Command {
    ...
    Format {
        ...
        /// Added on top of any `exclude`/`extend-exclude` from `meaning.toml`.
    ...
    /// `--synctex-editor-command "meaning inverse-search --input %{input} --line
    /// %{line}"`. The position is handed to a running meaning language server,
    ...
    InverseSearch {
        ...
        /// `$MEANING_IPC_DIR`, then a per-user directory under the runtime (or
        /// temporary) directory.
...
/// Subcommands under `meaning debug`.
#[derive(Subcommand)]
...
/// Which checks `meaning debug format` runs.
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
