//! The `tex-ls` command-line surface.
//!
//! Clap definitions for the executable and its help output.
//! Conversions to language-library types live in `main.rs`.
...
pub struct Cli {
    #[command(subcommand)]
    pub command: Command,
    /// Path to a `tex-ls.toml` to use instead of discovering one. Applies to
    /// `format` and `lint`; ignored by `parse`, `lsp`, and `init`.
    #[arg(long, value_name = "PATH", global = true, conflicts_with = "no_config")]
    pub config: Option<PathBuf>,
    /// Ignore any `tex-ls.toml` (project, `$TEX_LS_CONFIG`, or global) and
...
pub enum Command {
    ...
    Format {
        ...
        /// Added on top of any `exclude`/`extend-exclude` from `tex-ls.toml`.
    ...
    /// `--synctex-editor-command "tex-ls inverse-search --input %{input} --line1
    /// %{line}"`. The position is handed to a running tex-ls language server,
    ...
    InverseSearch {
        ...
        input: PathBuf,
        /// Line number counting from 1. Supply exactly one of --line0/--line1.
        #[arg(long, value_name = "LINE", conflicts_with = "line0", value_parser = clap::value_parser!(u32).range(1..))]
        line1: Option<u32>,
        ...
        /// `$TEX_LS_IPC_DIR`, then a per-user directory under the runtime (or
        /// temporary) directory.
        #[arg(long, value_name = "DIR")]
        ipc_dir: Option<PathBuf>,
    },
    /// Write a commented starter `tex-ls.toml` to the current directory.
    Init {
        /// Overwrite an existing `tex-ls.toml`.
        #[arg(long)]
        force: bool,
    },
...
/// Subcommands under `tex-ls debug`.
#[derive(Subcommand)]
...
/// Which checks `tex-ls debug format` runs.
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
