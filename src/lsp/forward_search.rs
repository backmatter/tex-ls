...
//!
//! Meaning never typesets, and it never reads a `.synctex.gz`. It resolves three
...
//!
//! This native extension accepts a source position and reports viewer launch
//! status. Viewer commands use `%f`, `%p`, and `%l` for the source file, PDF,
...
/// an editor plugin written against texlab drives meaning unchanged.
pub(crate) enum ForwardSearchRequest {}

impl lsp_types::Request for ForwardSearchRequest {
    type Params = TextDocumentPositionParams;
    type Result = ForwardSearchResult;
    const METHOD: lsp_types::LspRequestMethod<'static> =
        lsp_types::LspRequestMethod::new("textDocument/forwardSearch");
        ...
        .spawn()
    {
        Ok(mut child) => {
            if std::thread::Builder::new()
                .name("meaning-viewer-reap".to_owned())
                .spawn(move || {
                    let _ = child.wait();
                })
                .is_err()
