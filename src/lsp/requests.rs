//! Native requests responsibility.
use super::*;
...
pub(super) fn on_formatting(
    ...
    // A discovered `meaning.toml` wins outright; only when none
...
pub(super) fn on_range_formatting(
    ...
    let params = match req
        .extract::<DocumentRangeFormattingParams>(DocumentRangeFormattingRequest::METHOD.as_str())
    {
        Ok((_, params)) => params,
        Err(_) => {
            let resp = Response::new_err(
...
pub(super) fn on_forward_search(
    ...
    let params =
        match req.extract::<TextDocumentPositionParams>(ForwardSearchRequest::METHOD.as_str()) {
            Ok((_, params)) => params,
            Err(_) => {
                let resp = Response::new_err(
                    id,
...
pub(super) fn on_execute_command(
    ...
    let params = match req.extract::<ExecuteCommandParams>(ExecuteCommandRequest::METHOD.as_str()) {
        Ok((_, params)) => params,
        Err(_) => {
            respond_err(
    ...
    match params.command.as_str() {
        CHANGE_ENVIRONMENT_COMMAND => {
            ...
            let uri = args.text_document_position_params.text_document.uri;
            let position = args.text_document_position_params.position;
            let Some(_) = state.documents.get(&uri) else {
                respond_err(
                    ErrorCode::InvalidParams,
...
/// `meaning.toml` watched-change path.
