...
fn editor_settings_namespaced_and_bare() {
    ...
    let namespaced = serde_json::json!({ "tex-ls": { "lineWidth": 72 } });
...
fn resolve_settings_builds_exclude_filter_for_sibling_discovery() {
    ...
    // worker.
    let dir = tempfile::tempdir().expect("tempdir");
    std::fs::write(dir.path().join("tex-ls.toml"), "exclude = [\"vendor/\"]\n")
        .expect("write config");
...
fn resolve_settings_without_config_excludes_nothing() {
    ...
    // `vendor/` sibling is still discovered when no `tex-ls.toml` governs.
    let dir = tempfile::tempdir().expect("tempdir");
    std::fs::write(dir.path().join("main.tex"), "").expect("write main");
    std::fs::create_dir(dir.path().join("vendor")).expect("mkdir vendor");
    std::fs::write(dir.path().join("vendor").join("lib.tex"), "").expect("write lib");
...
fn resolve_settings_detects_nearer_config_creation_and_deletion() {
    ...
    let nearer = nested.join("tex-ls.toml");
...
fn invalid_configuration_keeps_last_good_settings_until_repaired() {
    ...
    let path = dir.path().join("tex-ls.toml");
    let uri = file_uri_in(dir.path());
    let mut state = state_with_editor(EditorSettings::default());
    std::fs::write(
        &path,
...
#[test]
fn scoped_client_settings_use_the_most_specific_folder() {
    let mut state = state_with_editor(EditorSettings::default());
    let directory = tempfile::tempdir().unwrap();
    for (path, width) in [("project", 30), ("project/child", 50)] {
        state.scoped_editor_settings.insert(
    ...
    assert_eq!(
        state
            .client_settings(
                &tex_ls_protocol::path_to_uri(&directory.path().join("project/main.tex")).unwrap()
            )
            .line_width,
        Some(30)
    );
    assert_eq!(
        state
            .client_settings(
                &tex_ls_protocol::path_to_uri(&directory.path().join("project/child/main.tex"))
                    .unwrap()
            )
            .line_width,
        Some(50)
    ...
    assert_eq!(
        state
            .client_settings(
                &tex_ls_protocol::path_to_uri(&directory.path().join("project-other/main.tex"))
                    .unwrap()
            )
            .line_width,
        None
...
#[test]
fn client_configuration_is_atomic_scoped_and_ignores_replaced_requests() {
    let mut state = state_with_editor(EditorSettings::default());
    let directory = tempfile::tempdir().unwrap();
    state.workspace_roots = vec![directory.path().join("p"), directory.path().join("q")];
    let (server, client) = Connection::memory();
    let (jobs, _) = unbounded();
    let request = |state: &mut GlobalState| {
        request_configuration(&server, state);
        let Message::Request(request) = client.receiver.recv().unwrap() else {
            panic!("request");
        };
        assert_eq!(
            request.params["items"][0]["scopeUri"],
            tex_ls_protocol::path_to_uri(&directory.path().join("p"))
                .unwrap()
                .as_str()
        );
        request.id
    ...
    handle_client_response(
        ...
        Response::new_ok(id, serde_json::json!([{"lineWidth":30},{"lineWidth":40}])),
    );
    ...
    handle_client_response(
        ...
        &jobs,
        "workspace/configuration",
        Response::new_ok(old, serde_json::json!([{"lineWidth":60},{"lineWidth":60}])),
    );
    handle_client_response(
        &server,
        &mut state,
        ...
        Response::new_ok(
            current,
            serde_json::json!([{"lineWidth":70},{"lineWidth":0}]),
        ),
    );
    for (path, width) in [("p/a.tex", 30), ("q/a.tex", 40)] {
        assert_eq!(
            state
                .client_settings(
                    &tex_ls_protocol::path_to_uri(&directory.path().join(path)).unwrap()
                )
                .line_width,
            Some(width)
        );
...
#[test]
fn server_acknowledgements_track_registration_and_refusal() {
    let mut state = state_with_editor(EditorSettings::default());
    ...
    for (result, expected) in [
        ...
        assert_eq!(state.watcher_acknowledged, expected);
    }
    handle_client_response(
        &server,
        &mut state,
