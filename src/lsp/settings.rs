...
/// discovered `tex-ls.toml` and the per-request [`FormattingOptions`]; `texmf` is
/// *machine* configuration with no `tex-ls.toml` counterpart — the editor is its
...
///
/// *Machine* configuration with no `tex-ls.toml` counterpart, for the same
...
impl EditorSettings {
    ...
    /// bare options object or a tree namespaced under a `"tex-ls"` key (how
...
pub(super) struct ResolvedSettings {
    ...
    /// formatter — so it cannot affect `tex-ls format` output.
...
/// absent entries matter: if a nearer `tex-ls.toml` is created, it must displace
...
impl GlobalState {
    ...
    fn invalid_settings(
        ...
        self.config_cache
            ...
            .unwrap_or_else(|| ResolvedSettings::from_editor(editor))
    }

    /// Resolve (and cache) the [`ResolvedSettings`] for `uri`'s document: discover a
    /// `tex-ls.toml` from the document's anchor directory (its parent), falling back
    /// to the global user config (`~/.config/tex-ls/config.toml`), then to the
        ...
        let (resolved, source_path) =
            match crate::config::ConfigLoader::resolve(None, false, &anchor) {
                Ok((config, source)) => {
                    ...
                    let mut resolved = ResolvedSettings::from_config(&config, present, &editor);
                    if present {
                ...
                Err(error) => return self.invalid_settings(&anchor, &editor, error.to_string()),
            };
