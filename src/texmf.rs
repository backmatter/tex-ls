...
//! completion).
//!
//! This is the one place meaning looks past the document's own directory into the
//! wider TeX installation. It is deliberately fenced off from the formatter: the
//! index never feeds `scope_signatures`/`DiskPackageSource`, so `meaning format`
...
/// and installed-set completion). This never feeds the formatter — `meaning format`
...
/// `workspace/didChangeConfiguration`, camelCase JSON), never from `meaning.toml` —
...
/// The on-disk cache file (`<cache>/meaning/texmf-index.json`), or `None` when no OS
/// cache dir is available.
fn cache_path() -> Option<PathBuf> {
    Some(dirs::cache_dir()?.join("meaning").join("texmf-index.json"))
}
...
mod tests {
    ...
    fn cache_round_trips_and_invalidates_on_fingerprint() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("meaning/texmf-index.json");
