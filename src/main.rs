//! The `meaning` command-line interface.
//!
...
//! from the [`meaning::cli`] definitions by `build.rs` (via `clap_mangen` /
//! `clap_complete` / `clapdown`).

use meaning::format::{ChangedFile, check_paths_with_style, format_file_with_packages_sentence};
use meaning_analysis::linter::{OutputMode, render_findings};
use std::io::{BufWriter, IsTerminal, Read, Write};
use std::path::{Path, PathBuf};
use std::process::ExitCode;

use meaning::config::Config;
use meaning::config::ConfigSource;
use meaning::file_discovery::{ExcludeFilter, FileDiscoveryError, collect_lint_files};
use meaning_analysis::linter::{
    Diagnostic, Fix, RuleSelection, apply_fixes, apply_fixes_multi, check_document_fixable,
    lint_document,
};
use meaning_analysis::source::{FileKind, file_kind_or_tex};
use meaning_formatter::formatter::perturb::{
...
use meaning_analysis::project::labels::{
    document_label_names, document_ref_names, is_document_root,
};
use meaning_analysis::project::{
...
use meaning_parser::parser::{LexConfig, parse_with_declarations, parse_with_flavor};
use meaning_parser::semantic::SemanticModel;
use meaning_parser::syntax::SyntaxNode;
use rayon::prelude::*;
...
fn main() -> ExitCode {
    ...
    match command {
        Command::Format {
            ...
            // Discover/load `meaning.toml` from the working directory (one config
        ...
        Command::Lint {
            ...
            for id in &unknown {
                eprintln!("meaning: warning: unknown lint rule `{id}`");
            }
            run_lint(
                &paths,
                fix,
        ...
        Command::Parse { path } => {
            ...
            // would make `meaning parse --no-config` dump a tree no other
...
/// `meaning debug echo-args`: record the trailing arguments, one per line.
///
...
fn run_debug_echo_args(out: &Path, args: &[String]) -> ExitCode {
    ...
    match std::fs::write(&staging, body).and_then(|()| std::fs::rename(&staging, out)) {
        ...
        Err(err) => {
            ...
            eprintln!("meaning: failed to write {}: {err}", out.display());
            ExitCode::from(2)
        }
    }
}
...
fn cwd_anchor() -> Result<PathBuf, ExitCode> {
    std::env::current_dir().map_err(|err| {
        eprintln!("meaning: cannot determine the current directory: {err}");
        ExitCode::from(2)
    })
}
...
fn resolve_config(
    ...
    no_config: bool,
    anchor: &Path,
) -> Result<(meaning::config::ValidatedConfig, ConfigSource), ExitCode> {
    meaning::config::ConfigLoader::resolve(explicit, no_config, anchor).map_err(|err| {
        eprintln!("meaning: {err}");
        ExitCode::from(2)
    })
}
...
/// `meaning.toml`, or relative to `anchor` for the env and global user configs
...
/// A commented starter `meaning.toml` showing every key at its default.
const STARTER_CONFIG: &str = "\
# meaning configuration. All keys are optional; values shown are the defaults.
...
/// `meaning inverse-search`: hand a viewer's source position to a running
/// language server.
///
...
fn run_inverse_search(
    ...
    let Some(line) = line.or_else(|| line0.map(|l| l + 1)) else {
        eprintln!(
            "meaning: pass --line (counting from 1, what most viewers emit) \
    ...
    let dir = ipc_dir
        ...
        .unwrap_or_else(meaning::ipc::ipc_dir);
    match meaning::ipc::send_inverse_search_in(&dir, &path, line, character) {
        Ok(()) => ExitCode::SUCCESS,
        Err(err) => {
            eprintln!("meaning: {err}");
            ExitCode::from(2)
...
/// `meaning init`: write a commented starter config to `<cwd>/meaning.toml`.
fn run_init(force: bool) -> ExitCode {
    let anchor = match cwd_anchor() {
        Ok(anchor) => anchor,
        Err(code) => return code,
    };
    let path = anchor.join(meaning::config::CONFIG_FILE_NAME);
    if path.exists() && !force {
        eprintln!(
            "meaning: {} already exists; pass --force to overwrite",
            path.display()
        );
        return ExitCode::from(2);
    }
    match std::fs::write(&path, STARTER_CONFIG) {
        ...
        Err(err) => {
            eprintln!("meaning: failed to write {}: {err}", path.display());
            ExitCode::from(2)
        }
    }
}
...
fn run_lsp() -> ExitCode {
    match meaning::lsp::run() {
        Ok(()) => ExitCode::SUCCESS,
        Err(err) => {
            eprintln!("meaning: language server error: {err}");
            ExitCode::FAILURE
        }
    }
}
...
fn run_explain(id: &str) -> ExitCode {
    let doc = meaning_analysis::linter::docs::explain_rule(id)
        .or_else(|| meaning_analysis::bib::linter::docs::explain_rule(id));
    match doc {
        Some(doc) => {
            print!("{doc}");
            ExitCode::SUCCESS
        ...
        None => {
            eprintln!("meaning: unknown lint rule `{id}`");
            eprintln!(
                "known rules: {}",
                meaning_analysis::linter::rules::all_known_rule_ids()
                    .collect::<Vec<_>>()
                    .join(", ")
            );
            ExitCode::from(2)
...
fn analyze_source(
    ...
    match kind {
        FileKind::Bib => {
            ...
            let parsed = meaning_analysis::bib::parse(content);
            let mut diagnostics: Vec<Diagnostic> = parsed
                .errors
                .iter()
                .map(|err| Diagnostic {
                    rule: "parse",
                    severity: meaning_analysis::linter::Severity::Error,
            ...
            let model = meaning_analysis::bib::semantic::Model::build(&root);
            let keys = model.entries().iter().map(|e| e.key.clone()).collect();
            diagnostics.extend(meaning_analysis::bib::linter::lint_document(
                path, &root, &model,
            ));
            FileAnalysis::Bib {
                diagnostics,
...
/// BibTeX findings to `meaning lint doc.tex`.
fn supplemental_bibliographies(
    ...
    for (path, content, kind) in sources {
        ...
        for target in collect_bib_resource_targets(&root, path.parent()) {
            ...
            let Some(actual) =
                meaning::bibliography::resolve_bibliography_file(&requested, base_dir)
            else {
                continue;
            };
            if actual != requested {
...
fn run_lint(
    ...
    if let Some(paths) = paths {
        ...
        if files.is_empty() {
            ...
            eprintln!(
                "meaning: no .tex, .sty, .cls, .dtx, .ins, or .bib files found under the provided input paths"
            );
            return ExitCode::FAILURE;
        }
        ...
        for result in read_results {
            match result {
                ...
                Err((path, err)) => {
                    eprintln!("meaning: cannot read {}: {err}", path.display());
                    failed = true;
                }
            }
        }
        ...
        if let Err(err) = std::io::stdin().read_to_string(&mut input) {
            eprintln!("meaning: cannot read stdin: {err}");
            return ExitCode::FAILURE;
        }
...
fn collect_project_diagnostics_with_bibliographies(
    ...
    for (path, content) in &supplemental.sources {
        let parsed = meaning_analysis::bib::parse(content);
        let model = meaning_analysis::bib::semantic::Model::build(&parsed.syntax());
        bib_keys.insert(
            path.clone(),
            model
                .entries()
...
fn apply_fixes_to_paths(
    ...
    if files.is_empty() {
        ...
        eprintln!("meaning: no .tex or .bib files found under the provided input paths");
        return Some(ExitCode::FAILURE);
    }
    ...
    let outcomes: Vec<FixOutcome> = files
        ...
        .map(
            |(path, kind)| match fix_file(path, *kind, include_unsafe, rules, declared) {
                ...
                Err(err) => {
                    FixOutcome::Failed(format!("meaning: cannot fix {}: {err}", path.display()))
                }
            },
        )
        .collect();
    ...
    if let Err(err) = apply_cross_file_fixes(&files, include_unsafe, rules, declared) {
        eprintln!("meaning: cannot apply cross-file fixes: {err}");
        failed = true;
    }
...
fn fix_file(
    ...
    for _ in 0..MAX_FIX_ITERATIONS {
        let diagnostics = match kind {
            ...
            FileKind::Bib => meaning_analysis::bib::linter::check_document(path, &content),
        };
...
fn apply_cross_file_fixes(
    ...
    if skipped > 0 {
        eprintln!(
            "meaning: {skipped} cross-file fix{} skipped (conflicting edits)",
            plural(skipped)
        );
    }
...
fn debug_parse_error_count(
    ...
    match kind {
        FileKind::Bib => meaning_analysis::bib::parse(content).errors.len(),
...
fn run_parse(paths: &[PathBuf], declared: &ResolvedDeclarations) -> ExitCode {
    ...
    if let Err(err) = std::io::stdout().write_all(out.as_bytes()) {
        eprintln!("meaning: cannot write stdout: {err}");
        return ExitCode::FAILURE;
    }
...
///
/// Bare `meaning format` reads stdin, matching rustfmt/gofmt/clang-format. That
...
fn usage_error(subcommand: &str, kind: ErrorKind, message: &str) -> ! {
    ...
    sub.clone()
        .bin_name(format!("meaning {subcommand}"))
        .error(kind, message)
        .exit()
}
...
fn run_format_stdin(
    ...
    let formatted = match kind {
        ...
        FileKind::Bib => {
            meaning_analysis::bib::format_with_style(&input, style).map_err(|e| e.to_string())
        }
    };
    match formatted {
        Ok(formatted) => {
...
fn report_discovery_error(err: &FileDiscoveryError) {
    match err {
        FileDiscoveryError::UnsupportedLintFilePath { path } => {
            eprintln!(
                "meaning: input file {} is not a .tex, .sty, .cls, .dtx, .ins, or .bib file",
                path.display()
            );
        }
        FileDiscoveryError::WalkError { path, message } => {
            eprintln!(
                "meaning: failed while scanning {}: {message}",
                path.display()
            );
        }
    }
    ...
    let outcomes: Vec<Option<String>> = files
        ...
        .map(|(path, kind)| {
            let content = match std::fs::read_to_string(path) {
                Ok(content) => content,
                Err(err) => return Some(format!("meaning: cannot read {}: {err}", path.display())),
            };
            let mut style = style;
            style.wrap = wrap_override.unwrap_or_default();
            let formatted = match kind {
                ...
                .map_err(|e| e.to_string()),
                FileKind::Bib => meaning_analysis::bib::format_with_style(&content, style)
                    .map_err(|e| e.to_string()),
            };
            match formatted {
                Ok(formatted) => {
                    ...
                    {
                        return Some(format!("meaning: cannot write {}: {err}", path.display()));
                    }
                    None
                }
                Err(msg) => Some(format!("meaning: cannot format {}: {msg}", path.display())),
            }
        })
        .collect();
...
fn comment_sequence(text: &str, config: LexConfig) -> String {
    parse_with_flavor(text, config)
        ...
        .filter(|token| token.kind() == meaning_parser::syntax::SyntaxKind::COMMENT)
...
///
/// Idempotency formats twice through the same pipeline `meaning format` uses.
...
fn run_debug_checks_for_file(
    ...
    if matches!(checks, DebugChecksArg::Losslessness | DebugChecksArg::All) {
        let reconstructed = match kind {
            FileKind::Bib => meaning_analysis::bib::parse(content).syntax().to_string(),
...
/// `meaning debug format`: check invariants over the discovered files, writing
...
fn run_debug_format(
    ...
    if paths.is_empty() {
        eprintln!("meaning: debug format requires at least one file or directory");
        return ExitCode::from(2);
    }
    let files = match collect_lint_files(paths, exclude) {
        Ok(files) => files,
    ...
    let outcomes: Vec<(String, FileKind, Result<DebugArtifacts, String>)> = files
        ...
        .map(|(path, kind)| {
            ...
            let outcome = match std::fs::read_to_string(path) {
                ...
                Err(err) => Err(format!("meaning: cannot read {label}: {err}")),
            };
            (label, *kind, outcome)
        })
        .collect();
            ...
            Ok(artifacts) => {
                ...
                if let Some(dir) = dump_dir {
                    ...
                    if let Err(err) = write_debug_artifacts(dir, &stem, &artifacts, dump_passes) {
                        eprintln!(
                            "meaning: cannot write debug artifacts to {}: {err}",
                            dir.display()
                        );
                        io_failed = true;
                    }
...
mod tests {
    use super::*;
    use meaning_analysis::linter::Edit;
    ...
    fn no_paths_reads_stdin_unless_it_is_a_terminal() {
        // Behind a pipe, a redirect, or a CI runner, bare `meaning format` keeps
