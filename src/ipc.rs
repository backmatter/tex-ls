...
//! command. So `meaning inverse-search` is a *client* that has to find the server
...
impl std::fmt::Display for IpcError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::NoServer => write!(
                f,
                "no meaning language server is listening for inverse search \
                 (is your editor running one, and does it support window/showDocument?)"
            ),
            Self::NoServerForFile(path) => write!(
                f,
                "no listening meaning language server has {} in its workspace",
                path.display()
            ),
...
/// Where advertisements live: `$MEANING_IPC_DIR`, else the user's runtime
...
pub fn ipc_dir() -> PathBuf {
    if let Some(dir) = std::env::var_os("MEANING_IPC_DIR") {
        return PathBuf::from(dir);
    }
    let base = dirs::runtime_dir().unwrap_or_else(std::env::temp_dir);
    base.join(format!("meaning-{}", user_scope()))
}
...
// is `u32` on every Unix meaning builds for.
#[cfg(unix)]
unsafe extern "C" {
    #[link_name = "getuid"]
    fn libc_getuid() -> u32;
...
impl Drop for Listener {
    fn drop(&mut self) {
        ...
        // The directory itself is left alone: `MEANING_IPC_DIR` may point at one
...
mod tests {
    ...
    fn advertisement_round_trips() {
        let ad = Advertisement {
            ...
            address: "/run/user/1000/meaning-1000/7.sock".to_owned(),
