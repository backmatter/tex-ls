//! CLI/LSP bridge over the [`tex_ls_formatter`] engine.
//!
//! The formatting engine lives in the `tex-ls-formatter` crate; this module
//! re-exports it and hosts the CLI-side concerns that do not belong in the
//! shared engine: the batch path-walking check API ([`check`]) and the
//! disk-backed package-aware entries ([`format_file_with_packages`]), which
//! pull local `.sty`/`.cls` signatures in via `crate::package_source::disk_scope_signatures`.

pub mod check;

pub use tex_ls_formatter::formatter::*;
...
use crate::package_source::disk_scope_signatures;
