...
impl ConfigLoader {
    /// Parse a `meaning.toml` from disk and validate it.
    ...
    /// Walk `start` and its ancestors looking for a `meaning.toml`. Stops at the
    ...
    /// an explicit `--config` path, then a discovered project `meaning.toml`,
    /// then a file named by [`$MEANING_CONFIG`](CONFIG_ENV_VAR), then the
    ...
    fn resolve_with_fallbacks(
        ...
        // A set `$MEANING_CONFIG` shadows the global config entirely, and a
...
pub enum ConfigSource {
    ...
    Discovered(PathBuf),
    /// Named by the [`$MEANING_CONFIG`](CONFIG_ENV_VAR) environment variable,
    /// used when no project config is discovered.
    Env(PathBuf),
    /// The global user config (e.g. `~/.config/meaning/config.toml`), used when
    /// no project config is discovered and `$MEANING_CONFIG` is unset.
    Global(PathBuf),
...
/// Path named by the [`$MEANING_CONFIG`](CONFIG_ENV_VAR) environment variable,
...
/// Path to the global user config, the fallback when no project `meaning.toml`
/// is discovered: the first existing file among
/// `$XDG_CONFIG_HOME/meaning/config.toml`, `~/.config/meaning/config.toml`, and
/// `<platform config dir>/meaning/config.toml` (Windows `%APPDATA%`, macOS
...
fn global_config_path() -> Option<PathBuf> {
    ...
    candidates
        .into_iter()
        .map(|dir| dir.join("meaning").join("config.toml"))
        .find(|path| path.is_file())
}

#[cfg(test)]
use meaning_formatter::formatter::{FormatStyle, ItemIndent, LineEnding, MathWrap, WrapMode};
...
mod tests {
    ...
    fn parse(text: &str) -> Result<ValidatedConfig, ConfigError> {
        Config::parse_str(text, Path::new("meaning.toml"))
    }
    ...
    fn write_global(dir: &Path) -> PathBuf {
        let global = dir.join("config-home").join("meaning").join("config.toml");
    ...
    fn write_env_config(dir: &Path) -> PathBuf {
        let env = dir.join("synced").join("meaning.toml");
    ...
    fn missing_env_config_is_an_error() {
        let (dir, repo) = bounded_project();
        let env = dir.path().join("nowhere").join("meaning.toml");
        let global = write_global(dir.path());

        // A set but dangling `$MEANING_CONFIG` must not silently fall through
        // to the global config or the defaults.
        let err =
            ConfigLoader::resolve_with_fallbacks(None, false, &repo, Some(&env), Some(&global))
                .expect_err("typo'd $MEANING_CONFIG path must not be silently ignored");
        assert!(matches!(err, ConfigError::Io { .. }));
    }
    ...
    fn broken_env_config_is_an_error() {
        let (dir, repo) = bounded_project();
        let env = dir.path().join("synced").join("meaning.toml");
        fs::create_dir_all(env.parent().unwrap()).unwrap();
        fs::write(&env, "[format]\nline-widht = 80\n").unwrap();

        let err = ConfigLoader::resolve_with_fallbacks(None, false, &repo, Some(&env), None)
            .expect_err("broken $MEANING_CONFIG file must not be silently ignored");
        assert!(matches!(err, ConfigError::Parse { .. }));
    }

    #[test]
    fn exclude_root_for_project_config_is_its_directory() {
        let source = ConfigSource::Discovered(PathBuf::from("/proj/meaning.toml"));
        assert_eq!(
    ...
    //
    // The type and its wire spellings are tested in `meaning_parser::declarations`;
    ...
    /// The rules themselves are tested in `meaning_parser::declarations`; what
    ...
    fn a_broken_declaration_fails_at_load() {
        ...
        let rendered = err.to_string();
        assert!(rendered.contains("meaning.toml"), "{rendered}");
    ...
    fn a_declared_opener_without_a_closer_loads() {
        ...
        assert_eq!(
            entry.begin,
            vec![meaning_parser::declarations::CommandName::new("bea")]
        );
