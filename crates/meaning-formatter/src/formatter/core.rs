...
/// parse cleanly. Note: meaning's [`crate::parser::Parse`] carries `errors` +
/// `syntax()`. Uses the
...
/// abbreviations). The CLI resolves these from `meaning.toml`; other wrap modes
/// ignore them.
...
/// a formatter honoring `[environments.…]` for `meaning format file.tex` but not
/// for `meaning format < file.tex` (nor for the one editor request that races an
/// edit) would be a trap.
///
/// The declarations reach **both** the parse and the signature scope, exactly as
/// they do on the path-bearing entries in the `meaning` crate
...
/// `incremental::scope_signatures`, both in the `meaning` crate), with nothing
/// under it.
...
/// `incremental::scope_signatures`, both in the `meaning` crate) — into the lowering, so calls to
...
/// salsa-cached `incremental::scope_signatures` (in the `meaning` crate) here; the document's own
...
/// span. The caller (the `meaning` crate's LSP) expands the editor selection to whole
...
///
/// `pub` so the linter (in the `meaning` crate) shares the *same* region
...
struct LowerCtx<'a> {
    ...
    /// (`% meaning-format off`/`skip`/`skip-file` and the combined `% meaning`
...
/// `meaning` crate) recognizes the same shape; single-sourced so the two never
/// drift.
