...
use meaning_formatter::declarations::{Declarations, ResolvedDeclarations};
...
use meaning_formatter::parser::{LatexFlavor, LexConfig, parse, parse_with_flavor, reconstruct};
use meaning_formatter::semantic::SignatureDb;
use meaning_formatter::syntax::SyntaxKind;
...
/// parsing under a project's [declarations](meaning_parser::declarations).
///
...
/// spellings are self-consistent fixed points. `meaning debug format --checks
...
fn format_invariants_corpus() {
    let dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("../meaning-parser/tests/corpus");
    let mut count = 0;
...
fn env_alias_resolves_with_no_external_signatures() {
    ...
    // or the plugin would become a second sanctioned divergence from `meaning
...
// ---------------------------------------------------------------------------
// Comment directives (`% meaning-format …`, `% meaning …`)
// ---------------------------------------------------------------------------
...
fn format_skip_preserves_the_documented_construct() {
    let src = format!("% meaning-format skip: hand-aligned\n{HAND_ALIGNED}");
...
fn format_off_on_preserves_every_block_between_them() {
    let src = format!(
        "% meaning-format off\n{HAND_ALIGNED}\n{HAND_ALIGNED}% meaning-format on\n{HAND_ALIGNED}"
    );
    let out = format(&src).expect("formats");
    let (suppressed, formatted) = out
        .split_once("% meaning-format on\n")
        .expect("the closer survives");
    assert_eq!(
        suppressed,
        &src[..src.find("% meaning-format on").expect("has closer")],
...
fn format_skip_file_preserves_the_whole_document() {
    let src = format!("% meaning-format skip-file: generated\n{HAND_ALIGNED}");
...
/// The bare `% meaning` family suppresses layout too — it is the combined axis,
/// not a lint-only one.
#[test]
fn combined_family_suppresses_layout() {
    let src = format!("% meaning skip: leave it alone\n{HAND_ALIGNED}");
    assert_eq!(format(&src).expect("formats"), src);
    assert_format_invariants(&src);
}
...
fn unrecognized_and_lint_directives_do_not_suppress_layout() {
    for lead in [
        ...
        "% meaning-ignore-file: noisy",
        "% meaning-format nonsense",
        "% meaning",
        "% an ordinary comment",
    ] {
        let src = format!("{lead}\n{HAND_ALIGNED}");
...
fn unclosed_off_runs_to_end_of_file() {
    let src = format!("% meaning-format off\n{HAND_ALIGNED}\n{HAND_ALIGNED}");
    assert_eq!(format(&src).expect("formats"), src);
    assert_format_invariants(&src);
}
...
// --- declared environments (`meaning.toml`) ----------------------------------
