...
use meaning_formatter::bib::semantic::Model;
use meaning_formatter::bib::syntax::SyntaxKind;
use meaning_formatter::bib::{ast, format, format_with_style, parse, reconstruct};
use meaning_formatter::formatter::{FormatStyle, LineEnding};
...
fn field_values(text: &str) -> Vec<(String, String)> {
    fn signature(value: &meaning_formatter::bib::syntax::SyntaxNode) -> String {
        value
...
fn format_invariants_corpus() {
    let dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("../meaning-parser/tests/bib_corpus");
    let mut count = 0;
