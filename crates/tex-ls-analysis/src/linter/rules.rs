...
//! `tex-ls.toml` `[lint]` `select`/`ignore` keys (and the CLI's matching flags)
...
pub struct RuleContext<'a> {
    ...
    /// rules, on the first request for a mode.
    mode_index: OnceLock<ModeIndex>,
    /// The `\if…\else…\fi` branch path per byte offset, computed on first use so the
    ...
    conditionals: OnceLock<super::conditional::ConditionalIndex>,
...
impl<'a> RuleContext<'a> {
    /// Assemble the context for one file with lazy shared math/conditional
    /// indexes. `resolution`/`citations`/`packages` are `None` when there is no
    /// project view.
    pub fn new(
        path: &'a Path,
        root: &'a SyntaxNode,
        ...
        Self {
            ...
            citations,
            packages,
            mode_index: OnceLock::new(),
            conditionals: OnceLock::new(),
    ...
    pub(crate) fn conditional_path_at(&self, offset: usize) -> &[super::conditional::Frame] {
        self.conditionals
            .get_or_init(|| super::conditional::ConditionalIndex::compute(self.root))
            .path_at(offset)
    }

    pub fn mode_at(&self, offset: usize) -> Mode {
        self.mode_index
            .get_or_init(|| ModeIndex::build(self.root))
            .mode_at(offset)
    }
...
/// control-word swap: `\let\x\rmfamily` copies a *different* semantics, and where
/// this idiom guards the plain-TeX/ConTeXt branch, `\rmfamily`/`^` is undefined.
/// Redefinitions (a new semantics) are handled upstream by `user_definitions` — they
...
/// tex-ls-bearing minus into an en dash. Curated and deliberately small like the
...
/// not built-ins, project declarations, or loaded-package signatures.
...
/// number-range en-dash rewrite would corrupt a tex-ls-bearing minus.
...
/// subtraction, whose en-dash rewrite would corrupt a tex-ls-bearing minus.
...
///
/// The rule reference (`docs/reference/linter-rules.md`) is generated by
...
pub trait Rule: Send + Sync {
    ...
    /// targeted by `% tex-ls-lint skip <id>`.
