...
/// Whether a [`Fix`] preserves the document's tex-ls. `Safe` fixes are applied
...
/// action. Serialized lowercase (`"safe"`).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize)]
#[serde(rename_all = "lowercase")]
...
pub struct Fix {
    ...
    pub edits: Vec<Edit>,
    /// Whether applying the fix preserves tex-ls.
...
impl Fix {
    /// A semantics-preserving fix with a single contiguous replacement.
    pub fn safe(
        start: usize,
        end: usize,
    ...
    /// A single-replacement fix that may change semantics; applied only on
    /// explicit opt-in.
    pub fn unsafe_(
        start: usize,
        end: usize,
    ...
    /// A semantics-preserving fix touching one or more disjoint spans.
    ...
    /// A multi-span fix that may change semantics; applied only on explicit
    /// opt-in.
