...
//! `docs/reference/linter-rules.md` and the pinned snapshots can never
...
/// pass their [`Rule::example_path`] so
/// their examples fire.
...
pub(crate) fn render_rule_section(
    ...
    for example in section.examples {
        ...
        // Safe fixes only, matching what `tex-ls lint --fix` applies.
...
/// or `None` if the id is unknown. Backs `tex-ls lint --explain <rule>`, reusing
...
rule's `description()`/`examples()` in `crates/tex-ls-analysis/src/linter/rules/` and regenerate. -->
