...
impl Rule for MissingProvides {
    ...
    fn description(&self) -> &'static str {
        "Report a `.sty` file without `\\ProvidesPackage` or a `.cls` file without \
         `\\ProvidesClass`. A declaration of the wrong kind does not satisfy the rule. Other file \
         types are excluded. No fix is offered because the author must choose the declaration's \
         placement, date, and version."
    }
