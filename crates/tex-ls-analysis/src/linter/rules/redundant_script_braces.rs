...
//! argument that can be dropped without changing semantics (`x^{2}` -> `x^2`,
//! `y_{\alpha}` -> `y_\alpha`).
//!
...
//! the *unspaced* text. tex-ls's catcode-faithful lexer globs a run of word
//! characters into one `WORD` (`is_word_char`), and a bare `^`/`_` binds that
//! whole `WORD`, so removing the braces is semantics-preserving only when the
...
//!   would bind `2-3` — a semantics change. (The formatter *used* to strip this
...
impl Rule for RedundantScriptBraces {
    ...
    fn description(&self) -> &'static str {
        "Report braces around a single-token subscript or superscript. The safe fix removes the \
         braces unless adjacent text would change tokenization. For example, `x^{2}-3` and \
         `y_{\\alpha}b` stay braced because `2-3` and `\\alphab` would become single tokens. Named \
         math operators such as `\\max` also retain braces because they cannot be unbraced script \
         fields."
    }
