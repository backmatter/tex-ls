...
//! whose LaTeX form is a single, tex-ls-identical token (`\sb`/`\sp`, the plain
...
/// construct named in the message, and — where a 1:1 tex-ls-identical
...
impl Rule for PrimitiveCommand {
    ...
    fn description(&self) -> &'static str {
        "Report plain-TeX primitives with LaTeX alternatives. Most replacements restructure \
         arguments, such as `\\over` becoming `\\frac`, so no automatic fix is offered. \
         tex-ls-equivalent replacements such as `\\sb` to `_` and `\\sp` to `^` have safe fixes. \
         Locally redefined names are excluded, and commands referenced by `\\let` or `\\ifx` \
         receive no fix."
    }
    ...
    fn check(&self, el: &SyntaxElement, ctx: &RuleContext<'_>, sink: &mut Vec<Diagnostic>) {
        ...
        // replacement is a single tex-ls-identical token, so it is correct by
