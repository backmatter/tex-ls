...
impl Rule for UnknownOption {
    ...
    fn description(&self) -> &'static str {
        "Report package options absent from a loaded local package's `\\DeclareOption` \
         ...
         A default handler, key-value processor, option forwarding, or `\\input` makes the \
         package's declared set incomplete and disables the check. No fix is offered because \
         removing or renaming an option changes the requested behavior."
    }
...
mod tests {
    ...
    fn findings_with(src: &str, stys: &[(&str, &str)]) -> Vec<Diagnostic> {
        let facts = stys.iter().filter_map(|(path, sty_src)| {
            let kind = crate::source::file_kind_or_tex(Path::new(path));
