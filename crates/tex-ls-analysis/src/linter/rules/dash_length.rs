...
//! would corrupt a tex-ls-bearing minus. Angle-delimited command and environment
...
impl Rule for DashLength {
    ...
    fn description(&self) -> &'static str {
        "Report incorrect dash lengths in text. Numeric ranges such as `5-10` receive an unsafe \
         fix to `5--10`. An en dash between words receives no fix because the intended dash is \
         ambiguous; names with an uppercase letter on either side are excluded. The rule checks \
         only words containing one internal dash run. It skips dates with several dashes, ISBNs, \
         spaced dashes, flags, column specifications, label/citation keys, Beamer overlays, math, \
         comments, and verbatim text."
    }
    ...
    fn check(&self, el: &SyntaxElement, ctx: &RuleContext<'_>, sink: &mut Vec<Diagnostic>) {
        ...
        // en-dash rewrite would corrupt a tex-ls-bearing minus. Skip inside a
