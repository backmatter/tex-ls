...
impl Rule for DuplicatePackage {
    ...
    fn description(&self) -> &'static str {
        "Report repeated `\\usepackage` or `\\RequirePackage` loads in one file. Loads in mutually \
         exclusive conditional branches are excluded. Macros such as `\\ifthenelse` do not define \
         recognized branches, and class loads are not checked. No fix is offered because removing \
         a load may discard options."
    }
