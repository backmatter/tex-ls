...
impl Rule for MathOperatorName {
    ...
    fn description(&self) -> &'static str {
        "Report operator names such as `sin`, `cos`, `log`, and `lim` written without a backslash \
         in math. Names must start a word and end at a word boundary; longer matches such as \
         `sinh` take precedence. Subscripts, superscripts, text arguments, and unknown arguments \
         are excluded. Adding the backslash requires unsafe opt-in because it changes glyphs and \
         spacing, and the letters could represent a product."
    }
