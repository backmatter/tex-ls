...
//! argument, not "a swallowed space before text"), and we would need semantics we
...
impl Rule for SwallowedSpace {
    ...
    fn description(&self) -> &'static str {
        "Report spaces that TeX discards after argumentless logo commands such as `\\LaTeX`, \
         `\\TeX`, and `\\BibTeX`. For example, `\\LaTeX is` prints \"LaTeXis\". The rule applies only \
         in text before an alphanumeric word. The fix inserts `{}` after the command and requires \
         unsafe opt-in because it changes typeset output."
    }
...
mod tests {
    ...
    fn non_logo_command_is_left_alone() {
        ...
        // (`\foo bar`) from a swallowed space without semantics we do not model.
