...
//! two gates apply (from [`ResolvedCitations`]):
//!
...
impl Rule for UndefinedCitation {
    ...
    fn description(&self) -> &'static str {
        "Report citation keys absent from a document's bibliography. The rule requires a rooted \
         project with every bibliography resource resolved to an analyzed file. Resource lookup \
         respects `BIBINPUTS` and `TEXBIB`. A `\\nocite{*}` marks entries as used but does not \
         define explicit missing keys. Without cross-file citation resolution, the rule is \
         inactive. No automatic fix is offered."
    }
    ...
    fn check_file(&self, ctx: &RuleContext<'_>, sink: &mut Vec<Diagnostic>) {
        ...
        if !citations.is_closed(ctx.path) || !citations.is_root_component(ctx.path) {
            return;
        }
...
mod tests {
    ...
    #[test]
    fn wildcard_nocite_does_not_hide_missing_explicit_keys() {
        let r = resolution(&[], true, true);
        assert_eq!(findings("\\cite{anything}\n", Some(&r)).len(), 1);
    }
