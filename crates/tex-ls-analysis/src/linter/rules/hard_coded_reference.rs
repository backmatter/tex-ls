...
impl Rule for HardCodedReference {
    ...
    fn description(&self) -> &'static str {
        "Report numbered references such as `Figure 3`, `Table~1`, or `Section 2`. Literal \
         numbers do not update when the document changes. The rule requires a capitalized \
         reference word followed by one space or tie and an Arabic number. It excludes plurals, \
         lowercase forms, reference commands, citation locators, environment titles, item labels, \
         math, comments, and verbatim text. No fix is offered because the target label is \
         unknown."
    }
