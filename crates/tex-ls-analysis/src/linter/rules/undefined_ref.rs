...
impl Rule for UndefinedRef {
    ...
    fn description(&self) -> &'static str {
        "Report references to labels absent from the document. The rule requires a rooted project \
         with every include resolved to an analyzed file. It is inactive for stdin or without \
         cross-file label resolution. No automatic fix is offered."
    }
