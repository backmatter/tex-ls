...
impl Rule for MissingNonbreakingSpace {
    ...
    fn description(&self) -> &'static str {
        "Report a space or single newline before a bare-number reference or bracketed citation, \
         such as `Figure \\ref{x}` or `see \\cite{a}`. A tie keeps the reference with its preceding \
         word. Self-describing references such as `\\autoref` and textual citations such as \
         `\\textcite` are excluded. A same-line space has an unsafe fix to `~` because line \
         breaking changes. A newline receives no fix; blank lines are not reported."
    }
