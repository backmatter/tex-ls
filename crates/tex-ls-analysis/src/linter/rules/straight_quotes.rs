...
impl Rule for StraightQuotes {
    ...
    fn description(&self) -> &'static str {
        "Report ASCII double quotes used for quotation. LaTeX uses two backticks to open a \
         quotation and two apostrophes to close it. A paired quotation produces one finding and \
         one fix; an unpaired quote produces its own finding. The fix infers direction from \
         surrounding whitespace and delimiters, so it requires unsafe opt-in. Apostrophes, math, \
         comments, verbatim text, TeX hex constants, and `\\pdfmapline` font maps are excluded."
    }
