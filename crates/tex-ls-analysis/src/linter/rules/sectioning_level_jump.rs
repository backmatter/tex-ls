...
//! heading or inserting an intermediate heading — a structural, semantics-changing
...
//! [`signature`] DB directly, like
//! [`outline()`](fn@crate::semantic::outline) — sectioning is a static standard set, so
...
impl Rule for SectioningLevelJump {
    ...
    fn description(&self) -> &'static str {
        "Report a downward heading jump that skips a structural level, such as `\\section` \
         followed by `\\subsubsection`. The hierarchy includes `\\chapter` when the class supports \
         it or the source uses it. Paragraph and subparagraph headings do not affect the \
         comparison. Repeated levels and upward jumps are allowed. No fix is offered because the \
         intended structure is unknown."
    }
