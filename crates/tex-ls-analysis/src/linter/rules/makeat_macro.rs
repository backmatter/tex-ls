...
impl Rule for MakeatMacro {
    ...
    fn description(&self) -> &'static str {
        "Report a command containing `@` outside a `\\makeatletter` region. The lexer splits such \
         a name into a command and following text, so TeX will not call the intended macro. Names \
         inside the region and sentence-spacing `\\@` are excluded. No fix is offered because the \
         author must choose the region's boundaries."
    }
