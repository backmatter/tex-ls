...
//! replacement requires explicit unsafe-fix opt-in: column conventions, spacing
//! and package requirements differ. The atomic edit renames the matched pair;
//! users must review the body and ensure the replacement environment is loaded.
//!
...
impl Rule for ObsoleteEnvironment {
    ...
    fn description(&self) -> &'static str {
        "Flag math environments the community has superseded, naming the modern \
         ...
         an unsafe change requiring review of columns and package availability."
    }
...
/// An explicit unsafe swap of an environment's name at both ends (`eqnarray` → `align`):
...
mod tests {
    ...
    #[test]
    fn alignment_swap_requires_opt_in() {
    ...
    fn swaps_starred_variant() {
        ...
        assert_eq!(
            apply_fixes(src, &[fix], true).output,
            "\\begin{align*}\na\n\\end{align*}\n"
        );
    }
    ...
    fn reports_but_withholds_fix_when_unterminated() {
        let out = findings("\\begin{eqnarray}\na &=& b\n");
        assert_eq!(out.len(), 1);
        assert!(out[0].fix.is_none(), "no `\\end` -> no atomic swap");
    }
}
