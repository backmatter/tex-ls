...
impl Rule for BlankLineInKeyval {
    ...
    fn description(&self) -> &'static str {
        "Report a blank line between entries in a closed, curated `key=value` argument. TeX \
         treats the blank line as `\\par`, which aborts the keyval processor. Blank lines inside a \
         value's braces are allowed; unclosed groups receive parse diagnostics instead. The safe \
         fix removes the blank line and preserves the following indentation."
    }
