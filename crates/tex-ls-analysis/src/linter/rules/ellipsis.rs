...
//!   following word; TeX absorbs that space, so semantics are preserved. Correct by
...
impl Rule for Ellipsis {
    ...
    fn description(&self) -> &'static str {
        "Report three or more literal periods. In text, the safe fix uses `\\dots` and separates \
         it from a following letter. In math, the fix chooses `\\cdots` beside an operator or \
         relation and `\\ldots` elsewhere. This choice requires unsafe opt-in because it changes \
         the glyph placement. Comments and verbatim text are excluded."
    }
    ...
    fn check(&self, el: &SyntaxElement, ctx: &RuleContext<'_>, sink: &mut Vec<Diagnostic>) {
        ...
        while i < bytes.len() {
            ...
            // following ASCII letter; TeX absorbs it, so semantics are preserved.
