//! `deprecated-suppression-syntax`: retired `% tex-ls-ignore` suppression
...
//! `tex-ls-ignore` with `tex-ls-lint skip`, or `tex-ls-ignore-file` with
//! `tex-ls-lint skip-file`, preserves the selector and reason verbatim and
...
const EXAMPLES: &[Example] = &[Example {
    ...
    source: "% tex-ls-ignore deprecated-command: legacy source\n{\\bf text}\n",
}];
...
impl Rule for DeprecatedSuppressionSyntax {
    ...
    fn description(&self) -> &'static str {
        "Flag the retired `% tex-ls-ignore <rule>` and `% tex-ls-ignore-file \
         ...
         and verb to `% tex-ls-lint skip <rule>` or `% tex-ls-lint skip-file \
    ...
    fn check_file(&self, ctx: &RuleContext<'_>, sink: &mut Vec<Diagnostic>) {
        for located in ctx.suppressions.directives().iter().filter(|located| {
            ...
            let replacement = match located.directive.verb {
                Verb::Skip => "tex-ls-lint skip",
                Verb::SkipFile => "tex-ls-lint skip-file",
...
mod tests {
    ...
    fn rewrites_node_suppression_safely() {
        ...
        assert_eq!(&src[out[0].start..out[0].end], "tex-ls-ignore");
    ...
    fn rewrites_file_suppression_and_preserves_reason() {
        let src = "%%% tex-ls-ignore-file: generated file\ntext\n";
        let out = findings(src);
        assert_eq!(out.len(), 1);
        let fix = out[0].fix.as_ref().expect("has fix");
        assert_eq!(
            apply_fixes(src, std::slice::from_ref(fix), false).output,
            "%%% tex-ls-lint skip-file: generated file\ntext\n"
        );
    }

    #[test]
    fn modern_and_ordinary_comments_are_fine() {
        assert!(findings("% tex-ls-lint skip deprecated-command\n\\bf\n").is_empty());
        assert!(findings("% tex-ls is good\n").is_empty());
    }

    #[test]
    fn does_not_rewrite_dtx_documentation_prose() {
        let src = "% tex-ls-ignore deprecated-command\nDocumentation.\n";
