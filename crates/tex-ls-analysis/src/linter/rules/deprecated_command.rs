...
//! offered as an explicit, unsafe replacement: legacy switches reset font
//! attributes and have separate math behavior, so a direct modern declaration
//! cannot preserve semantics in every context. `\em` remains supported.
//!
...
impl Rule for DeprecatedCommand {
    ...
    fn description(&self) -> &'static str {
        "Report the obsolete font switches `\\bf`, `\\it`, `\\rm`, `\\sf`, `\\tt`, `\\sc`, and `\\sl`. \
         The supported `\\em` switch and locally redefined names are excluded. The fix replaces \
         the command with its LaTeX declaration, such as `\\bfseries`, but requires unsafe opt-in \
         because font reset and math behavior differ. Commands used as references in forms such \
         as `\\let` or `\\ifx` receive no fix."
    }
    ...
    fn check(&self, el: &SyntaxElement, ctx: &RuleContext<'_>, sink: &mut Vec<Diagnostic>) {
        ...
        // `\let`/`\ifx` forms (which carry no new semantics) fall through and are
        ...
        // replacement copies in the modern declaration. Font resets and math
        // behavior differ, so this is an explicit unsafe fix. It is withheld in two cases: the fallback span,
        ...
        // than executed, so the swap changes semantics (and `\rmfamily` may not even
...
mod tests {
    ...
    fn reference_position_reports_without_a_fix() {
        ...
        // changes semantics (and `\rmfamily` may not even exist, as in plain-TeX/ConTeXt
    ...
    #[test]
    fn control_word_replacement_requires_opt_in() {
