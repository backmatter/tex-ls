...
impl Rule for MissingRequiredArgument {
    ...
    fn description(&self) -> &'static str {
        "Report missing required arguments from curated command signatures. Because TeX accepts \
         single-token arguments such as `\\frac12`, a finding requires a hard boundary: a closing \
         group, math or environment boundary, alignment separator, line break, blank line, or end \
         of file. Macro definitions, unknown-command arguments, standalone scope groups, aliases, \
         and locally redefined commands are excluded. No fix can supply the missing content."
    }
