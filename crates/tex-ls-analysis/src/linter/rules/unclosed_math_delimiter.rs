...
impl Rule for UnclosedMathDelimiter {
    ...
    fn description(&self) -> &'static str {
        "Report unmatched `$`, `\\[`, `\\(`, or `\\left` openers in document prose when the parser \
         retained them as plain tokens. Macro code can use these tokens literally, so the rule \
         skips brace groups, optional arguments, expl3 regions, and `macrocode` bodies. No fix is \
         offered because adding a closer and removing the opener are both possible corrections."
    }
