...
impl Rule for SpaceBeforeCommand {
    ...
    fn description(&self) -> &'static str {
        "Report a same-line space between a word and `\\footnote`, `\\footnotemark`, `\\index`, or \
         `\\label`. Removing it requires unsafe opt-in because typeset spacing changes. Spaces \
         after braces, at line starts, and inside math are excluded. For `\\index` and `\\label`, \
         the fix also requires whitespace or paragraph end after the argument so it cannot join \
         surrounding words."
    }
