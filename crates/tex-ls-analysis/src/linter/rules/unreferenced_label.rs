...
impl Rule for UnreferencedLabel {
    ...
    fn description(&self) -> &'static str {
        "Report labels with no reference in the document. The rule requires a rooted project with \
         every include resolved to an analyzed file. It is inactive for stdin or without \
         cross-file label resolution. No fix is offered because the author may want to add a \
         reference or remove the label."
    }
