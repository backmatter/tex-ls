...
impl Rule for AbbreviationSpacing {
    ...
    fn description(&self) -> &'static str {
        "Report incorrect spacing after abbreviations and acronyms. Before a lowercase word, \
         abbreviations such as `e.g.` need `\\ ` to prevent sentence spacing. Before an uppercase \
         word, a sentence ending in two or more capitals needs `\\@` before its period. Initials, \
         dotted acronyms, math, comments, and verbatim text are excluded. The rule is disabled \
         under `\\frenchspacing`. Both fixes require unsafe opt-in because they change typeset \
         spacing."
    }
