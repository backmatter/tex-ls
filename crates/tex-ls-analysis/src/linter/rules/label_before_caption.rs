...
impl Rule for LabelBeforeCaption {
    ...
    fn description(&self) -> &'static str {
        "Report a `\\label` before the caption or first numbered item that establishes its \
         counter. Such a label captures the previous counter and can print the wrong reference \
         number. The rule checks curated floats, `\\captionof` containers, and the standard \
         `enumerate` list. It skips labels nested in command arguments and ambiguous counter \
         changes. The fix moves the label after the caption or item marker while preserving \
         custom item labels and Beamer overlays. It requires unsafe opt-in because it changes \
         reference output."
    }
