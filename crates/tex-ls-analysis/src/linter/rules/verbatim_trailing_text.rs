...
impl Rule for VerbatimTrailingText {
    ...
    fn description(&self) -> &'static str {
        "Report text after a verbatim environment's closing delimiter on the same line. TeX \
         discards that text when it closes the environment. Trailing comments and ordinary \
         environments are excluded. No fix is offered because the author must decide whether to \
         move or remove the text."
    }
