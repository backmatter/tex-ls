...
impl Rule for TimesVariable {
    ...
    fn description(&self) -> &'static str {
        "Report a lowercase `x` between two ASCII integers, such as `640x200`. The unsafe fix \
         uses `\\times` in math and `$\\times$` in text. Ordinary words, spaced products, \
         hexadecimal literals, and label/reference keys are excluded. Unsafe opt-in is required \
         because the `x` could be an intended variable."
    }
