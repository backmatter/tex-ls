...
//! ([`builtin`]); `@string` macro names from the
...
/// A TeX control word is completed only inside a braced/quoted field value.
/// URL/file-like fields and ordinary bibliography identifiers stay literal.
pub fn tex_command_prefix(root: &SyntaxNode, offset: usize) -> Option<String> {
    tex_command_completion(root, offset).map(|(_, prefix)| prefix)
}

/// Full control-word/symbol span (excluding the backslash) and the typed prefix.
/// BibTeX stores TeX prose as value text, so apply TeX's token boundary here.
pub fn tex_command_completion(
    root: &SyntaxNode,
    offset: usize,
    ...
    let word = |ch: char| ch.is_alphabetic() || "@_:".contains(ch);
    let symbol_start = prefix.char_indices().next_back().and_then(|(at, ch)| {
        (!word(ch) && ch != '\\' && !ch.is_whitespace() && prefix[..at].ends_with('\\'))
            .then(|| at - 1)
    });
    let start = symbol_start.or_else(|| {
        prefix
            .char_indices()
            .rev()
            .find(|(_, ch)| !word(*ch))
    ...
    // A doubled backslash is not the start of a control word.
    if prefix[..start]
        .bytes()
        .rev()
    ...
    let token = root
        .token_at_offset(TextSize::from(start as u32))
        .right_biased()?;
    let ancestors: Vec<_> = token.parent_ancestors().collect();
    if !ancestors
        .iter()
        .any(|node| matches!(node.kind(), SyntaxKind::BRACE_GROUP | SyntaxKind::QUOTED))
    {
        return None;
    }
    let field = ancestors
        .iter()
        .find(|node| node.kind() == SyntaxKind::FIELD)?;
    if builtin().category(&ast::field_name(field)?) == crate::bib::semantic::FieldCategory::Verbatim
    {
        return None;
    }
    let name = start + 1;
    let end = match source[name..].chars().next() {
        Some(ch) if word(ch) => source[name..]
            .char_indices()
            .find(|(_, ch)| !word(*ch))
            .map_or(source.len(), |(at, _)| name + at),
        Some(ch) if !ch.is_whitespace() => name + ch.len_utf8(),
        _ => name,
    };
    (offset <= end).then(|| {
        (
            rowan::TextRange::new(TextSize::from(name as u32), TextSize::from(end as u32)),
            source[name..offset].into(),
        )
    })
}
