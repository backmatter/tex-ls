...
//!
//! **Suppression** is carried in `@comment{tex-ls-lint …}` entries (bib has no
//! `%` line-comment token), parsed by [`suppression::BibSuppressionMap`] — the bib
//! analog of the LaTeX `% tex-ls-lint` directive.
//!
...
pub use check::{check_document, lint_document, lint_document_with_project};
