...
//! (wrapping in braces is a semantics-preserving edit, but choosing *what* to protect
...
impl BibRule for TitleCapitalization {
    ...
    fn description(&self) -> &'static str {
        "Report unprotected acronyms and mid-word capitals in title fields. Bibliography styles \
         may lowercase them unless braces protect the spelling, as in `{DNA}`. The rule detects \
         consecutive capitals and names such as `iPhone`. It excludes ordinary title case, names \
         such as `McDonald`, mixed-case words such as `LaTeX`, and already braced text. No fix is \
         offered because the author must choose which spelling to preserve."
    }
