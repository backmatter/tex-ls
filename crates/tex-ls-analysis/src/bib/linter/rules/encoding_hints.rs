...
impl BibRule for EncodingHints {
    ...
    fn description(&self) -> &'static str {
        "Report non-ASCII field text as a hint. Accented text is valid when the file encoding and \
         TeX toolchain agree. Check the encoding or use an escape such as `\\'e` for `é`. No fix \
         is offered because the choice depends on the project's toolchain."
    }
