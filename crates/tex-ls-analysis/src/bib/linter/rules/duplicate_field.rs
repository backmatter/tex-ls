...
//! deleting the redundant one is semantics-preserving — we offer a safe deletion via
//! [`super::edits::field_deletion_fix`]. When the values **differ**,
//! which one wins is engine/style-dependent and dropping either changes semantics,
...
const EXAMPLES: &[Example] = &[
    ...
    Example {
        caption: "Differing values are report-only (which copy the engine keeps is \
                  style-dependent, so dropping either would change semantics):",
        source: "@misc{knuth84,\n  note = {First draft},\n  note = {Second draft}\n}\n",
    },
];
...
impl BibRule for DuplicateField {
    ...
    fn description(&self) -> &'static str {
        "Report repeated field names within an entry, compared case-insensitively. Every \
         occurrence after the first is reported. A safe fix removes a repeated value when it \
         matches the retained value byte for byte. Different values receive no fix because the \
         retained occurrence depends on the bibliography engine."
    }
    ...
    fn check(&self, el: &SyntaxElement, _ctx: &BibRuleContext<'_>, sink: &mut Vec<Diagnostic>) {
        ...
        // Walk fields in source order. The map keeps each name's *first* value node
        // (lowercased name → first `VALUE`), so a later occurrence is flagged and
        ...
        let mut first_value: HashMap<String, Option<SyntaxNode>> = HashMap::new();
        for field in fields(entry) {
            ...
            // is rendered only for repeated names. Identity compares the actual `{…}`/`"…"` text, not stray whitespace.
