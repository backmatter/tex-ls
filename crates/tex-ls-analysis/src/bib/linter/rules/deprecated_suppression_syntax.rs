...
const EXAMPLES: &[Example] = &[Example {
    ...
    source: "@comment{tex-ls-ignore unused-string: intentional}\n@string{x = {X}}\n",
}];
...
impl BibRule for DeprecatedSuppressionSyntax {
    ...
    fn description(&self) -> &'static str {
        "Flag the retired `@comment{tex-ls-ignore <rule>}` and \
         `@comment{tex-ls-ignore-file [<rule>]}` suppression spellings, which \
         remain accepted for compatibility but are no longer documented. The \
         Safe autofix rewrites only the family and verb to `tex-ls-lint skip` or \
         `tex-ls-lint skip-file`; the selector, reason, delimiters, and remaining \
...
mod tests {
    ...
    fn rewrites_node_suppression_safely() {
        let src = "@comment{tex-ls-ignore unused-string: intentional}\n@string{x = {X}}\n";
    ...
    fn rewrites_parenthesized_file_suppression() {
        let src = "@comment(tex-ls-ignore-file: imported)\n@misc{k}\n";
        let out = findings(src);
        assert_eq!(out.len(), 1);
        let fix = out[0].fix.as_ref().expect("has fix");
        assert_eq!(
            apply_fixes(src, std::slice::from_ref(fix), false).output,
            "@comment(tex-ls-lint skip-file: imported)\n@misc{k}\n"
        );
    }

    #[test]
    fn modern_and_ordinary_comment_entries_are_fine() {
        assert!(findings("@comment{tex-ls-lint skip unused-string}\n").is_empty());
