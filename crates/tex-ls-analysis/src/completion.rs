...
use crate::semantic::roles::{ArgumentRole, key_argument_role};
...
impl FileArgKind {
    pub fn from_role(kind: tex_ls_parser::semantic::roles::FileRoleKind) -> Self {
        ...
        match kind {
            ...
            FileRoleKind::Inkscape => Self::Inkscape,
            FileRoleKind::Raw => Self::Raw,
        }
    }
...
fn group_context(
    ...
    match (group.kind(), parent.kind()) {
        ...
        (SyntaxKind::GROUP | SyntaxKind::BARE_ARGUMENT, SyntaxKind::COMMAND) => {
...
fn command_arg_context(
    ...
    if matches!(
        key_argument_role(semantic_name, index),
        Some(ArgumentRole::Label)
    ) {
        return Some(CompletionContext::LabelDefinition {
            prefix: group_prefix(group, offset).trim_start().to_string(),
        });
    }
    if matches!(
        key_argument_role(semantic_name, index),
        Some(ArgumentRole::Reference(_))
    ) {
        ...
        && index == path_index
    {
        ...
        return Some(CompletionContext::FilePath { prefix, kind });
    }
...
fn enclosing_group(token: &SyntaxToken) -> Option<SyntaxNode> {
    ...
    while let Some(current) = node {
        match current.kind() {
            SyntaxKind::GROUP | SyntaxKind::NAME_GROUP | SyntaxKind::BARE_ARGUMENT => {
                return Some(current);
            }
            SyntaxKind::COMMAND
            | SyntaxKind::BEGIN
...
fn group_index(command: &SyntaxNode, group: &SyntaxNode) -> Option<usize> {
    command
        .children()
        .filter(|child| matches!(child.kind(), SyntaxKind::GROUP | SyntaxKind::BARE_ARGUMENT))
...
pub fn candidates_with_declarations(
    ...
    match context {
        ...
        CompletionContext::Option { prefix, values, .. }
...
/// Environment name candidates. Structural insertion belongs to protocol conversion,
/// which has the source context needed to preserve existing bodies and delimiters.
...
mod tests {
    ...
    #[test]
    fn begin_name_candidates_leave_structural_insertion_to_protocol() {
        let src = "\\begin{item}\n";
    ...
    fn non_enum_command_arg_is_not_completed() {
        let src = "\\textbf{words}\n";
        assert_eq!(
            classify(src, at(src, "\\textbf{words")),
            CompletionContext::None
        );
    }
