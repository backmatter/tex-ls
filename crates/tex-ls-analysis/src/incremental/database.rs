...
impl Database {
    ...
    pub fn set_file_alias(&mut self, requested: &Path, actual: &Path) -> bool {
        self.set_file_alias_in(self.default_project, requested, actual)
    }
    pub fn set_file_alias_in(
        &mut self,
        project: ProjectId,
        requested: &Path,
        actual: &Path,
        ...
        self.set_file_resolution_observation(
            project,
            requested,
            crate::external::Observation::Present(actual),
        )
    }
    pub fn file_alias_in(&self, project: ProjectId, requested: &Path) -> Option<&Path> {
        let location = self
            .project(project)
            .locations(self)
            .get(&normalize_path(requested))?;
        match location.file_resolution(self) {
            ...
            let locations = input
                ...
                .map(|(path, location)| {
                    let mapped = LocationInput::new(
                        ...
                        location.file_resolution(self).clone(),
                    );
                    (path.clone(), mapped)
                })
                .collect();
