...
//! lexical fact, never tex-ls: the edge fires only when the mandatory group
//! reads exactly `subfiles`, so an ordinary `\documentclass[a4paper]{article}`
//! carries no include edge. See [`subfiles_parent_arg`].
//!
//! Literal bare `\input file` names are attached by the parser. Dynamic names
//! remain unresolved. `includeonly` adds explicit build participation to edges;
//! excluded sources remain in the dependency set. Root-relative interpretation
//! of imported paths is the resolver's responsibility.
...
pub struct IncludeEdgeKey {
    ...
    /// Known build participation; None means a dynamic includeonly filter.
...
impl IncludeEdge {
    ...
    pub fn key(&self) -> IncludeEdgeKey {
        IncludeEdgeKey {
            ...
            build_participation: self.build_participation,
...
pub fn collect_include_edges(root: &SyntaxNode, base_dir: Option<&Path>) -> Vec<IncludeEdge> {
    let filter = tex_ls_parser::semantic::roles::include_only(root);
    root.descendants()
        .filter(|node| node.kind() == SyntaxKind::COMMAND)
        .filter_map(|node| {
            let mut edge = include_edge(&node, base_dir)?;
            if edge.kind == IncludeKind::Include {
                edge.build_participation =
                    nth_group_text(&node, 0).and_then(|name| filter.participates(&name));
            }
            Some(edge)
        })
        .collect()
...
fn include_edge(command: &SyntaxNode, base_dir: Option<&Path>) -> Option<IncludeEdge> {
    ...
    let literal = match include_target(command, kind, None) {
...
fn include_kind(name: &str) -> Option<IncludeKind> {
    use tex_ls_parser::semantic::roles::{FileRoleKind, SourceRole, file_role};
    let FileRoleKind::Source(role) = file_role(name)?.kind else {
        return None;
    };
    Some(match role {
        SourceRole::Input => IncludeKind::Input,
        SourceRole::Include => IncludeKind::Include,
        SourceRole::Import => IncludeKind::Import,
        SourceRole::SubImport => IncludeKind::SubImport,
        SourceRole::SubFile => IncludeKind::SubFile,
        SourceRole::SubFileInclude => IncludeKind::SubFileInclude,
        SourceRole::Glossary => IncludeKind::GlsEntries,
    })
}
...
pub fn collect_bib_resource_targets(root: &SyntaxNode, base_dir: Option<&Path>) -> Vec<BibTarget> {
        ...
        .filter(|node| node.kind() == SyntaxKind::COMMAND)
    {
        ...
        let Some(role) = file_role(&name).filter(|role| role.kind == FileRoleKind::Bibliography)
        else {
            continue;
        };
        match nth_group_text(&command, role.argument) {
            Some(value) if role.list => {
                ...
                targets.extend(
                    names
                        .into_iter()
                        .map(|name| resolve_bib(PathBuf::from(name), base_dir)),
                );
            }
            Some(value) => targets.push(resolve_bib(PathBuf::from(value.as_str()), base_dir)),
...
mod tests {
    ...
    fn a_leading_parent_segment_is_preserved() {
        ...
        // `tex-ls lint ../proj` run) — collapsing it would break the match.
    ...
    #[test]
    fn bare_input_without_braces_is_a_literal_edge() {
        let e = edges("\\input foo.tex\n", None);
        assert_eq!(e.len(), 1);
        assert_eq!(e[0].target, IncludeTarget::Path(PathBuf::from("foo.tex")));
    }
