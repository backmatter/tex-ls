...
///
/// Value equality backdates unchanged option facts across graph reconstruction.
...
///
/// The owned result contains no Salsa references and compares by value.
#[salsa::tracked(returns(ref), unsafe(non_salsa_values))]
pub(crate) fn resolved_package_options(
    ...
    ResolvedPackageOptions::build(
        crate::project::workspace_project(db, context)
            .members
            .iter()
            .filter(|member| member.kind.is_latex())
            .filter_map(|member| {
                file_package_option_facts(db, db.source_input(member.file, *context.identity(db)))
                    .clone()
            }),
    )
}
