...
pub(crate) enum Ir {
    ...
    ColumnZero(Rc<str>),
    /// An ordered, nonempty list of candidate layouts selected by an all-lines
    ...
    ///
    /// `Group`/`ConditionalGroupAllLines` cannot express this: a group is all-or-nothing
    ...
    /// `prefix` is opaque text the engine attaches no semantics to (the `.dtx`
...
mod tests {
    ...
    fn conditional_group_forces_by_its_flat_most_candidate_only() {
        ...
        let out = Ir::group(Ir::conditional_group_all_lines([
            Ir::text("flat"),
        ...
        let Ir::ConditionalGroupAllLines(cands) = inner.as_ref() else {
            unreachable!()
        };
    ...
    fn the_pass_is_idempotent() {
        let ir = Ir::group(Ir::concat([
            ...
            Ir::conditional_group_all_lines([Ir::text("flat"), Ir::group(Ir::hard_line())]),
        ]));
