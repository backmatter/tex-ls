...
//! wherever the swap is semantics-preserving) and check the formatter over them.
...
//!
//! This is a debug/test surface shared by `tex-ls debug format --checks
//! trivia`/`--checks trivia-strict` and the invariant tests; it carries no
//! stability promise.

use rowan::TextRange;

use tex_ls_parser::parser::{LexConfig, parse_with_flavor};
use tex_ls_parser::syntax::{SyntaxElement, SyntaxKind, SyntaxNode};

/// One semantics-preserving trivia perturbation of an input text.
#[derive(Debug, Clone)]
...
pub struct ConvergenceFailure {
    ...
    pub reason: String,
    /// Stable classification for machine reports.
...
pub fn trivia_perturbations(
    ...
    // Post-hoc safety net: a swap is semantics-preserving by construction at the
...
pub fn check_trivia_convergence(
    ...
    let violation =
        |variant: &PerturbedVariant, class, reason: String, once: String, twice: String| {
...
/// so a swap next to one is not semantics-preserving for the oracle's purposes;
...
/// only *semantics* safety — layout ownership is not consulted, since the
...
mod tests {
    ...
    use tex_ls_parser::parser::LatexFlavor;
    ...
    fn group_interior_gaps_are_eligible() {
        // Eligibility encodes semantics safety only, never layout ownership:
...
#[cfg(test)]
#[test]
fn convergence_reports_a_non_fixed_point_without_relying_on_a_formatter_bug() {
    let result = check_trivia_convergence(
        "hello world\n",
        tex_ls_parser::parser::LatexFlavor::Document,
        8,
        |text| {
    ...
    let Err(ConvergenceError::Violation(failure)) = result else {
        panic!("expected a violation")
    };
    assert_eq!(failure.class, "non-fixed-point");
    assert_ne!(failure.once, failure.twice);
}
