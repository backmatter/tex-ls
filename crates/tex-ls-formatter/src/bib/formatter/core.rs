...
//!   reference or a number) is **never** wrapped — that would change its semantics —
...
use tex_ls_parser::bib::ast;
use tex_ls_parser::bib::parse;
use tex_ls_parser::bib::semantic::{BibFieldDb, FieldCategory, builtin};
use tex_ls_parser::bib::syntax::{SyntaxKind, SyntaxNode};
use tex_ls_parser::directives::Verb;
...
/// shape make wrapping tex-ls-safe; otherwise fall back to the byte-exact
...
fn lower_value_piece(piece: &SyntaxNode, normalize: bool) -> Ir {
    match piece.kind() {
        ...
        // its semantics). It is a single token, so it carries no newline.
...
/// so the result is a well-formed group with identical semantics (`"` is an ordinary
