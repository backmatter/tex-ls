...
//! content, so semantics are preserved and the lowering stays a pure replay of the
//! reordered nodes. They read only the syntactic accessors ([`tex_ls_parser::bib::ast`]) plus
...
use tex_ls_parser::bib::semantic::{BibFieldDb, RequiredField};
...
mod tests {
    ...
    use tex_ls_parser::bib::semantic::builtin;
    ...
    fn lint_skip_target_is_not_reordered() {
        let keys = ordered_keys(
            "@comment{tex-ls-lint skip unknown-field}\n\
             @misc{zulu, unknown = {U}}\n\
             @misc{alpha, title = {A}}\n",
        );
