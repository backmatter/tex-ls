...
use tex_ls_formatter::bib::{format, format_with_style};
use tex_ls_formatter::formatter::{FormatStyle, LineEnding};
use tex_ls_parser::bib::semantic::Model;
use tex_ls_parser::bib::syntax::SyntaxKind;
use tex_ls_parser::bib::{ast, parse, reconstruct};
...
/// to reorder entries (and fields), so semantics are the *bag* of facts, not their
/// positions. Byte ranges are dropped — they shift when layout changes.
fn semantics(text: &str) -> (Vec<(String, String)>, Vec<String>, Vec<String>) {
...
/// a split inside a braced token) that the entry/`@string`-level `semantics()` oracle
...
fn field_values(text: &str) -> Vec<(String, String)> {
    fn signature(value: &tex_ls_parser::bib::syntax::SyntaxNode) -> String {
        value
...
fn assert_bib_format_invariants(input: &str) {
    ...
    // tex-ls preserved: same entries, @string defs, and @string uses.
    assert_eq!(
        semantics(input),
        semantics(&formatted),
        "formatting changed semantics for {input:?}"
    );
...
fn format_invariants_corpus() {
    let dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("../tex-ls-parser/tests/bib_corpus");
    let mut count = 0;
