...
use tex_ls_parser::parser::{LatexFlavor, LexConfig, parse, parse_with_flavor, reconstruct};
use tex_ls_parser::semantic::SignatureDb;
...
/// instead of panicking. The corpus sweep aggregates failures across widths. Inputs the parser rejects are out of scope for the
...
/// parsing under a project's [declarations](tex_ls_parser::declarations).
///
...
/// Run every sweep width and report all invariant failures for this file.
fn sweep_corpus_file(name: &str, text: &str, config: LexConfig) {
    ...
    for &width in SWEEP_WIDTHS {
        ...
        if let Err(message) = check_format_invariants(text, style, config) {
            failures.push(format!("width {width}: {message}"));
        }
    }
    assert!(
        failures.is_empty(),
        "invariant failures in {name}:\n{}",
        failures.join("\n")
    );
}
...
///
/// These examples prove only that the listed shapes stay invariant. Add new
/// shapes when the formatter gains coverage.
///
...
/// spellings are self-consistent fixed points. `tex-ls debug format --checks
...
fn format_invariants_corpus() {
    let dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("../tex-ls-parser/tests/corpus");
    let mut count = 0;
...
const PACKAGE_FIXTURES: &[(&str, &str)] = &[
    ...
    // Regression for a recognized head following unknown expl3 material.
...
const DTX_FIXTURES: &[&str] = &[
    ...
    // as a trailing `%<…>` comment — losing its docstrip semantics and re-lexing on
    ...
    // — a docstrip semantics change that also never reaches a fixed point. The
...
/// semantics must yield the same output (determinism). The single-line form is the
...
/// semantics change in horizontal mode. A whitespace
...
fn env_alias_resolves_with_no_external_signatures() {
    // `format` passes an empty external `SignatureDb`, which is the WASM
    // plugin's path. The alias must resolve from the document's own scan alone,
    // or the plugin would become a second sanctioned divergence from `tex-ls
...
// ---------------------------------------------------------------------------
// Comment directives (`% tex-ls-format …`, `% tex-ls …`)
// ---------------------------------------------------------------------------
...
fn format_skip_preserves_the_documented_construct() {
    let src = format!("% tex-ls-format skip: hand-aligned\n{HAND_ALIGNED}");
...
fn format_off_on_preserves_every_block_between_them() {
    let src = format!(
        "% tex-ls-format off\n{HAND_ALIGNED}\n{HAND_ALIGNED}% tex-ls-format on\n{HAND_ALIGNED}"
    );
    let out = format(&src).expect("formats");
    let (suppressed, formatted) = out
        .split_once("% tex-ls-format on\n")
        .expect("the closer survives");
    assert_eq!(
        suppressed,
        &src[..src.find("% tex-ls-format on").expect("has closer")],
...
fn format_skip_file_preserves_the_whole_document() {
    let src = format!("% tex-ls-format skip-file: generated\n{HAND_ALIGNED}");
...
/// The bare `% tex-ls` family suppresses layout too — it is the combined axis,
/// not a lint-only one.
#[test]
fn combined_family_suppresses_layout() {
    let src = format!("% tex-ls skip: leave it alone\n{HAND_ALIGNED}");
    assert_eq!(format(&src).expect("formats"), src);
    assert_format_invariants(&src);
}
...
fn unrecognized_and_lint_directives_do_not_suppress_layout() {
    for lead in [
        ...
        "% tex-ls-ignore-file: noisy",
        "% tex-ls-format nonsense",
        "% tex-ls",
        "% an ordinary comment",
    ] {
        let src = format!("{lead}\n{HAND_ALIGNED}");
...
fn unclosed_off_runs_to_end_of_file() {
    let src = format!("% tex-ls-format off\n{HAND_ALIGNED}\n{HAND_ALIGNED}");
    assert_eq!(format(&src).expect("formats"), src);
    assert_format_invariants(&src);
}
...
// --- declared environments (`tex-ls.toml`) ----------------------------------
...
#[test]
fn expl_fallback_and_tie_boundaries_converge() {
    let samples = [
        "\\ExplSyntaxOn\n\\cs_set_eq:NN\n\\l_query_tilde_tl\n~\n\\cs_set_eq:NN\n~\n\\c_tilde_str\n\\keys_set:nn{QueryFiles}{#1}\n",
        "\\ExplSyntaxOn{{{}{{}}{{}{}}}{}}{{}}{{{{}}}} {{{n{tl}}}l {{n} c {ool {\\b \\l_r{\\tl}}}{\\l_e}x {{2}} 2}}{{{}}}{}{{{{}}}{}}{{{{}}}}{{{}}}{{{{}}}}{{{}}}{{{{}}{}}}{{}}{}{{}}{}",
        "\\ExplSyntaxOn\\exp_after:wN \\foo \\tl_set:Nn \\l_a { x }\n\\group_begin:\n\\ExplSyntaxOff",
    ];
    for input in samples {
        for line_width in [30, 50, 80, 120] {
            let style = FormatStyle {
                line_width,
                ..FormatStyle::default()
            };
            check_format_invariants(input, style, LatexFlavor::Document.into()).unwrap();
        }
    }
}
