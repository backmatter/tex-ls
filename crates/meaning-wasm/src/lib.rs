//! meaning-wasm — the wasm-bindgen shim behind the docs playground
//! (<https://backmatter.github.io/meaning/playground.html>).
//!
//! A thin, non-published wrapper over [`meaning_formatter`] and
//! [`meaning_parser`]: [`format`] formats a document, [`check`] reports parse
...
use meaning_formatter::{FormatStyle, MathWrap, WrapMode};
use meaning_parser::parser::{LatexFlavor, LexConfig};
...
/// [`FileKind`](https://docs.rs/meaning) dispatch.
...
fn check_impl(input: &str, file_type: &str) -> Result<Vec<Diagnostic>, String> {
    ...
    let errors: Vec<(String, usize, usize)> = match file_type {
        FileType::Bib => meaning_parser::bib::parse(input)
            .errors
            .into_iter()
            .map(|e| (e.message, e.start, e.end))
            .collect(),
        _ => meaning_parser::parser::parse_with_flavor(input, lex_config(file_type))
            .errors
            .into_iter()
            .map(|e| (e.message, e.start, e.end))
            .collect(),
...
fn format_impl(
    ...
    match file_type {
        FileType::Bib => {
            meaning_formatter::bib::format_with_style(input, style).map_err(|e| e.to_string())
        }
        _ => meaning_formatter::formatter::format_with_style_flavored(
            input,
            style,
