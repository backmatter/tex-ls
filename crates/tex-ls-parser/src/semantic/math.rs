...
/// Presentation only: glyphs never change math classification or parsing.
pub fn command_glyph(name: &str) -> Option<char> {
    COMMAND_GLYPHS.get(name).copied().or(match name {
        "alpha" => Some('α'),
        "beta" => Some('β'),
        "gamma" => Some('γ'),
        "delta" => Some('δ'),
...
fn curated_command_info(name: &str) -> Option<MathAtomInfo> {
    ...
    let value = match name {
        ...
        // pairs for tex-ls's structural bracket accounting.
