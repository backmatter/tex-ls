...
use crate::semantic::label::{CitationRef, ColorDef, GlossaryDef, LabelDef, LabelRef};
use crate::semantic::pkgmeta;
use crate::semantic::roles::*;
...
pub fn build_with_declarations(
    ...
    model.include_only = include_only(root);
    resolve(&mut model);
    model
}

/// Extract semantic keys according to the shared role of each required argument.
fn collect_key_command(
    ...
    for index in 0..command
        .children()
        .filter(|node| node.kind() == SyntaxKind::GROUP)
        .count()
    {
        let Some(kind) = key_argument_role(semantic_name, index) else {
            continue;
        };
        recognized = true;
        let Some((inner_range, inner)) = nth_group_inner(command, index) else {
            continue;
        };
        if matches!(kind, ArgumentRole::Citation(CiteCommand::Nocite)) && inner.trim() == "*" {
            model.nocite_all = true;
            continue;
        }

        let split = match kind {
            ArgumentRole::Reference(kind) => kind.is_key_list(),
            ArgumentRole::Citation(_) => true,
            _ => false,
        };
        for (key, key_range) in key_spans(&inner, inner_range, split) {
            let key = SmolStr::new(key);
            match kind {
                ArgumentRole::BibliographyItem => model.bibitems.push(crate::semantic::BibItem {
                    name: key,
                    range: first_group_range(command),
                    key_range,
                }),
                ArgumentRole::Label => model.labels.push(LabelDef {
                    name: key,
                    range: first_group_range(command),
                    key_range,
                    referenced: false,
                }),
                ArgumentRole::Reference(command_kind) => model.refs.push(LabelRef {
                    name: key,
                ...
                ArgumentRole::GlossaryDefinition(kind) => model.glossary_defs.push(GlossaryDef {
                    key,
                    kind,
                    range: first_group_range(command),
                    key_range,
                }),
                ArgumentRole::ColorDefinition(kind) => model.color_defs.push(ColorDef {
                    name: key,
                    kind,
                    range: first_group_range(command),
                    key_range,
                }),
                ArgumentRole::GlossaryUse => {
                    model.glossary_uses.push(crate::semantic::GlossaryUse {
                        key,
                        range: command.text_range(),
                        key_range,
                    })
                }
                ArgumentRole::Citation(_) => model.citations.push(CitationRef {
                    name: key,
...
pub fn key_argument_command(name: &str) -> bool {
    key_argument_role(name, 0).is_some()
        || file_role(name).is_some()
        || matches!(name, "tag" | "hyperref")
...
mod tests {
    ...
    fn citation_command_table_is_closed_and_shape_specific() {
        ...
        for name in ["citestyle", "citetext", "citebox"] {
