...
fn walk(
    ...
    let mode = match node.kind() {
        ...
        SyntaxKind::NAME_GROUP | SyntaxKind::BARE_ARGUMENT => Mode::Unknown,
        _ => inherited,
    };
    ...
    // Every gap between child nodes consists solely of this node's tokens,
    // which all inherit the same mode. Cover the gap at once instead of creating
    // a red cursor for every word, space and newline in a prose paragraph.
    let mut cursor = node.text_range().start();
    for child in node.children() {
        push_range(
            ranges,
            TextRange::new(cursor, child.text_range().start()),
            mode,
        );
        ...
        cursor = child.text_range().end();
    }
    push_range(
        ranges,
        TextRange::new(cursor, node.text_range().end()),
        mode,
    );
}
...
mod tests {
        ...
        for element in node.children_with_tokens() {
            match element {
                ...
                SyntaxElement::Node(child) => {
                    ...
                    walk_tokens_reference(&child, child_mode, signatures, ranges);
                }
            }
        }
    }
    ...
    #[test]
    fn mode_gaps_match_token_walk_over_the_corpus() {
        let dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/corpus");
        let mut paths = vec![dir];
        let mut files = 0;
        while let Some(path) = paths.pop() {
            if path.is_dir() {
                paths.extend(std::fs::read_dir(path).unwrap().map(|p| p.unwrap().path()));
                continue;
            }
            if !path
                .extension()
                .is_some_and(|e| e == "tex" || e == "sty" || e == "dtx" || e == "cls")
            {
                continue;
            }
            ...
            walk_tokens_reference(&root, Mode::Text, &signatures, &mut expected);
            assert_eq!(
                ModeIndex::build(&root).ranges,
                expected,
                "{}",
                path.display()
            );
        ...
        assert!(files >= 20, "{files} corpus files");
    }
