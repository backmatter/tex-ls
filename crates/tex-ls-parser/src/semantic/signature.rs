...
//! verbatim-ness, math-ness). Semantics are assigned here rather than in the parser.
//!
...
pub struct CommandSig {
    ...
    /// undecidable without semantics, so those fall back to the formatter's
...
pub struct EnvironmentSig {
    ...
    /// package discards. `bash scripts/check_typeset_stability.sh` carries the proving case
    ...
    /// semantics are unrelated to `\label`, so only curated package facts may set it.
...
pub(crate) const fn command(generated: GeneratedCommand) -> CommandSig {
    CommandSig {
        ...
        // Citation grammar is curated semantics, never a mechanical CWL fact.
        citation: None,
