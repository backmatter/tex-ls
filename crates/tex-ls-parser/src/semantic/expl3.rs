        ...
        match &elements[i] {
            ...
            SyntaxElement::Node(n) if n.kind() == SyntaxKind::COMMAND => {
                match expl3_unit(elements, i) {
                    Some(unit) => {
                        let end = unit.last;
                        let full = if node_is_expl_toggle(n) {
                            end
                        } else {
                            absorb_trailing_junk(elements, end)
                        };
                        if full > end {
                    ...
                    None => i = fallback_statement(elements, i, &mut flags),
                }
            }
            _ => i = fallback_statement(elements, i, &mut flags),
        }
    }
    StatementMap { flags }
}
...
fn fallback_statement(
    ...
    while next < elements.len() {
        ...
        if is_collapsible_trivia(element.kind()) {
            newlines += usize::from(element.kind() == SyntaxKind::NEWLINE);
            if newlines >= 2 {
                break;
            }
            next += 1;
        ...
        if next > start && matches!(element, SyntaxElement::Node(node) if is_recognized_head(node))
        {
            break;
        }
        if next > start && matches!(element.kind(), SyntaxKind::GUARD | SyntaxKind::DOC_MARGIN) {
            break;
        }
        if element.kind() == SyntaxKind::COMMENT && newlines > 0 {
            break;
        }
        newlines = 0;
        last = next;
    ...
    for flag in &mut flags[start..=last] {
        flag.insert(ElementFlags::FALLBACK);
    }
    next
}
    ...
    while j < elements.len() {
        match &elements[j] {
            SyntaxElement::Token(t) if is_collapsible_trivia(t.kind()) => {
                newlines += usize::from(t.kind() == SyntaxKind::NEWLINE);
                if newlines >= 2 {
                    break;
                }
                j += 1;
        ...
        loop {
            ...
            match &el {
                ...
                SyntaxElement::Token(t) if t.kind() == SyntaxKind::TILDE => {
                    // A tie separates trivia runs. Newlines on its two sides
                    // are not a blank line and cannot truncate an argument list.
                    gap_newlines = 0;
                }
                SyntaxElement::Token(t)
                    if matches!(t.kind(), SyntaxKind::GUARD | SyntaxKind::DOC_MARGIN) =>
...
mod segmentation_tests {
    ...
    #[test]
    fn ties_separate_newline_runs_in_argument_lists() {
        let multiline = "\\ExplSyntaxOn\n\\cs_set_eq:NN\n\\l_query_tilde_tl\n~\n\\cs_set_eq:NN\n~\n\\c_tilde_str\n\\keys_set:nn{QueryFiles}{#1}\n";
        let compact = "\\ExplSyntaxOn\n\\cs_set_eq:NN \\l_query_tilde_tl ~\\cs_set_eq:NN ~\\c_tilde_str\n\\keys_set:nn { QueryFiles } {#1}\n";
        ...
        assert_eq!(normalize(multiline), normalize(compact));
        let shape = |text: &str| {
            parse(text)
                .syntax()
                .preorder()
                .map(|event| match event {
                    rowan::WalkEvent::Enter(node) => (true, node.kind()),
                    rowan::WalkEvent::Leave(node) => (false, node.kind()),
                })
                .collect::<Vec<_>>()
        };
        assert_eq!(shape(multiline), shape(compact));
    }
    ...
    #[test]
    fn comment_in_a_consumed_slot_ends_the_fallback_statement() {
        let got = statements(
    ...
    #[test]
    fn unknown_head_stops_before_the_next_recognized_call() {
        let got = statements(
    ...
    #[test]
    fn a_multi_line_group_node_does_not_end_a_fallback_statement() {
        let src = "\\ExplSyntaxOn\n\
        ...
        assert_eq!(
            ...
            vec![
                "\\ExplSyntaxOn",
                "\\int_do_until:w { \\l_tmpa_int } > {#2} { \\lipsum_add:V { \\l_tmpa_int } \\int_incr:N \\l_tmpa_int }",
        ...
        assert!(
            ...
            "the next recognized call ends the fallback statement"
        );
        ...
        assert!(
            !map.is_fallback(head),
            "the recognized head starts a structural statement"
        );
    }
