...
pub use label::{
    BibItem, CitationRef, ColorDef, ColorDefKind, GlossaryDef, GlossaryDefKind, LabelDef, LabelId,
    LabelRef, RefCommand, RefId,
};
pub use math::{
...
pub struct SemanticModel {
    ...
    /// of the bibliography into the document. Missing explicit keys remain errors.
...
impl SemanticModel {
    ...
    /// Explicit build participation filter; excluded sources remain dependencies.
    pub fn include_only(&self) -> &roles::IncludeOnly {
        &self.include_only
    }

    pub fn glossary_uses(&self) -> &[GlossaryUse] {
        &self.glossary_uses
    }
...
/// A literal glossary/acronym key use; quoted descriptions are never identifiers.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct GlossaryUse {
    pub key: SmolStr,
    pub range: rowan::TextRange,
    pub key_range: rowan::TextRange,
}
