...
//!
//! Accessors are positional and do not assign semantics or arity to commands.
//!
...
pub fn nth_group_text(command: &SyntaxNode, n: usize) -> Option<SmolStr> {
    nth_group_inner(command, n).map(|(_, text)| text)
}
...
pub fn nth_group_inner(command: &SyntaxNode, n: usize) -> Option<(TextRange, SmolStr)> {
    let argument = command
        .children()
        .filter(|node| matches!(node.kind(), SyntaxKind::GROUP | SyntaxKind::BARE_ARGUMENT))
        .nth(n)?;
    if argument.kind() == SyntaxKind::BARE_ARGUMENT {
        // An adjacent macro or group may extend a TeX filename dynamically.
        if let Some(next) = command.next_sibling_or_token()
            && next.text_range().start() == command.text_range().end()
            && !matches!(
                next.kind(),
        ...
        return Some((
            argument.text_range(),
            SmolStr::new(argument.text().to_string()),
        ));
    }
    Group::cast(argument)?.inner()
}
