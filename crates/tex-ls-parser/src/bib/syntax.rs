...
//! binding for tex-ls's BibTeX/BibLaTeX surface CST.
//!
//! BibTeX is a distinct grammar from LaTeX, so it carries its own kind set and
//! [`rowan::Language`] marker rather than reusing [`crate::syntax`]. The structure
...
/// The rowan language marker for tex-ls's BibTeX CST.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub enum BibLang {}
