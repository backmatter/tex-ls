//! Math productions and script binding.
use super::*;
...
impl Parser<'_> {
    ...
    pub(super) fn math_element(&mut self) {
        let before = self.pos;
        self.math_body_element();
        assert!(
            self.pos > before || self.pos == self.tokens.len(),
            "math element made no progress"
        );
    }

    pub(super) fn math_body_element(&mut self) {
        match self.kind() {
    ...
    pub(super) fn math_scripted(&mut self) {
    ...
    pub(super) fn math_word_fragment(&mut self, idx: usize, start: usize, end: usize) {
    ...
    pub(super) fn math_scripts(&mut self, checkpoint: usize) {
        if !self.at_script() {
    ...
    /// break ends the math).
    pub(super) fn at_script(&self) -> bool {
    ...
    pub(super) fn math_atom(&mut self) {
    ...
    pub(super) fn math_script_arg(&mut self) -> Option<(usize, usize, usize)> {
        if self.at_paragraph_break() {
    ...
    pub(super) fn math_group(&mut self) {
    ...
    pub(super) fn left_right(&mut self) {
    ...
    pub(super) fn math_delim(&mut self, after: &str) {
        self.skip_trivia();
        let missing = match self.kind() {
            None | Some(SyntaxKind::R_BRACE | SyntaxKind::DOLLAR) => true,
            Some(SyntaxKind::CONTROL_SYMBOL) => matches!(self.text(), "\\]" | "\\)"),
    ...
    pub(super) fn stray_right(&mut self) {
