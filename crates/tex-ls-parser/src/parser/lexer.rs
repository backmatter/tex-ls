...
impl ParseCtx {
    ...
    /// A command whose spelling cannot change this parse's lexer state.
    pub(crate) fn inert_control_word(&self, text: &str) -> bool {
        ...
        !name.is_empty()
            ...
            && !matches!(name, "verb" | "makeatletter" | "makeatother")
            && !self.commands.contains_key(name)
            && !self.begin_aliases.contains_key(name)
            && !self.end_aliases.contains_key(name)
            && builtin()
                .command(name)
                .is_none_or(|sig| !sig.verbatim && sig.args.is_empty())
    }
...
impl<'a> Lexer<'a> {
    ...
    fn run(mut self) -> Vec<Token> {
        while self.pos < self.input.len() {
            ...
            // These bytes cannot start a capture or a mode transition. Keep the
            // ordinary token routine authoritative for boundaries and state;
            // avoid probing every raw-capture family for each prose token.
            if !self.config.dtx
                && self.pending.is_none()
                && self.short_verbs.is_empty()
                && matches!(self.input.as_bytes()[self.pos], b'a'..=b'z' | b'A'..=b'Z' | b' ' | b'\t' | b'\n' | b'\r')
            {
                self.lex_token(None);
                continue;
            }
