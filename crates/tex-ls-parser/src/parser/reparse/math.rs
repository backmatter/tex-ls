...
pub(super) fn reparse_math(
    ...
    // A fragment starts with a fresh recursion budget. CST depth conservatively
    // bounds recursive grammar entries, including the unchanged outer context.
    // Keep a margin for nodes wrapped after their children have been parsed.
    let outer_depth = target.ancestors().count();
    if replacement
        .descendants()
        .any(|node| outer_depth + node.ancestors().count() >= crate::MAX_PARSE_DEPTH / 2)
        || parsed
            .errors
            .iter()
...
/// tex-ls. Empty edits inspect both boundary candidates because adding a letter
