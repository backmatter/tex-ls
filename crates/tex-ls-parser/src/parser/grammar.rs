...
/// Closed grammar policies that a generic control-word splice must not change.
pub(crate) fn is_special_reparse_command(text: &str) -> bool {
    text == "\\input"
        || facts::is_big_delimiter_command(text)
        || facts::is_definition_body_command(text)
        || facts::is_def_prefix_command(text)
}
...
struct Parser<'t> {
    ...
    /// The `last_alias_closer` treatment, generalized: each shape gate succeeds only at one closer token shape, so
    ...
    /// `mod tests`, never a scan budget. `Cell` because the gates take `&self`, and
...
impl<'t> Parser<'t> {
    ...
    fn parse_block(&mut self, block: Block) {
        loop {
            ...
            loop {
                ...
                // A plain word is one text element. A space or single newline
                // immediately before it cannot separate paragraphs or bind a
                // comment. The general path below emits exactly the same token
                // and counters, but also probes command and comment gates.
                // Statement bodies still need their semicolon bookkeeping.
                if !self.in_statement_body {
                    let kind = self.kind();
                    let word = kind == Some(SyntaxKind::WORD);
                    if word
                        || (matches!(kind, Some(SyntaxKind::WHITESPACE | SyntaxKind::NEWLINE))
                            && self.nth_kind(1) == Some(SyntaxKind::WORD))
                    {
                        self.bump();
                        if word {
                            nontrivia_count += 1;
                            lone_block_env = false;
                        }
                        continue;
                    }
                }
    ...
    fn element(&mut self) {
        ...
        assert!(
            self.pos > before || self.pos == self.tokens.len(),
            "text element made no progress"
        );
    }

    /// Once the budget is exhausted, retain the remaining bytes as flat error
    /// content. Unwinding keeps both the CST and subsequent feature walks bounded.
    fn enter_recursion(&mut self) -> bool {
        if self.recursion_depth < crate::MAX_PARSE_DEPTH {
            self.recursion_depth += 1;
            return true;
        }
        self.nesting_exhausted = true;
        self.error(crate::NESTING_LIMIT_ERROR);
    ...
    fn command(&mut self) {
        ...
        if bare_input {
            ...
            if !scan.saw_blank_line
                && matches!(
                    scan.next_kind,
                    Some(SyntaxKind::WORD | SyntaxKind::UNDERSCORE)
                )
            {
                self.skip_trivia();
                self.open(SyntaxKind::BARE_ARGUMENT);
                while matches!(self.kind(), Some(SyntaxKind::WORD | SyntaxKind::UNDERSCORE)) {
                    self.bump();
                }
                self.close();
    ...
    ///
    /// Runs on the shared batch driver as [`MacrocodeBracketGate`], which carries the chunk frame as its bound and
    ...
    ///   it. Does not consume.
    ///
    /// Runs on the shared batch driver as [`MathBracketGate`], where the transparent `$…$` region, the flavor
    ...
    ///
    /// Runs on the shared batch driver as [`TextBracketGate`]. The claim countdown above *is* the driver's
    ...
    ///
    /// Runs on the shared batch driver as [`DollarGate`] — for the uniformity, not for speed: the gate is
    ...
    ///
    /// Runs on the shared batch driver as [`DelimMathGate`], which carries the C0 bound — the last `\]`/`\)` in
    ...
    /// Does not consume.
    ///
    /// Runs on the shared batch driver as [`LeftRightGate`], which is where those anchors and the deliberate
