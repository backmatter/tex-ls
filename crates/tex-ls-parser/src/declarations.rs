...
use crate::semantic::roles::{cite_command, ref_command};
...
pub struct Declarations {
    /// Completion-only key/value schemas, keyed by command or environment name.
...
impl Declarations {
    ...
    pub fn is_empty(&self) -> bool {
        self.commands.is_empty() && self.environments.is_empty() && self.options.is_empty()
    }
...
impl ResolvedDeclarations {
    ...
    pub fn is_empty(&self) -> bool {
        self.db == SignatureDb::default() && self.commands.is_empty() && self.options.is_empty()
    }
...
///
/// Deliberately shape-only, with no check that the name is one tex-ls knows: a
...
/// bare key so the result can be pasted back into `tex-ls.toml`.
///
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::UnknownLikeTarget { target } => write!(
                f,
                "unknown environment `{target}`; `like` must name an environment tex-ls \
                 knows about"
            ),
            ...
            Self::UndeclaredTarget => write!(
                f,
                "declares delimiters for an environment tex-ls does not know; add `like` \
            ...
            Self::SpellingIsALiteralDelimiter { name, environment } => write!(
                ...
                "`{name}` is the delimiter itself, not a command standing in for one — and \
                 tex-ls already pairs a declared spelling with the written-out \
            ...
            Self::SpellingIsABuiltinCommand { name } => write!(
                f,
                "`{name}` is already a LaTeX command tex-ls knows; a delimiter spelling must \
