...
//! curated corpus file that *tex-ls itself parses cleanly* (no syntax errors), the
...
//! (tex-ls's generic CST and texlab's semantic CST are not meant to match).
//!
...
/// Corpus files tex-ls legitimately parses cleanly but texlab cannot, each with
...
const TEXLAB_KNOWN_DIVERGENT: &[&str] = &[
    ...
    // opaque VERB spans for TexLs (issue #57), unmatched braces for texlab.
    "doc_shortverb.tex",
    ...
    // tex-ls, but texlab reads their `$`/`\[`/`\]` as math delimiters.
    "char_constant.tex",
    "alignment_char_constant.tex",
    // A `$` in `\def` parameter text delimits an argument; it does not open math.
    // tex-ls models that TeX syntax, while texlab lets the apparent math span
    ...
    // stray `}` inside are data for TexLs, an unclosed env and an unmatched
    // brace for texlab, whose environment DB does not carry the signature.
    "verbatim_ltxexample.tex",
];

#[test]
fn texlab_accepts_tex_ls_clean_corpus() {
    ...
    for entry in fs::read_dir(&dir).expect("read corpus dir") {
        ...
        // Only assert on inputs tex-ls accepts cleanly (mirrors air_parser_harness).
        ...
        assert!(
            ...
            "texlab reported a parse error for TexLs-clean corpus file `{name}`"
        );
        checked += 1;
    }
