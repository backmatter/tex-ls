...
//! the losslessness invariant. Regenerate snapshots with `INSTA_UPDATE=always cargo test --workspace`.
...
fn left_right_pairs_inside_macro_code() {
    ...
    // count regardless of macro semantics, so the shape gate may not skip `\left`/
...
fn a_nested_left_shields_the_outer_pair_from_a_paragraph_break() {
    ...
    // preserved by the batch migration; the shape is only reachable inside a math *environment*, since the
...
fn a_let_source_operand_is_not_an_opener() {
    // `\let` binds *two* names: the definee and the semantics it is given. Both are
...
// --- declared environments (`tex-ls.toml`; AGENTS.md decision #12) ----------
...
fn a_declared_verbatim_environment_captures_its_body() {
    // The parked `codeexample` knob: an environment tex-ls cannot name, whose
...
fn a_declared_statement_environment_wraps_like_the_entry_it_copies() {
    ...
    let count = |parsed: &tex_ls_parser::parser::Parse| {
        parsed
            .syntax()
            .descendants()
            .filter(|n| n.kind() == SyntaxKind::STATEMENT)
...
//
// Arity-directed expl3 attachment: in-region colon-suffixed heads attach by
