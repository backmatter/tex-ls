...
//! `parse_compat.rs`).
//!
//! tex-ls's CST is deliberately *generic* (`\section`→`COMMAND`, environments by
...
///
/// tex-ls and texlab pin *different* rowan releases, so `rowan::Language`,
...
/// `rowan::Language` silently welds tex-ls's rowan version to texlab's.
///
...
fn project_elem<P: Projector>(elem: &Elem<P>) -> Vec<Atom> {
    match elem {
        ...
        // A bare command-name token with no enclosing command node. tex-ls emits
...
impl_tree!(
    TexLs,
    tex_ls_parser::syntax::SyntaxKind,
    tex_ls_parser::syntax::SyntaxNode,
    tex_ls_parser::syntax::SyntaxToken
);

impl Projector for TexLs {
    fn cat(kind: tex_ls_parser::syntax::SyntaxKind) -> Cat {
    ...
    fn is_begin_or_end(kind: tex_ls_parser::syntax::SyntaxKind) -> bool {
        use tex_ls_parser::syntax::SyntaxKind::{BEGIN, END};
        matches!(kind, BEGIN | END)
    }

    fn is_command_token(kind: tex_ls_parser::syntax::SyntaxKind) -> bool {
        use tex_ls_parser::syntax::SyntaxKind::{CONTROL_SYMBOL, CONTROL_WORD};
        matches!(kind, CONTROL_WORD | CONTROL_SYMBOL)
    }

    fn is_word_token(kind: tex_ls_parser::syntax::SyntaxKind) -> bool {
        kind == tex_ls_parser::syntax::SyntaxKind::WORD
    }

    fn is_verbatim_token(kind: tex_ls_parser::syntax::SyntaxKind) -> bool {
        use tex_ls_parser::syntax::SyntaxKind::{VERB, VERBATIM_BODY};
        matches!(kind, VERB | VERBATIM_BODY)
    }
}

/// Project tex-ls's CST for `text`.
pub fn project_tex_ls(text: &str) -> Vec<Atom> {
    let parsed = tex_ls_parser::parser::parse(text);
    project::<TexLs>(&parsed.syntax())
}
...
impl Projector for Texlab {
    fn cat(kind: texlab_syntax::latex::SyntaxKind) -> Cat {
        ...
        match kind {
            ...
            // dropping texlab's semantic role so it lines up with tex-ls's
            // generic `COMMAND`.
            GENERIC_COMMAND
            | PART
            | CHAPTER
            ...
            // ROOT, PREAMBLE, TEXT, KEY/VALUE wrappers, PAREN_GROUP (tex-ls treats
