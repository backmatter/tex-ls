...
//! `bib_parse_compat.rs`).
//!
//! Like the LaTeX `parse_skeleton`, this projects both tex-ls's and texlab's bib CSTs
...
//!    (`ACCENT`), commands (`COMMAND`), and `~` (`NBSP`) inside braces/quotes; tex-ls
...
/// Project tex-ls's bib CST for `text` onto the skeleton forest.
pub fn project_tex_ls(text: &str) -> Vec<BibAtom> {
...
/// The entry-type word of a tex-ls `ENTRY` node, lowercased.
fn tex_ls_entry_type(entry: &bib::syntax::SyntaxNode) -> String {
...
/// The `Field(name)` atoms of a tex-ls `ENTRY` node.
fn tex_ls_fields(entry: &bib::syntax::SyntaxNode) -> Vec<BibAtom> {
...
/// tex-ls does for a tex-ls-clean file: a one-way soundness floor that catches
/// tex-ls hallucinating structure texlab does not see, without depending on an error
