...
use tex_ls_parser::declarations::Declarations;
...
fn roundtrip_declared_corpus_file() {
    ...
    let environments = |root: &tex_ls_parser::syntax::SyntaxNode| {
        root.descendants()
            .filter(|n| n.kind() == tex_ls_parser::syntax::SyntaxKind::ENVIRONMENT)
            .count()
    };
