...
//!
//! Run it with `bash scripts/check_reparse_baselines.sh`, which builds release, runs this, and
//! diffs the tallies against `tests/reparse_baselines/`. Directly:
//!
//! ```sh
//! cargo test --release -p tex-ls-parser --test reparse_corpus_sweep -- --ignored --nocapture
//! ```
//!
...
use rayon::prelude::*;
use tex_ls_parser::parser::{Edit, LatexFlavor, LexConfig};
...
struct Cell {
    ...
    /// splice rate semantics what it says.
    files: usize,
}
...
#[test]
#[ignore = "sweeps the pinned gate corpora; run via `bash scripts/check_reparse_baselines.sh`"]
...
/// Which corpora to sweep. `TEX_LS_REPARSE_SWEEP_CORPORA` restricts the set so
...
fn selected_corpora() -> Vec<String> {
    match std::env::var("TEX_LS_REPARSE_SWEEP_CORPORA") {
...
fn walk(root: &Path, dir: &Path, out: &mut Vec<(String, PathBuf, String)>) {
    ...
    for entry in entries.flatten() {
        ...
        if !matches!(
            ext,
            Some("tex" | "sty" | "cls" | "dtx" | "ins" | "def" | "lco")
        ) {
            continue;
        }
