...
use tex_ls_parser::parser::{LatexFlavor, LexConfig, parse_with_declarations, parse_with_flavor};
...
fn property_config() -> ProptestConfig {
    let cases = std::env::var("TEX_LS_PROPERTY_CASES")
        .map(|value| {
            value
                .parse()
                .expect("TEX_LS_PROPERTY_CASES must be a positive integer")
        })
        .unwrap_or(DEFAULT_CASES);
    assert!(cases > 0, "TEX_LS_PROPERTY_CASES must be positive");
    ProptestConfig {
        cases,
