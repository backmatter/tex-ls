...
//!
//! The bib analog of `parse_compat.rs`. It measures how *structurally close* tex-ls's
//! bib CST is to texlab's over a corpus, after projecting both onto one coarse skeleton
//! (see `bib_skeleton`). A divergence from texlab is **never a bug by itself**: tex-ls
...
//! values; tex-ls keeps them generic). AGENTS.md: measure against, never match.
//!
...
//! ```
//!
//! The result is written to `target/BIB_PARSE_COMPAT.md`
...
use bib_skeleton::{dice, lcs_len, project_tex_ls, project_texlab, render_lines};
...
#[test]
#[ignore = "soft parse-concordance gauge; run via `cargo test -p tex-ls-parser --test bib_parse_compat -- --ignored --nocapture`"]
fn bib_parse_compat_report() {
    ...
    for (key, path) in &corpus {
        ...
        let tex_ls_lines = render_lines(&project_tex_ls(&text));
    ...
    let out_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../target/BIB_PARSE_COMPAT.md");
...
fn render_report(
    ...
    s.push_str(
        "_Generated by `cargo test -p tex-ls-parser --test bib_parse_compat -- --ignored --nocapture` (`tests/bib_parse_compat.rs`). Do not edit by hand._\n\n",
    );
    s.push_str(
        "This is a **soft gauge, not a quality gate.** It projects tex-ls's bib CST and \
