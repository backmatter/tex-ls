...
fn iterations(base: usize) -> usize {
    let scale: usize = std::env::var("TEX_LS_REPARSE_FUZZ_ITERS")
        .ok()
...
/// are two orders of magnitude larger and live behind `bash scripts/check_reparse_baselines.sh`.
#[test]
...
/// protected-body tier exists for.
#[test]
...
/// silently empty this harness by excluding the edits it is meant to exercise.
#[test]
...
fn the_harness_rejects_a_perturbed_error_vector() {
    ...
    let result = Reparsed {
        green: parse.green,
        errors: vec![tex_ls_parser::parser::SyntaxError {
...
/// parse's vector only if emission order is positional. tex-ls has one error
...
#[test]
fn standalone_control_words_reparse_without_changing_grammar_decisions() {
    let declared = ResolvedDeclarations::default();
    for prefix in [
        "",
        "\\begin{document}prose\\end{document}\n",
        "\\documentclass{article}\n\\begin{document}prose\\end{document}\n",
        "\\newcommand{\\foo}{text}\n\n",
    ] {
        let text = format!("{prefix}\\sec\n");
        ...
        for name in [
            ...
            let result = reparse(&base, &edit, &next);
            if name == "pro" {
                assert_eq!(result.as_ref().map(|r| r.tier), Some(ReparseTier::Token));
            }
            if let Some(result) = result {
                let (full, _) = parse_with_declarations_resolved(&next, config(), &declared);
                assert_eq!(result.green, full.green, "{text:?} {name}");
...
#[test]
fn command_splice_checks_literal_names_from_parser_and_lexer() {
    // Keep rare reserved spellings represented when parser/lexer policies grow.
    // This supplements the token text-read survey and dynamic-family cases above.
    let sources = [
        include_str!("../src/parser/grammar.rs"),
        include_str!("../src/parser/grammar/facts.rs"),
        include_str!("../src/parser/conditional.rs"),
        include_str!("../src/parser/lexer.rs"),
    ];
    ...
    let declared = ResolvedDeclarations::default();
    for name in names {
        for (old, new) in [("sec", name.as_str()), (name.as_str(), "pro")] {
            let text = format!("\\begin{{document}}prose\\end{{document}}\n\\{old}\n");
            let (parsed, ctx) = parse_with_declarations_resolved(&text, config(), &declared);
            let base = ReparseBase::from_parts(
                &text,
                &parsed.green,
            ...
            let at = text.len() - old.len() - 1;
            let edit = Edit {
                range: at..at + old.len(),
                insert: new.into(),
            };
            let next = edit.apply(&text);
            if let Some(result) = reparse(&base, &edit, &next) {
                let (full, _) = parse_with_declarations_resolved(&next, config(), &declared);
                assert_eq!(result.green, full.green, "{old} -> {new}");
                assert_eq!(result.errors, full.errors);
            }
        }
    }
...
#[test]
fn prose_command_edits_inside_environments_preserve_the_full_tree() {
    let declared = ResolvedDeclarations::default();
    for text in [
        "Prose \\sec\n",
        "\\begin{document}\nProse \\sec\n\\end{document}\n",
        ...
        let at = text.find("sec").unwrap();
        let edit = Edit {
            range: at..at + 3,
            insert: "pro".into(),
        };
        let next = edit.apply(text);
        let result = reparse(&base, &edit, &next).expect("ordinary command token splice");
        assert_eq!(result.tier, ReparseTier::Token);
        let (full, _) = parse_with_declarations_resolved(&next, config(), &declared);
    ...
    for text in [
        ...
        let next = edit.apply(text);
        assert!(
            reparse(&base, &edit, &next).is_none(),
            "must decline {text:?}"
        );
    }
}
...
#[test]
fn incomplete_math_and_recovery_prefixes_remain_lossless() {
    use tex_ls_parser::syntax::SyntaxNode;
    let examples = [
        ("tex", "$\\left(𝕏_{a^2}\\right)$\r\n"),
        ("tex", "\\[\\frac{a}{b} + x^{y_1}\\]\r\n"),
        (
            "tex",
            "\\begin{align}x &= y_1\\\\z &= \\sqrt{w}\\end{align}",
        ),
        (
            "tex",
            "\\newcommand{\\open}{\\begin{array}}\\ifnum1>0 {x}\\fi",
        ),
        ("tex", "\\catcode`\\$=12 $text {unclosed\\end{unknown}"),
        (
            "dtx",
            "%    \\begin{macrocode}\r\n\\def\\foo#1{$#1$}\r\n%    \\end{macrocode}\r\n",
        ),
    ...
    for (kind, source) in examples {
            ...
            .chain(std::iter::once(source.len()))
        {
            ...
            let (parsed, _) = parse_with_declarations_resolved(
                prefix,
                tex_ls_parser::parser::LexConfig {
                    dtx: kind == "dtx",
                    ..config()
                },
                &ResolvedDeclarations::default(),
            ...
            assert_eq!(
                SyntaxNode::new_root(parsed.green).to_string(),
                prefix,
                "{kind}: {prefix:?}"
            );
        }
