...
//!
//! A line-leading `%` becomes a `DOC_MARGIN` token in documentation. The
//! `macrocode` environment contains code, and `%<…>` docstrip guards are trivia
//! in either layer. Every case also checks losslessness.

use tex_ls_parser::parser::{LatexFlavor, LexConfig, parse_with_flavor};
use tex_ls_parser::semantic::{DocKind, doc_associations};
...
fn doc_associations_pair_prose_with_nested_code() {
    // Semantic prose/code association in a realistic `.dtx`
...
fn implicit_expl_module_guard_macrocode_body_is_expl3() {
    ...
    // control word rather than splitting at `_`/`:`.
    let (root, errors) = parse_dtx_with_errors(
        "%<@@=talk>\n\
         %    \\begin{macrocode}\n\
