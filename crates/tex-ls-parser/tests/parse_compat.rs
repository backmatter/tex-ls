...
//!
//! This harness measures how *structurally close* tex-ls's CST is to texlab's over a
...
//! same principle: a divergence from texlab is **never a bug by itself**. tex-ls is
...
//!
//! ```sh
//! cargo test -p tex-ls-parser --test parse_compat -- --ignored --nocapture
//! # or
//! cargo test --test parse_compat -- --ignored --nocapture
//! ```
//!
//! Methodology — for each corpus file we project tex-ls's and texlab's trees onto the
//! shared skeleton, render each to indented S-expression lines, and score the Dice
//! coefficient over those lines. Files tex-ls cannot parse cleanly are skipped (we
//! never measure against our own parse errors). The result is written to
//! `target/PARSE_COMPAT.md` and is a triage queue, not a
//! pass/fail signal.
...
use parse_skeleton::{
    TexLs, dice, lcs_len, project, project_texlab, render_lines, texlab_has_error,
};
...
enum Outcome {
    ...
    /// tex-ls could not parse the input cleanly (syntax errors).
    SkippedTexLs,
}
...
#[test]
#[ignore = "soft parse-concordance gauge; run via `cargo test -p tex-ls-parser --test parse_compat -- --ignored --nocapture`"]
fn parse_compat_report() {
    ...
    // Optional triage aid: `PARSE_COMPAT_DUMP=1 cargo test -p tex-ls-parser --test parse_compat -- --ignored --nocapture` writes a per-file
    // skeleton diff for every divergent/skipped file to `target/parse_compat_diffs.txt`,
    // so a classifier can read the actual tex-ls-vs-texlab shapes (the report only
    ...
    let mut total_lines = 0usize; // sum of (tex_ls_lines + texlab_lines) over compared files
    ...
    for (key, path) in &corpus {
        ...
        if !parsed.errors.is_empty() {
            if dump_enabled {
                let tex_ls_lines = render_lines(&project::<TexLs>(&parsed.syntax()));
                ...
                let errs: String = parsed
                    ...
                    .map(|e| format!("    {e:?}\n"))
                    .collect();
                dump.push_str(&dump_section(key, &errs, &tex_ls_lines, &texlab_lines));
            }
            reports.push(FileReport {
                key: key.clone(),
                outcome: Outcome::SkippedTexLs,
                texlab_error: false,
            });
            continue;
        }
        ...
        total_lcs2 += 2 * lcs_len(&tex_ls_lines, &texlab_lines);
        total_lines += tex_ls_lines.len() + texlab_lines.len();

        let outcome = if tex_ls_lines == texlab_lines {
            Outcome::Concordant
        } else {
            Outcome::Divergent {
                similarity: dice(&tex_ls_lines, &texlab_lines),
            }
        };
        if dump_enabled && !matches!(outcome, Outcome::Concordant) {
            dump.push_str(&dump_section(key, "", &tex_ls_lines, &texlab_lines));
        }
        reports.push(FileReport {
            key: key.clone(),
            outcome,
    ...
    let out_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../target/PARSE_COMPAT.md");
    fs::create_dir_all(out_path.parent().unwrap()).expect("create report directory");
...
/// One file's section in the triage dump: optional tex-ls parse errors followed
/// by an LCS-aligned diff of the two skeletons.
fn dump_section(key: &str, errors: &str, tex_ls: &[String], texlab: &[String]) -> String {
    let mut s = format!("===== {key} =====\n");
    if !errors.is_empty() {
        s.push_str("tex-ls parse errors (SkippedTexLs):\n");
        s.push_str(errors);
    }
    s.push_str("diff (`-` tex-ls-only, `+` texlab-only, ` ` common):\n");
    s.push_str(&unified_diff(tex_ls, texlab));
    s.push('\n');
    s
}

/// A minimal LCS-aligned unified diff of two line slices. `-` marks a line only in
/// `a` (tex-ls), `+` a line only in `b` (texlab), and a leading space a shared line.
...
fn render_report(
    ...
    for r in reports {
        ...
        match r.outcome {
            ...
            Outcome::SkippedTexLs => skipped_tex_ls += 1,
    ...
    s.push_str(
        "_Generated by `cargo test -p tex-ls-parser --test parse_compat -- --ignored --nocapture` (`tests/parse_compat.rs`). Do not edit by hand._\n\n",
    );
    s.push_str(
        "This is a **soft gauge, not a quality gate.** It projects tex-ls's generic CST \
         ...
         recorded deviation or an open question (tex-ls models TeX surface syntax; texlab \
    ...
    if skipped_tex_ls > 0 {
        s.push_str(&format!(
            "- **Skipped:** {skipped_tex_ls} (tex-ls could not parse cleanly)\n"
        ));
    }
    if texlab_errors > 0 {
        s.push_str(&format!(
            "- **texlab parse errors:** {texlab_errors} (tex-ls-clean inputs texlab flagged — see the gate, `parse_oracle.rs`)\n"
        ));
    }
    s.push('\n');
