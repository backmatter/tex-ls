...
/// An upstream delimiter-shaped role before tex-ls's curated exceptions.
#[derive(Deserialize, Clone, Copy)]
#[serde(rename_all = "lowercase")]
...
#[serde(deny_unknown_fields)]
struct RawDb {
    #[serde(default, rename = "commandPackages")]
...
fn generate_cwl_signatures() {
    let json = std::fs::read_to_string("data/cwl_signatures.json").expect(
        "data/cwl_signatures.json must exist (run `python3 scripts/gen_cwl_signatures.py --write`)",
    );
    ...
    for (name, packages) in [
        ...
        for (symbol, packages) in packages {
            map.entry(symbol.as_str(), format!("&{packages:?}"));
        }
        writeln!(
            out,
            "static {name}: phf::Map<&'static str, &'static [&'static str]> = {};",
            map.build()
        )
        .unwrap();
    }
...
fn generate_package_metadata() {
    let json = std::fs::read_to_string("data/package_metadata.json")
        .expect("data/package_metadata.json must exist (run `python3 scripts/gen_package_names.py --write`)");
...
fn generate_math_symbols() {
    let json = std::fs::read_to_string("data/math_symbols.json").expect(
        "data/math_symbols.json must exist (run `python3 scripts/gen_math_symbols.py --write`)",
    );
    ...
    let mut glyphs = phf_codegen::Map::new();
    for (codepoint, command, class, delimiter) in &file.symbols {
        ...
        glyphs.entry(command.as_str(), format!("{character:?}"));
    }
    ...
    writeln!(
        out,
        "static COMMAND_GLYPHS: phf::Map<&'static str, char> = {};",
        glyphs.build()
    )
    .unwrap();
