...
//!   `.aux` ([`meaning_analysis::project::aux`]) — `Figure 3: A chart`, `Section 1.2 (Intro)`.
//!   Degrades to the numberless preview when the project was never compiled.
//!
//! The user-macro definition body hover is deferred in `TODO.md`.
...
use meaning_analysis::bib::ast as bib_ast;
use meaning_analysis::bib::syntax::SyntaxNode as BibSyntaxNode;
use meaning_parser::ast::{command_name, nth_group, nth_group_inner};
use meaning_parser::semantic::completion::{PackageMeta, package_metadata};
use meaning_parser::semantic::pkgmeta::{NeedsFormatDecl, OptionDecl, ProvidesDecl, provides_kind};
use meaning_parser::semantic::signature::{
    ArgKind, CommandSig, EnvironmentSig, OutlineKind, builtin, cwl,
};
use meaning_parser::semantic::{LabelContext, label_context};
use meaning_parser::syntax::{SyntaxKind, SyntaxToken};
...
fn declaration_hover(
    ...
    let note = match name.as_str() {
        "ProcessOptions" => "**Processes package options** — recognized, never executed by meaning",
        "ExecuteOptions" => "**Executes default options** — recognized, never executed by meaning",
        _ => return None,
    };
...
pub fn arg_summary(args: &[meaning_parser::semantic::signature::ArgSpec]) -> Option<String> {
...
fn render_citation(
    ...
    for bib_path in citations.bib_definers(lint_path) {
        ...
        let Some(node) = bib_ast::entry_at_range(&root, entry.range) else {
            continue;
        };
...
pub fn render_entry(entry_type: &str, key: &str, node: &BibSyntaxNode) -> String {
...
/// call sites.
pub fn clean_value(value: &BibSyntaxNode) -> String {
...
mod tests {
    ...
    fn markdown_at(
        ...
        let hover = compute_hover(
            ...
            position,
            &crate::test_host::TestHost::default(),
        )?;
