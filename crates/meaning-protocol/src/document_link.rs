...
//!   same argument [`meaning_analysis::project::include`] turns into a graph edge.
...
//!
//! Unlike [`meaning_analysis::project::include`], resolution here is **disk-aware**: a link
...
use meaning_analysis::external::FileCandidates;
use meaning_analysis::project::include::subfiles_parent_arg;
use meaning_parser::ast::{command_name, nth_group_inner, nth_group_text};
...
fn resolve_existing(
    ...
    let candidates = FileCandidates::new(raw, exts, dtx, base_dir);
    candidates
        .local
        .into_iter()
        .find(|path| texmf.is_file(path))
        .or_else(|| {
            let (stem, extensions) = candidates.installed?;
...
///
/// Shared with [`super::hover`]'s package-name hover (`pub`), which picks the
...
mod tests {
    ...
    fn links(src: &str, base_dir: &Path) -> Vec<LinkTarget> {
        links_with_texmf(src, base_dir, &crate::test_host::TestHost::default())
    }

    /// [`links`] with an explicit installed-tree index for the system-package tier.
    fn links_with_texmf(src: &str, base_dir: &Path, texmf: &dyn HostServices) -> Vec<LinkTarget> {
