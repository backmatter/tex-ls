...
//! module is rule-agnostic: any finding whose [`meaning_analysis::linter::Diagnostic::fix`] is
...
use meaning_analysis::linter::diagnostic::Applicability;
use meaning_parser::ast::{AstNode, Command, Environment, Group, children};
use meaning_parser::syntax::SyntaxElement;
...
pub fn code_actions_for_range(
    findings: &[meaning_analysis::linter::Diagnostic],
    text: &TextBuffer,
    uri: &Uri,
    self_path: &Path,
    request_range: Range,
...
fn table_column_insertions(
    ...
    let definitions = meaning_parser::semantic::scan_definitions(root);
    let env = root
        .descendants()
    ...
    let columns =
        meaning_formatter::formatter::column_count(&spec.inner_source()).filter(|&n| n > 0)?;
...
fn row_insertion(
    ...
    boundary: TextSize,
    text: &str,
    definitions: &meaning_parser::semantic::SignatureDb,
...
fn is_rule_command(
    ...
    command.name().is_some_and(|name| {
        definitions.command(&name).is_none()
            && meaning_parser::semantic::signature::builtin()
                .command(&name)
...
fn workspace_changes(
    fix: &meaning_analysis::linter::Fix,
    self_uri: &Uri,
    self_idx: &LineIndex,
    enc: PositionEncoding,
    resolve: &dyn Fn(&Path) -> Option<(Uri, String)>,
...
mod tests {
    use super::*;
    use meaning_analysis::linter::check_document;
    use meaning_parser::parser::LexConfig;
    ...
    fn findings(src: &str) -> Vec<meaning_analysis::linter::Diagnostic> {
        check_document(
            std::path::Path::new("x.tex"),
            &buf(src),
            LexConfig::default(),
            &meaning_parser::declarations::ResolvedDeclarations::default(),
        )
    }
    ...
    fn cross_file_fix_spans_two_uris() {
        use meaning_analysis::linter::{Diagnostic, Edit, Fix, Severity};
    ...
    fn apply_table_action(src: &str, line: u32, character: u32) -> Option<(CodeAction, String)> {
        let root = SyntaxNode::new_root(meaning_parser::parser::parse(src).green);
        let buffer = buf(src);
