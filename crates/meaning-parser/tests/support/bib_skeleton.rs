...
//! `bib_parse_compat.rs`).
//!
//! Like the LaTeX `parse_skeleton`, this projects both meaning's and texlab's bib CSTs
...
//!    (`ACCENT`), commands (`COMMAND`), and `~` (`NBSP`) inside braces/quotes; meaning
...
/// Project meaning's bib CST for `text` onto the skeleton forest.
pub fn project_meaning(text: &str) -> Vec<BibAtom> {
...
/// The entry-type word of a meaning `ENTRY` node, lowercased.
fn meaning_entry_type(entry: &bib::syntax::SyntaxNode) -> String {
...
/// The `Field(name)` atoms of a meaning `ENTRY` node.
fn meaning_fields(entry: &bib::syntax::SyntaxNode) -> Vec<BibAtom> {
...
/// meaning does for a meaning-clean file: a one-way soundness floor that catches
/// meaning hallucinating structure texlab does not see, without depending on an error
