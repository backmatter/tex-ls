...
//! `parse_compat.rs`).
//!
//! meaning's CST is deliberately *generic* (`\section`→`COMMAND`, environments by
...
///
/// meaning and texlab pin *different* rowan releases, so `rowan::Language`,
...
/// `rowan::Language` silently welds meaning's rowan version to texlab's.
///
...
fn project_elem<P: Projector>(elem: &Elem<P>) -> Vec<Atom> {
    match elem {
        ...
        // A bare command-name token with no enclosing command node. meaning emits
...
impl_tree!(
    Meaning,
    meaning_parser::syntax::SyntaxKind,
    meaning_parser::syntax::SyntaxNode,
    meaning_parser::syntax::SyntaxToken
);

impl Projector for Meaning {
    fn cat(kind: meaning_parser::syntax::SyntaxKind) -> Cat {
    ...
    fn is_begin_or_end(kind: meaning_parser::syntax::SyntaxKind) -> bool {
        use meaning_parser::syntax::SyntaxKind::{BEGIN, END};
        matches!(kind, BEGIN | END)
    }

    fn is_command_token(kind: meaning_parser::syntax::SyntaxKind) -> bool {
        use meaning_parser::syntax::SyntaxKind::{CONTROL_SYMBOL, CONTROL_WORD};
        matches!(kind, CONTROL_WORD | CONTROL_SYMBOL)
    }

    fn is_word_token(kind: meaning_parser::syntax::SyntaxKind) -> bool {
        kind == meaning_parser::syntax::SyntaxKind::WORD
    }

    fn is_verbatim_token(kind: meaning_parser::syntax::SyntaxKind) -> bool {
        use meaning_parser::syntax::SyntaxKind::{VERB, VERBATIM_BODY};
        matches!(kind, VERB | VERBATIM_BODY)
    }
}

/// Project meaning's CST for `text`.
pub fn project_meaning(text: &str) -> Vec<Atom> {
    let parsed = meaning_parser::parser::parse(text);
    project::<Meaning>(&parsed.syntax())
}
...
impl Projector for Texlab {
    fn cat(kind: texlab_syntax::latex::SyntaxKind) -> Cat {
        ...
        match kind {
            ...
            // dropping texlab's semantic role so it lines up with meaning's
            // generic `COMMAND`.
            GENERIC_COMMAND
            | PART
            | CHAPTER
            ...
            // ROOT, PREAMBLE, TEXT, KEY/VALUE wrappers, PAREN_GROUP (meaning treats
