...
use meaning_parser::parser::{LatexFlavor, LexConfig, parse_with_declarations, parse_with_flavor};
...
fn property_config() -> ProptestConfig {
    let cases = std::env::var("MEANING_PROPERTY_CASES")
        .map(|value| {
            value
                .parse()
                .expect("MEANING_PROPERTY_CASES must be a positive integer")
        })
        .unwrap_or(DEFAULT_CASES);
    assert!(cases > 0, "MEANING_PROPERTY_CASES must be positive");
    ProptestConfig {
        cases,
