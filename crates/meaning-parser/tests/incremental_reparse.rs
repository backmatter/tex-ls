...
/// How many edits each snippet gets. Scaled by `MEANING_REPARSE_FUZZ_ITERS`, so the
/// gate before a default flip can run the same harness at 10x without a code change.
fn iterations(base: usize) -> usize {
    let scale: usize = std::env::var("MEANING_REPARSE_FUZZ_ITERS")
        .ok()
...
fn the_harness_rejects_a_perturbed_error_vector() {
    ...
    let result = Reparsed {
        green: parse.green,
        errors: vec![meaning_parser::parser::SyntaxError {
...
/// parse's vector only if emission order is positional. meaning has one error
