...
//!
//! ```sh
//! cargo test --release -p meaning-parser --test reparse_corpus_sweep -- --ignored --nocapture
//! ```
//!
...
use meaning_parser::parser::{Edit, LatexFlavor, LexConfig};
...
/// Which corpora to sweep. `MEANING_REPARSE_SWEEP_CORPORA` restricts the set so
...
fn selected_corpora() -> Vec<String> {
    match std::env::var("MEANING_REPARSE_SWEEP_CORPORA") {
