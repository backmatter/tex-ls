...
//! corpus file that *meaning itself parses cleanly* (no syntax errors), texlab must
//! recognize **at least as many** top-level structured entries (`@entry` / `@string` /
//! `@preamble`) as meaning does. That catches the dangerous direction — meaning
...
use bib_skeleton::{count_entries, project_meaning, project_texlab};

#[test]
fn texlab_recognizes_meaning_clean_entries() {
    ...
    for entry in fs::read_dir(&dir).expect("read bib_corpus dir") {
        ...
        // Only assert on inputs meaning accepts cleanly (mirrors `parse_oracle.rs`).
        if !meaning_parser::bib::parse(&text).errors.is_empty() {
            continue;
        }

        let name = path.file_name().unwrap().to_string_lossy();
        let meaning_entries = count_entries(&project_meaning(&text));
        let texlab_entries = count_entries(&project_texlab(&text));
        assert!(
            texlab_entries >= meaning_entries,
            "entry-recognition floor violated for meaning-clean corpus file `{name}`: \
             meaning recognized {meaning_entries} structured entries but texlab only {texlab_entries}"
        );
        checked += 1;
    }
