...
//!
//! The bib analog of `parse_compat.rs`. It measures how *structurally close* meaning's
//! bib CST is to texlab's over a corpus, after projecting both onto one coarse skeleton
//! (see `bib_skeleton`). A divergence from texlab is **never a bug by itself**: meaning
...
//! values; meaning keeps them generic). AGENTS.md: measure against, never match.
//!
...
//! ```
//!
//! The result is written to `docs/BIB_PARSE_COMPAT.md`
...
use bib_skeleton::{dice, lcs_len, project_meaning, project_texlab, render_lines};
...
fn bib_parse_compat_report() {
    ...
    for (key, path) in &corpus {
        ...
        if !meaning_parser::bib::parse(&text).errors.is_empty() {
        ...
        let meaning_lines = render_lines(&project_meaning(&text));
    ...
    let out_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../docs/BIB_PARSE_COMPAT.md");
...
fn render_report(
    ...
    s.push_str(
        "This is a **soft gauge, not a quality gate.** It projects meaning's bib CST and \
