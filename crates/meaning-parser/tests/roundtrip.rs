...
use meaning_parser::declarations::Declarations;
...
fn roundtrip_declared_corpus_file() {
    ...
    let environments = |root: &meaning_parser::syntax::SyntaxNode| {
        root.descendants()
            .filter(|n| n.kind() == meaning_parser::syntax::SyntaxKind::ENVIRONMENT)
            .count()
    };
