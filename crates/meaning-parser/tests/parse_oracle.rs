...
//! curated corpus file that *meaning itself parses cleanly* (no syntax errors), the
...
//! (meaning's generic CST and texlab's semantic CST are not meant to match).
//!
...
/// Corpus files meaning legitimately parses cleanly but texlab cannot, each with
...
const TEXLAB_KNOWN_DIVERGENT: &[&str] = &[
    ...
    // opaque VERB spans for meaning (issue #57), unmatched braces for texlab.
    "doc_shortverb.tex",
    ...
    // meaning, but texlab reads their `$`/`\[`/`\]` as math delimiters.
    "char_constant.tex",
    "alignment_char_constant.tex",
    // A `$` in `\def` parameter text delimits an argument; it does not open math.
    // Meaning models that TeX syntax, while texlab lets the apparent math span
    ...
    // stray `}` inside are data for meaning, an unclosed env and an unmatched
    // brace for texlab, whose environment DB does not carry the signature.
    "verbatim_ltxexample.tex",
];

#[test]
fn texlab_accepts_meaning_clean_corpus() {
    ...
    for entry in fs::read_dir(&dir).expect("read corpus dir") {
        ...
        // Only assert on inputs meaning accepts cleanly (mirrors air_parser_harness).
        ...
        assert!(
            ...
            "texlab reported a parse error for meaning-clean corpus file `{name}`"
        );
        checked += 1;
    }
