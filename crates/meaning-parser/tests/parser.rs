...
// --- declared environments (`meaning.toml`; AGENTS.md decision #12) ----------
...
fn a_declared_verbatim_environment_captures_its_body() {
    // The parked `codeexample` knob: an environment meaning cannot name, whose
...
fn a_declared_statement_environment_wraps_like_the_entry_it_copies() {
    ...
    let count = |parsed: &meaning_parser::parser::Parse| {
        parsed
            .syntax()
            .descendants()
            .filter(|n| n.kind() == SyntaxKind::STATEMENT)
