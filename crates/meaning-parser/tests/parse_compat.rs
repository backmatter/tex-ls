...
//!
//! This harness measures how *structurally close* meaning's CST is to texlab's over a
...
//! same principle: a divergence from texlab is **never a bug by itself**. meaning is
...
//! ```
//!
//! Methodology — for each corpus file we project meaning's and texlab's trees onto the
//! shared skeleton, render each to indented S-expression lines, and score the Dice
//! coefficient over those lines. Files meaning cannot parse cleanly are skipped (we
//! never measure against our own parse errors). The result is written to
//! `docs/PARSE_COMPAT.md` and is a triage queue, not a
//! pass/fail signal.
...
use parse_skeleton::{
    Meaning, dice, lcs_len, project, project_texlab, render_lines, texlab_has_error,
};
...
enum Outcome {
    ...
    /// meaning could not parse the input cleanly (syntax errors).
    SkippedMeaning,
}
...
fn parse_compat_report() {
    ...
    // so a classifier can read the actual meaning-vs-texlab shapes (the report only
    ...
    let mut total_lines = 0usize; // sum of (meaning_lines + texlab_lines) over compared files
    ...
    for (key, path) in &corpus {
        ...
        if !parsed.errors.is_empty() {
            if dump_enabled {
                let meaning_lines = render_lines(&project::<Meaning>(&parsed.syntax()));
                ...
                let errs: String = parsed
                    ...
                    .map(|e| format!("    {e:?}\n"))
                    .collect();
                dump.push_str(&dump_section(key, &errs, &meaning_lines, &texlab_lines));
            }
            reports.push(FileReport {
                key: key.clone(),
                outcome: Outcome::SkippedMeaning,
                texlab_error: false,
            });
            continue;
        }
        ...
        total_lcs2 += 2 * lcs_len(&meaning_lines, &texlab_lines);
        total_lines += meaning_lines.len() + texlab_lines.len();

        let outcome = if meaning_lines == texlab_lines {
            Outcome::Concordant
        } else {
            Outcome::Divergent {
                similarity: dice(&meaning_lines, &texlab_lines),
            }
        };
        if dump_enabled && !matches!(outcome, Outcome::Concordant) {
            dump.push_str(&dump_section(key, "", &meaning_lines, &texlab_lines));
        }
        reports.push(FileReport {
            key: key.clone(),
            outcome,
    ...
    let out_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../docs/PARSE_COMPAT.md");
...
/// One file's section in the triage dump: optional meaning parse errors followed
/// by an LCS-aligned diff of the two skeletons.
fn dump_section(key: &str, errors: &str, meaning: &[String], texlab: &[String]) -> String {
    let mut s = format!("===== {key} =====\n");
    if !errors.is_empty() {
        s.push_str("meaning parse errors (SkippedMeaning):\n");
        s.push_str(errors);
    }
    s.push_str("diff (`-` meaning-only, `+` texlab-only, ` ` common):\n");
    s.push_str(&unified_diff(meaning, texlab));
    s.push('\n');
    s
}

/// A minimal LCS-aligned unified diff of two line slices. `-` marks a line only in
/// `a` (meaning), `+` a line only in `b` (texlab), and a leading space a shared line.
...
fn render_report(
    ...
    let mut skipped_meaning = 0usize;
    ...
    for r in reports {
        ...
        match r.outcome {
            ...
            Outcome::SkippedMeaning => skipped_meaning += 1,
    ...
    s.push_str(
        "This is a **soft gauge, not a quality gate.** It projects meaning's generic CST \
         ...
         recorded deviation or an open question (meaning models TeX surface syntax; texlab \
    ...
    if skipped_meaning > 0 {
        s.push_str(&format!(
            "- **Skipped:** {skipped_meaning} (meaning could not parse cleanly)\n"
        ));
    }
    if texlab_errors > 0 {
        s.push_str(&format!(
            "- **texlab parse errors:** {texlab_errors} (meaning-clean inputs texlab flagged — see the gate, `parse_oracle.rs`)\n"
        ));
    }
    s.push('\n');
