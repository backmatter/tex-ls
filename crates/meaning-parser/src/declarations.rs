...
///
/// Deliberately shape-only, with no check that the name is one meaning knows: a
...
/// bare key so the result can be pasted back into `meaning.toml`.
///
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::UnknownLikeTarget { target } => write!(
                f,
                "unknown environment `{target}`; `like` must name an environment meaning \
                 knows about"
            ),
            ...
            Self::UndeclaredTarget => write!(
                f,
                "declares delimiters for an environment meaning does not know; add `like` \
            ...
            Self::SpellingIsALiteralDelimiter { name, environment } => write!(
                ...
                "`{name}` is the delimiter itself, not a command standing in for one — and \
                 meaning already pairs a declared spelling with the written-out \
            ...
            Self::SpellingIsABuiltinCommand { name } => write!(
                f,
                "`{name}` is already a LaTeX command meaning knows; a delimiter spelling must \
