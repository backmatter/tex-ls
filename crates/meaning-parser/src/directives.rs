//! Comment directives that turn meaning off for part of a file.
//!
...
//!
//! ```text
//! % meaning-format <verb>              layout only
//! % meaning-lint   <verb> [<rule>]     linting only, optionally one rule
//! % meaning        <verb>              both at once
//! ```
//!
//! with `<verb>` one of:
//!
...
//!
//! ## The retired `% meaning-ignore` family
//!
//! ```text
//! % meaning-ignore <rule>: <reason>        → % meaning-lint skip <rule>: <reason>
//! % meaning-ignore-file <rule>: <reason>   → % meaning-lint skip-file <rule>: <reason>
//! % meaning-ignore-file: <reason>          → % meaning-lint skip-file: <reason>
//! ```
//!
...
pub enum Axis {
    /// `% meaning-format …` — layout only. Lint findings are still reported.
    Format,
    /// `% meaning-lint …` — linting only, for one rule or all of them.
    Lint,
    /// `% meaning …` — layout *and* every lint rule.
    Both,
}
...
pub struct Directive {
    ...
    /// Written in the retired `% meaning-ignore` spelling. Behaves identically —
...
///
/// Leading `%`s are all stripped, so `%%% meaning-format off` works; the verb
...
pub fn parse_directive(comment: &str) -> Option<Directive> {
    ...
    if let Some(rest) = body.strip_prefix("meaning-ignore-file") {
    ...
    if let Some(rest) = body.strip_prefix("meaning-ignore") {
        // The retired node form always required a rule; a bare `% meaning-ignore`
    ...
    let (axis, rest) = if let Some(rest) = body.strip_prefix("meaning-format") {
        (Axis::Format, rest)
    } else if let Some(rest) = body.strip_prefix("meaning-lint") {
        (Axis::Lint, rest)
    } else {
        (Axis::Both, body.strip_prefix("meaning")?)
    };
    // The family name must end at a word boundary, so `% meaning-formatting off`
    // and `% meaninglint skip` are ordinary comments.
    if !rest.starts_with([' ', '\t']) {
        return None;
    }
...
impl Suppressions {
    ...
    pub fn build(root: &SyntaxNode) -> Self {
        ...
        // Regions are keyed by axis *and* rule: a `% meaning-lint off` covering
        // every rule is not closed by a `% meaning-lint on some-rule`, which
...
fn directive_family(directive: &Directive) -> &'static str {
    if directive.deprecated {
        return match directive.verb {
            Verb::SkipFile => "meaning-ignore-file",
            Verb::Skip => "meaning-ignore",
            Verb::Off | Verb::On => unreachable!("retired directives have no region verbs"),
        };
    }
    match directive.axis {
        Axis::Format => "meaning-format",
        Axis::Lint => "meaning-lint",
        Axis::Both => "meaning",
    }
}
...
mod tests {
    ...
    fn classifies_inert_and_incomplete_directives() {
        for (src, expected) in [
            (
                "% meaning-lint skip deprecated-command\n",
                DirectiveOutcome::DanglingSkip,
            ),
            (
                "% meaning-lint on deprecated-command\n",
                DirectiveOutcome::UnmatchedOn,
            ),
            (
                "% meaning-lint off deprecated-command\n\\bf\n",
    ...
    fn matched_and_targeted_directives_are_honored() {
        for src in [
            "% meaning-lint skip deprecated-command\n\\bf\n",
            "% meaning-lint off deprecated-command\n\\bf\n% meaning-lint on deprecated-command\n",
            "% meaning-lint skip-file deprecated-command\n",
        ] {
            let suppressions = suppressions_of(src);
            assert!(
                suppressions
    ...
    fn retains_dtx_doc_margin_directive_as_unsupported() {
        let src = "% meaning-lint skip deprecated-command\nDocumentation.\n";
    ...
    fn retains_directives_with_their_carrier_ranges() {
        let src = "% a note\n% meaning-ignore deprecated-command: legacy\n\\bf\n";
        let suppressions = suppressions_of(src);
    ...
    fn parses_every_form_on_every_axis() {
        for (family, axis) in [
            ("meaning-format", Axis::Format),
            ("meaning-lint", Axis::Lint),
            ("meaning", Axis::Both),
        ] {
            for (word, verb) in [
                ("skip", Verb::Skip),
    ...
    fn only_the_lint_axis_takes_a_rule() {
        assert_eq!(
            parse_directive("% meaning-lint skip deprecated-command: legacy"),
            Some(Directive {
                axis: Axis::Lint,
                verb: Verb::Skip,
        ...
        assert_eq!(
            parse_directive("% meaning-format skip deprecated-command"),
            Some(directive(Axis::Format, Verb::Skip))
        );
        assert_eq!(
            parse_directive("% meaning skip deprecated-command"),
    ...
    fn lint_rule_is_optional_and_means_every_rule() {
        assert_eq!(
            parse_directive("% meaning-lint skip-file: generated"),
    ...
    fn reason_is_optional_and_ignored() {
        assert_eq!(
            parse_directive("% meaning-format skip: hand-aligned by eye"),
            Some(directive(Axis::Format, Verb::Skip))
        );
        assert_eq!(
            parse_directive("%meaning skip-file:generated"),
    ...
    fn repeated_percent_is_allowed() {
        assert_eq!(
            parse_directive("%%% meaning-format off"),
    ...
    fn retired_ignore_family_still_parses() {
        assert_eq!(
            parse_directive("% meaning-ignore deprecated-command: legacy"),
            Some(Directive {
                axis: Axis::Lint,
                verb: Verb::Skip,
                rule: Some("deprecated-command".into()),
        ...
        assert_eq!(
            parse_directive("% meaning-ignore-file deprecated-command: legacy"),
            Some(Directive {
                axis: Axis::Lint,
                verb: Verb::SkipFile,
                rule: Some("deprecated-command".into()),
        ...
        assert_eq!(
            parse_directive("% meaning-ignore-file: noisy"),
            Some(Directive {
                axis: Axis::Lint,
                verb: Verb::SkipFile,
                rule: None,
    ...
    fn bare_retired_node_directive_stays_inert() {
        assert_eq!(parse_directive("% meaning-ignore"), None);
        assert_eq!(parse_directive("% meaning-ignore: no rule named"), None);
    }
    ...
    fn non_directives_are_inert() {
        for text in [
            "% just a note",
            "% meaning",                 // no verb
            "% meaning-lint",            // no verb
            "% meaning-format nonsense", // unknown verb
            "% meaningformat off",       // no word boundary
            "% meaning-formatting off",  // no word boundary
            "% meaninglint skip",        // no word boundary
            "% the meaning-format off",  // not at the start
        ] {
    ...
    fn skip_targets_the_documented_construct() {
        let src = "% meaning-format skip: hand-aligned\n\\begin{tikzpicture}\n\\draw (0,0);\n\\end{tikzpicture}\n";
    ...
    fn lint_region_is_rule_selective() {
        ...
        assert_eq!(
            slices(src, ranges),
            vec!["% meaning-lint off deprecated-command\n\\beta\n"]
        );
    }

    #[test]
    fn skip_file_covers_the_document_on_its_axis() {
        let src = "\\alpha\n% meaning-format skip-file: generated\n\\beta\n";
        let s = suppressions_of(src);
    ...
    fn overlapping_ranges_merge() {
        let src = "% meaning-format skip-file: generated\n% meaning-format off\n\\b\n";
        let s = suppressions_of(src);
        assert_eq!(slices(src, s.format_ranges()), vec![src]);
    }
    ...
    fn retired_and_current_spellings_resolve_identically() {
        ...
        for (old, new) in [
            (
                "% meaning-ignore deprecated-command: legacy\n\\bf x\n",
                "% meaning-lint skip deprecated-command: legacy\n\\bf x\n",
            ),
            (
                "% meaning-ignore-file deprecated-command: legacy\n\\bf x\n",
                "% meaning-lint skip-file deprecated-command: legacy\n\\bf x\n",
            ),
        ] {
        ...
        let old = suppressions_of("% meaning-ignore-file: noisy\n\\bf x\n");
        let new = suppressions_of("% meaning-lint skip-file: noisy\n\\bf x\n");
