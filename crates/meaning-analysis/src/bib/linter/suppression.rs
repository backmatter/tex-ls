...
//!
//! ```text
//! @comment{meaning-lint skip <rule>: <reason>}       the next entry, one rule
//! @comment{meaning-lint skip: <reason>}              the next entry, every rule
//! @comment{meaning-lint off <rule>} … {… on <rule>}  a region of entries
//! @comment{meaning-lint skip-file <rule>: <reason>}  the whole file
//! ```
//!
...
//! `@comment{meaning-ignore …}` and `@comment{meaning-ignore-file …}` spellings
//! still resolve through the same path and are no longer documented.
//!
//! **Only the lint axis acts here.** `meaning-format` is accepted by the shared
        ...
        for node in root.descendants() {
            ...
            let family = if directive.deprecated {
                ...
                match directive.axis {
                    crate::directives::Axis::Format => "meaning-format",
                    crate::directives::Axis::Lint => "meaning-lint",
                    crate::directives::Axis::Both => "meaning",
                }
            };
            let relative = text
                .find(family)
...
mod tests {
    ...
    fn classifies_inert_and_incomplete_directives() {
        for (src, expected) in [
            (
                "@comment{meaning-lint skip missing-required-field}\n",
                DirectiveOutcome::DanglingSkip,
            ),
            (
                "@comment{meaning-lint on missing-required-field}\n",
                DirectiveOutcome::UnmatchedOn,
            ),
            (
                "@comment{meaning-lint off missing-required-field}\n@book{a}\n",
                DirectiveOutcome::UnclosedOff,
            ),
            (
                "@comment{meaning-format skip-file}\n@book{a}\n",
                DirectiveOutcome::Unsupported,
            ),
        ] {
            let map = map_of(src);
    ...
    fn retains_the_full_bib_directive_range() {
        let src = "@comment{meaning-format skip-file}\n";
        let map = map_of(src);
    ...
    fn skip_file_without_a_rule_suppresses_everything() {
        let m = map_of("@comment{meaning-lint skip-file: noisy}\n@book{a, title={T}}\n");
        assert!(m.is_suppressed("anything", 0, 1));
    }

    #[test]
    fn skip_file_with_a_rule_suppresses_only_that_rule() {
        let m = map_of("@comment{meaning-lint skip-file missing-required-field: gone}\n");
    ...
    fn skip_targets_the_following_entry() {
        let src = "@comment{meaning-lint skip missing-required-field: gone}\n@book{a, title={T}}\n";
        let m = map_of(src);
    ...
    fn region_covers_the_entries_between_its_markers() {
        let src = "@comment{meaning-lint off missing-required-field}\n\
                   @book{a, title={T}}\n\
                   @comment{meaning-lint on missing-required-field}\n\
                   @book{b, title={U}}\n";
        let m = map_of(src);
    ...
    fn retired_ignore_family_still_suppresses() {
        let src = "@comment{meaning-ignore missing-required-field: gone}\n@book{a, title={T}}\n";
        let m = map_of(src);
        let at = src.find("@book").expect("has the entry");
        assert!(m.is_suppressed("missing-required-field", at, at + 5));

        let file = map_of("@comment{meaning-ignore-file: noisy}\n@book{a, title={T}}\n");
        assert!(file.is_suppressed("anything", 0, 1));
    }

    /// `meaning-format` parses but has no bib meaning; it must not silence a
    /// diagnostic on its way through.
    #[test]
    fn format_family_does_not_suppress_lint() {
        let m = map_of("@comment{meaning-format skip-file}\n@book{a, title={T}}\n");
        assert!(!m.is_suppressed("missing-required-field", 0, 1));
    }
