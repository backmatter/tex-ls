...
pub(crate) fn resolved_labels(db: &dyn IncrementalDb, context: ProjectInput) -> ResolvedLabels {
    ...
    let files: Vec<(PathBuf, Vec<SmolStr>, Vec<SmolStr>, bool)> = project
        ...
        .map(|member| {
            (
                member.path.clone(),
                file_labels(db, db.source_input(member.file, *context.identity(db))).clone(),
                file_refs(db, db.source_input(member.file, *context.identity(db))).clone(),
                *file_is_document_root(db, db.source_input(member.file, *context.identity(db))),
            )
        })
        .collect();
