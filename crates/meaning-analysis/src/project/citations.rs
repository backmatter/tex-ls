...
pub(crate) fn resolved_citations(
    ...
    let project = crate::project::workspace_project(db, context);
    let graph = project_graph(db, context);
    ...
    for member in &project.members {
        match member.kind {
            ...
            | FileKind::Ins => {
                let facts =
                    file_cite_facts(db, db.source_input(member.file, *context.identity(db)));
                cite_facts.push(CiteFileFacts {
                    path: member.path.clone(),
                    bib_targets: facts.bib_targets.clone(),
                    nocite_all: facts.nocite_all,
                    is_document_root: *file_is_document_root(
                        db,
            ...
            FileKind::Bib => {
                bib_keys.insert(
                    member.path.clone(),
                    file_cite_names(db, db.source_input(member.file, *context.identity(db)))
                        .clone(),
                );
            }
        }
    ...
    let bib_aliases = crate::incremental::bibliography_aliases(db, context)
        .iter()
        .cloned()
        .collect();
