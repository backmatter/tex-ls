...
//!
//! ```text
//! % meaning-lint skip <rule>: <reason>       the next construct, one rule
//! % meaning-lint skip: <reason>              the next construct, every rule
//! % meaning-lint off <rule> … on <rule>      a region
//! % meaning-lint skip-file <rule>: <reason>  the whole file
//! % meaning skip / off / on / skip-file      the same, plus the formatter
//! ```
//!
//! Omitting `<rule>` means every rule. The retired `% meaning-ignore` and
//! `% meaning-ignore-file` spellings still resolve, through the same path, and
...
mod tests {
    ...
    fn skip_file_without_a_rule_suppresses_everything() {
        let m = map_of("% meaning-lint skip-file: noisy\n\\bf\n");
        assert!(m.is_suppressed("anything", 0, 1));
    }

    #[test]
    fn skip_file_with_a_rule_suppresses_only_that_rule() {
        let m = map_of("% meaning-lint skip-file deprecated-command: legacy\n\\bf\n");
    ...
    fn retired_ignore_family_still_suppresses() {
        let node = map_of("% meaning-ignore deprecated-command: legacy\n\\bf\n");
        let at = "% meaning-ignore deprecated-command: legacy\n".len();
        assert!(node.is_suppressed("deprecated-command", at, at + 3));
        assert!(!node.is_suppressed("duplicate-label", at, at + 3));

        let file_rule = map_of("% meaning-ignore-file deprecated-command: legacy\n\\bf\n");
        assert!(file_rule.is_suppressed("deprecated-command", 0, 1));
        assert!(!file_rule.is_suppressed("duplicate-label", 0, 1));

        let file_all = map_of("% meaning-ignore-file: noisy\n\\bf\n");
