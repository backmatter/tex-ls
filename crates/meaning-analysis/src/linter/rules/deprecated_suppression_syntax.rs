//! `deprecated-suppression-syntax`: retired `% meaning-ignore` suppression
...
//! `meaning-ignore` with `meaning-lint skip`, or `meaning-ignore-file` with
//! `meaning-lint skip-file`, preserves the selector and reason verbatim and
...
const EXAMPLES: &[Example] = &[Example {
    ...
    source: "% meaning-ignore deprecated-command: legacy source\n{\\bf text}\n",
}];
...
impl Rule for DeprecatedSuppressionSyntax {
    ...
    fn description(&self) -> &'static str {
        "Flag the retired `% meaning-ignore <rule>` and `% meaning-ignore-file \
         ...
         and verb to `% meaning-lint skip <rule>` or `% meaning-lint skip-file \
    ...
    fn check_file(&self, ctx: &RuleContext<'_>, sink: &mut Vec<Diagnostic>) {
        for located in ctx.suppressions.directives().iter().filter(|located| {
            ...
            let replacement = match located.directive.verb {
                Verb::Skip => "meaning-lint skip",
                Verb::SkipFile => "meaning-lint skip-file",
...
mod tests {
    ...
    fn rewrites_node_suppression_safely() {
        ...
        assert_eq!(&src[out[0].start..out[0].end], "meaning-ignore");
    ...
    fn rewrites_file_suppression_and_preserves_reason() {
        let src = "%%% meaning-ignore-file: generated file\ntext\n";
        let out = findings(src);
        assert_eq!(out.len(), 1);
        let fix = out[0].fix.as_ref().expect("has fix");
        assert_eq!(
            apply_fixes(src, std::slice::from_ref(fix), false).output,
            "%%% meaning-lint skip-file: generated file\ntext\n"
        );
    }

    #[test]
    fn modern_and_ordinary_comments_are_fine() {
        assert!(findings("% meaning-lint skip deprecated-command\n\\bf\n").is_empty());
        assert!(findings("% meaning is good\n").is_empty());
    }

    #[test]
    fn does_not_rewrite_dtx_documentation_prose() {
        let src = "% meaning-ignore deprecated-command\nDocumentation.\n";
