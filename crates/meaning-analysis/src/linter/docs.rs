...
pub(crate) fn render_rule_section(
    ...
    for example in section.examples {
        ...
        // Safe fixes only, matching what `meaning lint --fix` applies.
...
/// or `None` if the id is unknown. Backs `meaning lint --explain <rule>`, reusing
...
`meaning lint` runs a set of built-in rules over each file's parse tree and
...
and what a `% meaning-lint skip <id>` comment suppresses.
...
is applied by `meaning lint --fix`; an *unsafe* fix, one that may change output
...
```tex
% meaning-lint skip deprecated-command: legacy code, leave as-is
{\\bf here}
```
...
```tex
% meaning-lint off deprecated-command: legacy chapter
{\\bf here}
{\\it and here}
...
`% meaning skip` / `off` / `on` / `skip-file` do the same and turn off the
**formatter** at the same time; see [Formatting](../guide/formatting.md#turning-the-formatter-off)
for the layout-only `% meaning-format` spellings.
