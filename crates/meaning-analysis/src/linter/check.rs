...
//! stamp paths, and sort. The single entry point both `meaning lint` (CLI) and
...
/// `\bea`/`\eea` pair is an environment here exactly as it is under `meaning
...
mod tests {
    ...
    fn node_directive_suppresses_following_command() {
        let out = lint("% meaning-lint skip deprecated-command: legacy\n{\\bf x}\n");
    ...
    fn node_directive_only_targets_named_rule() {
        ...
        let out = lint("% meaning-lint skip duplicate-label: nope\n{\\bf x}\n");
    ...
    fn node_directive_suppresses_construct_with_arguments() {
        ...
        let out = lint(
            "\\usepackage{amsmath}\n\
             % meaning-lint skip duplicate-package: legacy, loaded twice on purpose\n\
             \\usepackage{amsmath}\n",
        );
        assert!(out.is_empty(), "expected suppression, got: {out:?}");
    }
    ...
    fn node_directive_suppresses_diagnostic_inside_argument() {
        ...
        // `\cline` control word).
        let out =
            lint("% meaning-lint skip dash-length: interval, not a numeric range\n\\cline{2-7}\n");
        assert!(out.is_empty(), "expected suppression, got: {out:?}");
    }

    #[test]
    fn file_directive_suppresses_all_occurrences() {
        let out = lint("% meaning-lint skip-file deprecated-command: legacy\n{\\bf x}\n{\\it y}\n");
        assert!(
            out.is_empty(),
