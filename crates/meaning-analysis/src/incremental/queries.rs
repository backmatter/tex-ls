...
pub fn scope_signatures(db: &dyn IncrementalDb, file: SourceInput) -> SignatureDb {
    ...
    let project = workspace_project(db, context);
    let graph = package_graph(db, context);
    // Map each member's path back to its tracked input, to fetch its scan.
    let by_path: HashMap<&Path, SourceInput> = project
        .members
        .iter()
        .map(|member| {
            (
                member.path.as_path(),
                db.source_input(member.file, *file.project(db)),
            )
        })
        .collect();
...
pub fn bib_lint_findings(
    ...
    crate::bib::linter::lint_document(
        file.path(db),
