#!/usr/bin/env bash
#
# Benchmark meaning's formatter speed against other LaTeX formatters
...
#   MEANING_BENCH_INPUT=path/to/file.tex ./benches/compare_format.sh
...
# chktex is regex-driven, meaning lint does a full CST parse + rule set).
#
...
MEANING="$REPO_ROOT/target/release/meaning"
...
MEANING_VER=$("$MEANING" --version | awk '{print $2}')
...
declare -a CORPUS=()
if [ -n "${MEANING_BENCH_INPUT:-}" ]; then
    [ -f "$MEANING_BENCH_INPUT" ] || { echo "error: MEANING_BENCH_INPUT='$MEANING_BENCH_INPUT' is not a file" >&2; exit 1; }
    CORPUS+=("override|$MEANING_BENCH_INPUT|$(basename "$MEANING_BENCH_INPUT")|10")
else
...
cmd_for() {
    local tool="$1" file="$2"
    case "$tool" in
        meaning)     echo "$MEANING format --no-config --stdin-filepath bench.tex < '$file'" ;;
...
# Findings make chktex exit 2 and meaning exit 1 — that is not a run error, and
...
lint_cmd_for() {
    local tool="$1" file="$2"
    case "$tool" in
        meaning) echo "$MEANING lint --no-config '$file'" ;;
...
# full formatting work, nothing written). Only meaning and tex-fmt support this;
# latexindent has no recursive directory mode and is excluded from the folder
# comparison.
dir_cmd_for() {
    local tool="$1" dir="$2"
    case "$tool" in
        meaning) echo "$MEANING format --no-config --check '$dir'" ;;
...
for entry in "${CORPUS[@]}"; do
    ...
    # Sanity gate: meaning must format the doc (it refuses input with parser
    # diagnostics). Skip any doc it cannot handle yet, with a note.
    if ! "$MEANING" format --no-config --stdin-filepath bench.tex < "$file" >/dev/null 2>&1; then
        log "⚠️  skip $label — meaning cannot format it yet (parser diagnostics)"
        continue
    fi
...
# A recursive `--check` over a real multi-file project (meaning vs tex-fmt only;
...
# `*.tex`, and `meaning format` is `.tex`-only while tex-fmt would also touch
# `.bib`/`.cls`, so staging just the `.tex` fragments keeps the compared set
# identical. Any file meaning cannot format yet is dropped from *both* tools via
# a generated `.ignore` (both honor it), so the comparison stays symmetric.

if [ -z "${MEANING_BENCH_INPUT:-}" ] && [ -d "$PROJECT_SRC" ]; then
    ...
    while IFS= read -r f; do
        if "$MEANING" format --no-config --stdin-filepath bench.tex < "$f" >/dev/null 2>&1; then
    ...
    if [ "$proj_files" -eq 0 ]; then
        log "⚠️  skip project — meaning cannot format any of its files yet"
    else
        label="project ($proj_files files)"
        [ "$proj_excluded" -gt 0 ] && \
            log "   ($proj_excluded file(s) excluded from both tools — meaning cannot format them yet)"
        ...
        for tool in meaning tex-fmt; do
...
mkdir -p "$(dirname "$JSON_OUT")"
{
        ...
        printf '    "min_runs": %d,\n' "$HYPERFINE_MIN_RUNS"
        printf '    "tools": {\n'
        printf '      "meaning": {"version": "%s"}' "$(json_escape "$MEANING_VER")"
