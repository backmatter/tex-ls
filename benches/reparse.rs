...
//! TEX_LS_BENCH_ASSERT=1 cargo bench --bench reparse                              # numbers + contracts
//! TEX_LS_BENCH_ASSERT=1 cargo bench --bench reparse   # the gate alone
//! ```
//!
...
//!
//! Env knobs:
//!   - `TEX_LS_BENCH_TARGET_MS` — per-measurement budget (default 500 ms).
//!   - `TEX_LS_BENCH_CASE` — run only cases whose id contains this substring, for
...
//!   - `TEX_LS_BENCH_ASSERT=1` — check every case against its contract and exit
