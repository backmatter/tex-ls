...
//! MEANING_BENCH_ASSERT=1 cargo bench --bench reparse   # the gate alone
//! ```
//!
...
//!
//! Env knobs:
//!   - `MEANING_BENCH_TARGET_MS` — per-measurement budget (default 500 ms).
//!   - `MEANING_BENCH_CASE` — run only cases whose id contains this substring, for
...
//!   - `MEANING_BENCH_ASSERT=1` — check every case against its contract and exit
...
/// What a case claims, checked under `MEANING_BENCH_ASSERT=1`.
///
...
struct Case {
    ...
    site: Site,
    edit: CaseEdit,
    config: meaning_parser::parser::LexConfig,
    expect: Expect,
}
...
fn cases() -> Vec<Case> {
    ...
    cases.push(Case {
        ...
        edit: CaseEdit::Site,
        config: meaning_parser::parser::LexConfig {
            flavor: meaning_parser::parser::LatexFlavor::Document,
            dtx: true,
        },
...
fn main() {
    println!("meaning incremental reparse bench (parser::reparse, timed directly)");
    ...
    let filter = env::var("MEANING_BENCH_CASE").ok();
    if assert_mode && filter.is_some() {
        eprintln!(
            "MEANING_BENCH_ASSERT=1 cannot run with MEANING_BENCH_CASE: a gate that measures \
    ...
    if assert_mode {
        ...
        println!(
            "\n{} problem(s) found (not fatal without MEANING_BENCH_ASSERT=1):",
            failures.len()
        );
        for failure in &failures {
            println!("  {failure}");
