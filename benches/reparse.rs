//! Measure incremental reparsing directly and report the selected tier.
//!
//! Each case declares an expected [`ReparseTier`] or a decline, with a speedup
//! floor or fallback-cost budget. Every case checks full-parse equivalence
//! before timing because the parser's debug oracle is disabled in release builds.
//! Edit sites are shared with `benches/keystroke.rs`.
//!
//! ```sh
//! cargo bench --bench reparse
//! TEX_LS_BENCH_ASSERT=1 cargo bench --bench reparse
//! ```
//!
//! Run on an idle machine. Results are medians of nine timing blocks.
//! These timings exclude text splicing and Salsa work; use `keystroke` for
//! end-to-end edit measurements.
//!
//! Optional environment variables:
//!
//! - `TEX_LS_BENCH_TARGET_MS`: timing budget per measurement, default 500 ms.
//! - `TEX_LS_BENCH_CASE`: case ID substring to select. Assertions require all cases.
...
/// What a case claims, checked under `TEX_LS_BENCH_ASSERT=1`.
///
...
/// for no reason.
///
/// These executable thresholds are the source of truth; read the gate output.
///
...
struct Case {
    ...
    site: Site,
    edit: CaseEdit,
    config: tex_ls_parser::parser::LexConfig,
    expect: Expect,
}
...
fn cases() -> Vec<Case> {
    ...
    cases.push(Case {
        ...
        edit: CaseEdit::Site,
        config: tex_ls_parser::parser::LexConfig {
            flavor: tex_ls_parser::parser::LatexFlavor::Document,
            dtx: true,
        },
...
fn main() {
    println!("tex-ls incremental reparse bench (parser::reparse, timed directly)");
    ...
    let filter = env::var("TEX_LS_BENCH_CASE").ok();
    if assert_mode && filter.is_some() {
        eprintln!(
            "TEX_LS_BENCH_ASSERT=1 cannot run with TEX_LS_BENCH_CASE: a gate that measures \
    ...
    for case in &cases {
        let Some(text) = load_case_document(case) else {
            println!(
                "  {:<40} {} not found — run `bash benches/documents/download.sh`, skipping",
    ...
    if assert_mode {
        ...
        println!(
            "\n{} problem(s) found (not fatal without TEX_LS_BENCH_ASSERT=1):",
            failures.len()
        );
        for failure in &failures {
            println!("  {failure}");
