...
The CLI script answers "how fast is the `meaning` binary"; the formatting bench
...
Meaning and TexLab editor sessions—whole-process-tree RSS and PSS—over a
complete pinned thesis workspace. The in-process harness asks whether Meaning
...
MEANING_BENCH_OUTPUT_JSON=benches/micro_results.json cargo bench --bench formatting
...
```bash
task bench:reparse
MEANING_BENCH_CASE=phd_dissertation.tex/word cargo bench --bench reparse
```
...
counts auto-calibrate to `MEANING_BENCH_TARGET_MS` (500 ms per row) because the
...
```bash
MEANING_BENCH_DOC=phd_dissertation.tex cargo bench --bench keystroke
MEANING_BENCH_OUTPUT_JSON=/tmp/head.json cargo bench --bench keystroke
```

## The external LSP harness

`benches/compare_lsp_memory.sh` starts three fresh sessions each of
`meaning lsp` and `texlab run`. Both see the complete pinned
...
TexLab and Meaning analyze different things, and both latency and RSS are
...
MEANING_MEMORY_SCENARIO=query-log \
MEANING_MEMORY_QUERY_GENERATIONS=1000 \
MEANING_MEMORY_OUTPUT_JSON=/tmp/meaning-memory.json \
  cargo bench --bench lsp_memory
```

`MEANING_MEMORY_SCENARIO` accepts `all`, `query-log`, or `project`.
`MEANING_MEMORY_PROJECT_GENERATIONS` overrides the default 48-directory project,
and `MEANING_MEMORY_ASSERT=1` enables the gates.
...
MEANING_BENCH_DOC=masters_dissertation.tex MEANING_BENCH_ITERATIONS=60 \
...
- `MEANING_BENCH_DOC`: profile only this document under `benches/documents/`.
- `MEANING_BENCH_ITERATIONS`: iteration count for the selected document (10).
- `MEANING_BENCH_OUTPUT_JSON`: write a machine-readable report to this path.
...
A bare `meaning --version` is only \~0.8 ms, so the extra \~3.7 ms of the format
