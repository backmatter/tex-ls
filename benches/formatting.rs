...
//!
//! Measures library entry points without process startup, in three stages:
...
//!     `tex-ls format` invocation does per byte.
//!
...
//!
//! ```bash
//! TEX_LS_BENCH_DOC=masters_dissertation.tex TEX_LS_BENCH_ITERATIONS=20 \
//!     cargo flamegraph --bench formatting
//! ```
//!
//! Env knobs:
//!   - `TEX_LS_BENCH_DOC` — profile only this doc under `benches/documents/`.
//!   - `TEX_LS_BENCH_ITERATIONS` — iteration count for the selected doc (10).
//!   - `TEX_LS_BENCH_OUTPUT_JSON` — write a machine-readable report to this path.
...
use tex_ls_formatter::formatter::{FormatStyle, format_node, format_with_style_flavored};
use tex_ls_parser::parser::{LatexFlavor, parse_with_flavor};

/// Format flavor/style the CLI uses for a `.tex` document (`format -`):
...
fn run_benchmark(name: &str, input: &str, iterations: usize) -> Option<BenchmarkResult> {
    ...
    // Sanity gate: only documents that format
    ...
    match format_with_style_flavored(input, style(), LatexFlavor::Document) {
        Ok(_) => {}
        Err(e) => {
            println!("⚠️  skipped — tex-ls cannot format it yet ({e})");
            return None;
        }
    }
...
fn report_signature_db_init() {
    let start = Instant::now();
    black_box(tex_ls_parser::semantic::signature::builtin());
    let builtin = start.elapsed();
    let start = Instant::now();
    black_box(tex_ls_parser::semantic::signature::cwl());
...
fn main() {
    println!("tex-ls formatter micro-benchmarks (in-process, no startup floor)");
    ...
    // `cargo flamegraph --bench formatting`.
    if let Ok(doc_name) = env::var("TEX_LS_BENCH_DOC") {
        let iterations = env::var("TEX_LS_BENCH_ITERATIONS")
            .ok()
            .and_then(|raw| raw.parse::<usize>().ok())
            .unwrap_or(10);
        let doc = load_document(&doc_name).unwrap_or_else(|| {
            panic!("TEX_LS_BENCH_DOC '{doc_name}' not found under benches/documents/")
        });
...
fn maybe_write_json_report(results: Vec<BenchmarkResult>) {
    let Ok(path) = env::var("TEX_LS_BENCH_OUTPUT_JSON") else {
        return;
    };
