...
//!     `meaning format` invocation does per byte.
//!
...
//!
//! ```bash
//! MEANING_BENCH_DOC=masters_dissertation.tex MEANING_BENCH_ITERATIONS=20 \
//!     cargo flamegraph --bench formatting
//! ```
//!
//! Env knobs:
//!   - `MEANING_BENCH_DOC` — profile only this doc under `benches/documents/`.
//!   - `MEANING_BENCH_ITERATIONS` — iteration count for the selected doc (10).
//!   - `MEANING_BENCH_OUTPUT_JSON` — write a machine-readable report to this path.

use meaning_formatter::formatter::{FormatStyle, format_node, format_with_style_flavored};
use meaning_parser::parser::{LatexFlavor, parse_with_flavor};
use serde::Serialize;
use std::env;
use std::fs;
...
fn run_benchmark(name: &str, input: &str, iterations: usize) -> Option<BenchmarkResult> {
    ...
    match format_with_style_flavored(input, style(), LatexFlavor::Document) {
        Ok(_) => {}
        Err(e) => {
            println!("⚠️  skipped — meaning cannot format it yet ({e})");
            return None;
        }
    }
...
fn report_signature_db_init() {
    let start = Instant::now();
    black_box(meaning_parser::semantic::signature::builtin());
    let builtin = start.elapsed();
    let start = Instant::now();
    black_box(meaning_parser::semantic::signature::cwl());
...
fn main() {
    println!("meaning formatter micro-benchmarks (in-process, no startup floor)");
    ...
    // `cargo flamegraph --bench formatting`.
    if let Ok(doc_name) = env::var("MEANING_BENCH_DOC") {
        let iterations = env::var("MEANING_BENCH_ITERATIONS")
            .ok()
            .and_then(|raw| raw.parse::<usize>().ok())
            .unwrap_or(10);
        let doc = load_document(&doc_name).unwrap_or_else(|| {
            panic!("MEANING_BENCH_DOC '{doc_name}' not found under benches/documents/")
        });
...
fn maybe_write_json_report(results: Vec<BenchmarkResult>) {
    let Ok(path) = env::var("MEANING_BENCH_OUTPUT_JSON") else {
        return;
    };
