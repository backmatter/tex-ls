...
impl Client {
    fn start() -> Self {
        ...
        let thread = std::thread::spawn(move || meaning::lsp::serve(server).unwrap());
        let mut client = Self {
            connection,
            server: Some(thread),
            next_id: 1,
...
fn main() {
    let scenario = std::env::var("MEANING_MEMORY_SCENARIO").unwrap_or_else(|_| "all".to_owned());
    let query_generations = env_usize("MEANING_MEMORY_QUERY_GENERATIONS", 10_000);
    let project_generations = env_usize("MEANING_MEMORY_PROJECT_GENERATIONS", 48);
    let assert = std::env::var_os("MEANING_MEMORY_ASSERT").is_some();
    assert!(
        matches!(scenario.as_str(), "all" | "query-log" | "project"),
        "MEANING_MEMORY_SCENARIO must be all, query-log, or project"
    );
    ...
    if let Some(path) = std::env::var_os("MEANING_MEMORY_OUTPUT_JSON") {
        fs::write(
            path,
