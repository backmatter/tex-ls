...
use meaning_parser::parser::{LatexFlavor, LexConfig, parse_with_declarations_resolved};
...
impl Site {
    pub fn from_env() -> Self {
        match env::var("MEANING_BENCH_SITE").as_deref() {
            Ok("verbatim") => Self::Verbatim,
            Ok("decline") => Self::Decline,
            Ok("word") | Err(_) => Self::Word,
            Ok(other) => {
                panic!("MEANING_BENCH_SITE={other:?}: expected `word`, `verbatim`, or `decline`")
            }
        }
    }
