...
//!
//! Env knobs:
//!   - `MEANING_BENCH_DOC` — bench only this doc under `benches/documents/`.
//!   - `MEANING_BENCH_SITE` — which keystroke to measure: `word` (the default),
//!     `verbatim`, or `decline` (see [`Site`]).
//!   - `MEANING_BENCH_TARGET_MS` — per-row timing budget (default 500 ms); each
//!     row auto-calibrates its iteration count to fill it.
//!   - `MEANING_BENCH_OUTPUT_JSON` — write a machine-readable report to this
//!     path (the A/B diff between two builds).
//!   - `MEANING_BENCH_ASSERT=1` — check every row against its declared contract
...
//! MEANING_BENCH_ASSERT=1 cargo bench --bench keystroke
//! ```
//!
...
/// buffer's [`LineIndex`](meaning_analysis::text::LineIndex) is built in.
...
fn handoff(live: &TextBuffer) -> Arc<meaning_analysis::text::SourceText> {
    live.text_arc()
}
...
/// ([`TextBuffer::with_replacement`](meaning_analysis::text::TextBuffer::with_replacement))
...
fn main() {
    println!("meaning keystroke pipeline bench (didChange -> upsert -> parse)");

    let target = Duration::from_millis(
        env::var("MEANING_BENCH_TARGET_MS")
            .ok()
    ...
    let assert_mode = matches!(
        env::var("MEANING_BENCH_ASSERT").as_deref(),
        Ok("1") | Ok("true")
    );
    ...
    let single = env::var("MEANING_BENCH_DOC").ok();
    if assert_mode && single.is_some() {
        eprintln!(
            "MEANING_BENCH_ASSERT=1 cannot run with MEANING_BENCH_DOC: every contract is a \
    ...
    if assert_mode {
        ...
        if !missing.is_empty() {
            eprintln!("MEANING_BENCH_ASSERT=1 needs the pinned corpus:");
    ...
    // worth keeping.
    if let Ok(path) = env::var("MEANING_BENCH_OUTPUT_JSON") {
        let report = Report {
            schema_version: 4,
