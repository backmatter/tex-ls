...
//!
//! `benches/formatting.rs` times the per-byte formatter work; this bench covers
...
//! TEX_LS_BENCH_DOC=phd_dissertation.tex cargo bench --bench keystroke
//! ```
//!
//! Env knobs:
//!   - `TEX_LS_BENCH_DOC` — bench only this doc under `benches/documents/`.
//!   - `TEX_LS_BENCH_SITE` — which keystroke to measure: `word` (the default),
//!     `verbatim`, or `decline` (see [`Site`]).
//!   - `TEX_LS_BENCH_TARGET_MS` — per-row timing budget (default 500 ms); each
//!     row auto-calibrates its iteration count to fill it.
//!   - `TEX_LS_BENCH_OUTPUT_JSON` — write a machine-readable report to this
//!     path (the A/B diff between two builds).
//!   - `TEX_LS_BENCH_ASSERT=1` — check every row against its declared contract
...
fn bench_document(name: &str, text: &str, target: Duration, site: Site) -> DocumentResult {
    ...
    // been measured back to a bench-layout artifact rather than a cost. The argument for row 0 is unaffected; the
...
/// turned out not to be a cost at all — a bench-layout artifact. The row still wants a gate; that number is not
...
/// ([`TextBuffer::with_replacement`](tex_ls_analysis::text::TextBuffer::with_replacement))
