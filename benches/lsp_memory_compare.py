...
def run_session(
    *,
    ...
    env = os.environ.copy()
    if key == "meaning":
        config = scratch_dir / f"meaning-{run_number}.toml"
        config.write_text("")
        env["MEANING_CONFIG"] = str(config)
    ...
    try:
        initialized = client.request(
            ...
            {
                "processId": os.getpid(),
                "clientInfo": {"name": "meaning-lsp-benchmark", "version": "2"},
        ...
        result["request_latencies"] = [
            ...
            benchmark_requests(
                ...
                params=[{**navigation_params, "newName": "meaningBenchmarkAup91"}],
                runs=latency_runs,
                warmups=latency_warmups,
                timeout=settle_timeout,
                result_unit="edit",
...
def add_relative_memory(records: list[dict]) -> None:
    meaning = next(record for record in records if record["key"] == "meaning")
    baseline = meaning["summary"]["settled_rss_mb"]
    for record in records:
        record["summary"]["relative_to_meaning"] = round(
...
def main() -> None:
    ...
    parser.add_argument("--meaning-version", required=True)
    ...
    records = []
    labels = {"meaning": "Meaning", "texlab": "TexLab"}
    ...
    output = {
        ...
        "host": host_metadata(),
        "versions": {"meaning": args.meaning_version, "texlab": args.texlab_version},
        "corpus": {
