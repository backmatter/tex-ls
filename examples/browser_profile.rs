//! Profile Citet's JSON fixture through native stages shared with the browser.
//! cargo run --release --example browser_profile -- /tmp/meaning-large-fixture.json
//!
...
fn main() {
    ...
    for i in 0..101 {
        ...
        let start = Instant::now();
        let ctx = meaning_analysis::completion::classify_context_with_declarations(
            &root,
            text.len() - 1,
        ...
        let start = Instant::now();
        std::hint::black_box(meaning_analysis::completion::candidates_with_declarations(
            &ctx,
            sigs,
            model,
            snapshot.declarations(),
