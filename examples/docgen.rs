...
//! snapshot tests pin ([`meaning_analysis::linter::docs::render_reference_page`] and
//! [`meaning_analysis::bib::linter::docs::render_reference_page`]) and writes it to the
//! mdBook source tree. Living as an `examples/` target (not a `[[bin]]`) keeps
//! meaning a single, publishable crate.
...
fn main() -> io::Result<()> {
    write_if_changed(
        ...
        &meaning_analysis::linter::docs::render_reference_page(),
    )?;
    write_if_changed(
        Path::new("docs/src/reference/bib-linter-rules.md"),
        &meaning_analysis::bib::linter::docs::render_reference_page(),
    )
}
