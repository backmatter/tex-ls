...
: "${MEANING_BIN:?MEANING_BIN must name the meaning executable}"
: "${MEANING_SHA:?MEANING_SHA must be set}"
: "${MEANING_VERSION:?MEANING_VERSION must be set}"
...
if [ ! -f "$FAILURES_TSV" ]; then
  printf 'source\tfailure_type\tfile\tlog_path\treport_path\tsource_revision\tmeaning_sha\tmeaning_version\tidempotency_input_path\tidempotency_once_path\tidempotency_twice_path\n' > "$FAILURES_TSV"
fi
...
scan_file() {
  ...
  status=0
  (cd "$PROJECT_DIR" && timeout 120 "$MEANING_BIN" debug format --checks all "${config_args[@]}" --dump-dir "$pass_dir" "$rel_file") > "$log_path" 2>&1 || status=$?
  if [ "$status" -eq 0 ]; then
    return
  fi

  if [ "$CONFIG_MODE" = project ] && grep -Fq 'meaning.toml' "$log_path"; then
    config_args=(--no-config)
    retry_status=0
    (cd "$PROJECT_DIR" && timeout 120 "$MEANING_BIN" debug format --checks all "${config_args[@]}" --dump-dir "$pass_dir" "$rel_file") > "$log_path" 2>&1 || retry_status=$?
    if [ "$retry_status" -eq 0 ]; then
      echo "notice: $SOURCE has an invalid meaning config; $rel_file passed with --no-config"
      return
    fi
    status=$retry_status
  fi

  (cd "$PROJECT_DIR" && timeout 120 "$MEANING_BIN" debug format --checks all "${config_args[@]}" --report "$rel_file") > "$report_path" 2>&1 || true
  matched=0
  failure_types=""
  ...
  if [ -n "$failure_types" ]; then
    ...
    while IFS= read -r failure_type; do
      ...
      printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
        ...
        "$SOURCE_REVISION" "$MEANING_SHA" "$MEANING_VERSION" \
