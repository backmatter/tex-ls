...
# Env:   MEANING=/path/to/meaning   (default: target/release/meaning)
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
MEANING="${MEANING:-${REPO_ROOT}/target/release/meaning}"
...
if [ ! -x "${MEANING}" ]; then
  echo "error: ${MEANING} not found or not executable — build it first:" >&2
...
for corpus in "${CORPORA[@]}"; do
  ...
  report="$(cd "${CORPORA_DIR}/${corpus}" \
    && "${MEANING}" --no-config debug format --checks trivia-strict --report . || true)"
