...
let project;
if (fixture.nativeConfig) {
  project = mkdtempSync(join(dirname(resolve(output)), 'settings-project-'));
  writeFileSync(join(project, 'meaning.toml'), fixture.nativeConfig);
  fixture = JSON.parse(JSON.stringify(fixture).replaceAll('file:///architecture-settings', pathToFileURL(project).href));
}
...
const samples = [];
try {
  await request('initialize', fixture.initialize ?? { capabilities: {}, initializationOptions: { build: { onSave: false } } });
  send({ method: 'initialized', params: {} });
  session.dispatch('initialize', JSON.stringify(fixture.initialize ?? {}));
  if (fixture.settings) {
    assert.deepEqual(JSON.parse(session.dispatch('meaning/updateSettings', JSON.stringify({ settings: fixture.settings }))), { applied: true });
  }
  for (const event of events) {
    ...
    if (event.request) {
      ...
      assert.deepEqual(wasm, native, event.method);
