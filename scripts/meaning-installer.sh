...
REPO="${MEANING_REPO:-backmatter/meaning}"
INSTALL_DIR="${MEANING_INSTALL_DIR:-$HOME/.local/bin}"
TAG="${MEANING_TAG:-}"
VERIFY="${MEANING_VERIFY_CHECKSUM:-true}"
...
detect_libc() {
  if [ -n "${MEANING_LIBC:-}" ]; then
    case "$MEANING_LIBC" in
    gnu | musl) printf '%s\n' "$MEANING_LIBC" ;;
    *)
      echo "MEANING_LIBC must be 'gnu' or 'musl', got '$MEANING_LIBC'" >&2
      exit 1
      ;;
    esac
    return 0
...
resolve_download_url() {
  if [ -n "$TAG" ]; then
    ...
    v* | meaning-v*)
      ...
      ;;
    *)
      tag_candidates="v${TAG} meaning-v${TAG}"
      ;;
    esac
    ...
    echo "Could not find release asset ${asset} for MEANING_TAG='${TAG}' in ${REPO}" >&2
    exit 1
  fi
...
mkdir -p "$INSTALL_DIR"
install -m 755 "$tmpdir/meaning" "$INSTALL_DIR/meaning"

echo "Installed meaning to $INSTALL_DIR/meaning"
case ":$PATH:" in
*":$INSTALL_DIR:"*) ;;
*)
