...
This script checks the pinned BibLaTeX 3.21 model. It does
...
With no ``--def``, fetch the pinned model. Local inputs must match its SHA-256.
"""
...
DATA_FILE = Path(__file__).resolve().parent.parent / "crates" / "tex-ls-parser" / "data" / "bib_fields.json"
...
MODEL_URL = "https://raw.githubusercontent.com/plk/biblatex/v3.21/tex/latex/biblatex/blx-dm.def"
MODEL_SHA256 = "caa6b7c0c6db33402fd93d00c01e9f99717f5e80fdf8be2a841c4a987771b115"
...
def _load_model(def_path: str | None) -> DataModel:
    if def_path is None:
        ...
        content = Path(def_path).read_bytes()
    if hashlib.sha256(content).hexdigest() != MODEL_SHA256:
        sys.exit("error: blx-dm.def does not match the pinned BibLaTeX 3.21 model")
    return DataModel(content.decode("utf-8"))
...
def main() -> int:
    ...
    doc = json.loads(DATA_FILE.read_text(encoding="utf-8"), object_pairs_hook=OrderedDict)

    if args.write:
        _apply(model, doc)
        DATA_FILE.write_text(_dump(doc), encoding="utf-8")
        ...
        print(f"wrote {DATA_FILE.relative_to(Path(__file__).resolve().parent.parent)}")
        return 0
