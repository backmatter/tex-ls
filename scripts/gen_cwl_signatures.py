...
arity** — exactly the bulk tier tex-ls wants *underneath* its hand-curated
...
DATA_FILE = Path(__file__).resolve().parent.parent / "crates" / "tex-ls-parser" / "data" / "cwl_signatures.json"
...
# add a basename here and `python3 scripts/gen_cwl_signatures.py --write`. (Document *classes* are a separate axis,
...
def _parse_arg_shape(rest: str) -> list:
    ...
    *classification* suffixes — behaviour claims this bulk tier deliberately refuses
    ...
    through ``bash scripts/check_typeset_stability.sh``."""
    args: list = []
    i = 0
    while i < len(rest):
        c = rest[i]
...
def build_db(files: dict[str, str], report: dict[str, set[str]] | None = None) -> OrderedDict:
    ...
    command_packages: dict[str, set[str]] = defaultdict(set)
    environment_packages: dict[str, set[str]] = defaultdict(set)
    ...
    for filename, text in files.items():
        ...
        for raw in text.splitlines():
            ...
            provenance = command_packages if entry.kind == "command" else environment_packages
            provenance[entry.name].add(Path(filename).stem)
    ...
    return OrderedDict(
        [
            (
                ...
                "`python3 scripts/gen_cwl_signatures.py --write` to regenerate (edit ALLOWLIST in the script to widen).",
            ),
            ("commands", emit(commands)),
            ("environments", emit(environments)),
            ("commandPackages", {name: sorted(command_packages[name]) for name in sorted(command_packages)}),
            ("environmentPackages", {name: sorted(environment_packages[name]) for name in sorted(environment_packages)}),
        ]
    )
...
def main() -> int:
    ...
    if args.write:
        ...
        print(f"wrote {DATA_FILE.relative_to(Path(__file__).resolve().parent.parent)} ({n_cmd} commands, {n_env} environments)")
        return 0

    current = DATA_FILE.read_text(encoding="utf-8") if DATA_FILE.is_file() else ""
    if current == rendered:
