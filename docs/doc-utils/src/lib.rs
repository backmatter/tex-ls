...
/// Substitute the `{{ meaning-version }}` marker with the crate version read from
...
fn insert_version(book: &mut Book) {
    ...
    const MARKER: &str = "{{ meaning-version }}";
...
struct Benchmarks {
    ...
    /// Linter timings (meaning lint vs lacheck/chktex), same row shape as
...
/// the meaning baseline, and the numbers the tooltip shows. Serialized inline
...
const FORMAT_CAPTION: &str = "Formatting speed relative to <code>meaning</code>. Each dot is one \
     document formatted by one tool; the vertical position is mean wall-clock time as a \
     ratio to <code>meaning</code> on a log scale, so <code>meaning</code> lies on the \
...
/// (`{{ benchmark-project-results }}`). Only `meaning` and `tex-fmt` appear
...
const FORMAT_PROJECT_CAPTION: &str =
    "Whole-project formatting speed relative to <code>meaning</code>. Each dot is one tool \
     running a recursive <code>--check</code> over a real multi-file LaTeX project; the \
     vertical position is mean wall-clock time as a ratio to <code>meaning</code> on a log \
     scale, so <code>meaning</code> lies on the dashed baseline at 1, faster tools fall below \
...
const LINT_CAPTION: &str =
    "Linting speed relative to <code>meaning</code>. Each dot is one document checked \
     by one tool; the vertical position is mean wall-clock time as a ratio to \
     <code>meaning</code> on a log scale, so <code>meaning</code> lies on the dashed \
...
/// The linter analog of `render_meta`: meaning/lacheck/chktex versions plus the
...
/// plots time-relative-to-meaning on a log axis, one dot per (document,
...
/// document's meaning baseline (meaning itself is `1.0`), in corpus order.
...
/// follow the order tools appear in `results`. `meaning` is the baseline and
...
fn render_results_tables_html(documents: &[Document], results: &[BenchResult]) -> String {
    ...
    for doc in documents {
        ...
        for r in results.iter().filter(|r| r.document == doc.id) {
            let relative = if r.formatter == "meaning" {
                "baseline".to_string()
            } else {
...
/// Human ratio of a tool's mean to the meaning baseline.
