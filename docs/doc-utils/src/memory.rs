...
fn render_results(benchmarks: &MemoryBenchmarks) -> String {
    ...
    for server in &benchmarks.servers {
        let summary = &server.summary;
        let relative = if (summary.relative_to_meaning - 1.0).abs() < f64::EPSILON {
            "baseline".to_string()
        } else {
            format!("{:.2}×", summary.relative_to_meaning)
        };
        output.push_str(&format!(
            "| {} | {:.1} MB | {:.1} MB | {:.1} MB | {:.1} MB | {} |\n",
            escape_table_cell(&server.label),
...
mod tests {
    ...
    fn fixture() -> MemoryBenchmarks {
        serde_json::from_str(
            r#"{
              ...
              "versions":{"meaning":"0.17.0","texlab":"5.26.0"},
              ...
              "session":{"runs_per_server":3,"sample_interval_seconds":0.15,"quiet_seconds":5.0,"settle_timeout_seconds":60.0,
                         ...
                         "operations":[]},
              "servers":[
                {"label":"Meaning","summary":{"baseline_rss_mb":18.0,"settled_rss_mb":25.0,"settled_pss_mb":22.0,"peak_rss_mb":27.0,"peak_pss_mb":24.0,"settled_seconds":12.0,"relative_to_meaning":1.0,
                ...
                {"label":"TexLab","summary":{"baseline_rss_mb":22.0,"settled_rss_mb":30.0,"settled_pss_mb":27.0,"peak_rss_mb":31.0,"peak_pss_mb":28.0,"settled_seconds":13.0,"relative_to_meaning":1.2,
    ...
    fn renders_versions_corpus_and_protocol_parameters() {
        ...
        assert!(rendered.contains("**Meaning**: `0.17.0`"));
    ...
    #[test]
    fn renders_memory_table_relative_to_meaning() {
        let rendered = render_results(&fixture());
        assert!(rendered.contains("| Meaning | 18.0 MB | 25.0 MB | 22.0 MB | 27.0 MB | baseline |"));
    ...
    fn renders_readiness_latency_and_returned_work() {
        ...
        assert!(rendered.contains("| Meaning | 20 ms | 80 ms | 120 ms |"));
        assert!(
            rendered.contains(
                "| Meaning | Go to definition | 0.30 ms | 0.45 ms | 1 location in 1 file, 180 B |"
            ),
            "{rendered}"
        );
        assert!(
            rendered.contains(
                "| Meaning | Find references | 4.20 ms | 5.10 ms | 2 locations in 2 files, 16 KiB |"
            ),
            "{rendered}"
        );
