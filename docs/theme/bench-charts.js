...
//
// Chart: x = tool, y = time relative to meaning (log scale, baseline = 1),
...
(function () {
  ...
  // legend read meaning -> tex-fmt -> latexindent rather than alphabetized.
  ...
  function spec(points) {
    ...
    return {
      ...
      description:
        "Dot plot of formatting speed relative to meaning. Each dot is one " +
        "document formatted by one tool; the vertical axis is mean time as a " +
        "ratio to meaning on a log scale, with meaning on a dashed baseline " +
      ...
      data: { values: points },
      layer: [
        // Baseline at 1.0 (meaning); everything below is faster, above slower.
        {
        ...
        },
        {
          ...
          encoding: {
            ...
            y: {
              field: "ratio",
              type: "quantitative",
              title: "Time relative to meaning",
              scale: { type: "log" },
              axis: {
