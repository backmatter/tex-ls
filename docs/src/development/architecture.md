...
The current crate and host boundaries are defined in [the independent language-service decision](../../../adr/0001-independent-language-service.md). The historical sections below explain the parsing and incremental algorithms; old root-crate paths refer to their extracted owners.

Meaning parses LaTeX into a lossless concrete syntax tree (CST) and puts a
...
This page is a practical tour of Meaning's design. Its goal is to help
...
most of Meaning's safety properties are established at the boundary between
those two components.

## What it does

At a high level, Meaning turns source text into a syntax tree, then uses that
...
The dependency graph separates analysis from every host:
...
meaning-parser
  └── meaning-formatter
        └── meaning-analysis
              └── meaning-protocol
                    ├── meaning (native CLI and LSP)
                    └── meaning-browser (browser session and WASM)
```

Higher layers also depend directly on the lower layers they use. `meaning-wasm` remains a separate formatter binding for the documentation playground. The dependency check in `scripts/check_architecture.py` enforces these directions in CI.

`meaning-analysis` owns `incremental/inputs`, `incremental/queries`, `incremental/database`, `incremental/analysis`, and `incremental/reparse`. The database owns writes; `Analysis` exposes read snapshots. Project facts and lint findings derive from explicit source inputs. Configuration parsing validates supplied text and does not discover configuration files.

`meaning-protocol` groups diagnostics, completion, navigation, formatting, symbols, code actions, structure, synchronization, and conversions into separate modules. Hosts supply file existence, directory entries, installed-package indexes, and auxiliary facts through `HostServices`. There is no ambient filesystem override.

The native host separates state, settings, request dispatch, write scheduling, response computations, synchronization, and inverse search under `src/lsp/`. `src/config.rs`, `src/file_discovery.rs`, `src/package_source.rs`, `src/format/`, `src/texmf.rs`, and `src/auxfile.rs` own operations that read the local environment. Installed-package and auxiliary caches belong to their server session. Auxiliary graphs use an iterative, cycle-aware traversal.

The browser crate owns its source manifest and in-memory session. Citet supplies incremental edits and applies returned edits to its existing Yjs source store. The engine does not introduce another persistent source store. Native integrations are added to native capabilities, rather than advertised by the shared layer and removed by the browser.

The old root-crate parser, formatter, semantic, and analysis re-export modules have been removed. Rust consumers import the crate that owns each concept. This is an intentional API break, recorded in [the ownership decision](../../../adr/0001-independent-language-service.md).
...
Meaning follows biber, consistently with the rest of its BibLaTeX-oriented
...
`.gitignore` and `meaning.toml` excludes.
...
`meaning.toml` is found by walking ancestors from each input. The CLI is its
...
`meaning.toml` candidates examined by the ancestor walk and of any resolved
...
# extra delimiter spellings for an environment meaning already knows
...
shared types live in `meaning-parser` so every consumer can use the same model.
In incremental mode, both are carried through one high-durability salsa input
(`incremental::DeclarationsInput`), so changing `meaning.toml` invalidates the
...
Meaning deliberately separates syntax from semantics. The syntax layer is a
...
Meaning does recognize a bounded, gradually growing set of patterns from static
...
Meaning can infer environment aliases from definitions in the current file. For
...
Curated overrides add kernel aliases and the few cases where Meaning must differ
...
(`crates/meaning-parser/tests/incremental_reparse.rs`) over hand-written hazard
...
(`crates/meaning-parser/tests/reparse_corpus_sweep.rs`,
...
surveying form is `meaning debug format --checks trivia-strict`.
...
`meaning_parser::directives` resolves suppression directives into sorted,
...
1. **Verb defines scope.** `% meaning-format ...`, `% meaning-lint ...`, and
   `% meaning ...` share one grammar, with `skip` / `off` / `on` / `skip-file`
   verbs.
2. **Legacy spellings still work.** `% meaning-ignore ...` is deprecated but
...
is a pure function of the input and data shipped with Meaning; it does not
...
Findings are suppressed inline with `% meaning-lint skip <rule>: <reason>`,
...
copied from fatou's. Meaning treats a bare `\r` as a line break, so whether a
...
Meaning is not a TeX interpreter. It does not expand macros, execute primitives,
...
For the same reason, Meaning does not attempt general `\catcode` evaluation. It
supports only the bounded, statically recognizable patterns listed under
[sanctioned lexer modes](#sanctioned-lexer-modes).

Meaning does not typeset documents. It never runs `latexmk`, `pdflatex`, or any
...
The Backmatter fork exposes `meaning::lsp::embedded::Session` for hosts that own
file discovery, document persistence, and scheduling. `meaning-browser` wraps
that session with wasm-bindgen. It accepts LSP method parameters directly,
without the native stdio loop, watcher, read pool, or build processes.

Native and embedded requests call the same language computations. Opens seed
an in-memory project; ordered UTF-16 `didChange` edits use `apply_content_changes`
and stage parser edits immediately after updating the incremental database.
The host must supply every project source document needed for analysis.

A scoped file manifest supplies directory entries and existence checks during
embedded requests. `meaning/registerFile` adds an asset URI without its bytes.
The scope restores the previous manifest on return or unwinding, so sessions
remain isolated. Native requests outside a scope retain OS file discovery.
Manifest writes happen outside the query scope to avoid cloning the manifest
on every document update. No source text is stored in the manifest.

The browser host currently uses default formatting, build, and rule settings.
It disables installed TEXMF discovery, forward search, and execute-command
integrations. Source analysis does not run a TeX compiler. Browser configuration,
compile-output ingestion, and remaining native integrations need host APIs.

Incremental transport is not zero-copy parsing. `TextBuffer` still creates a new
contiguous string when applying edits, and the incremental database maintains
syntax and semantic state. Measure this cost independently of Yjs transport.

The `browser` build profile uses whole-program LTO and one code generation unit.
Completion demands merged signatures only for command and environment names,
and the per-file semantic model only for labels and colors. Static and path
candidates avoid both queries. Citation and glossary requests retain their
cross-file resolution paths.

For a single staged edit, `reparse_edits` compares the supplied new buffer with
the old prefix, insertion and suffix before entering the unchanged tier ladder.
It avoids constructing intermediate full-file strings. Multiple edits still
replay in order. This changes allocation, not the admitted edits or the debug
full-parse oracle. Command-name edits still fall back to full parsing.

`task bench:browser-profile -- fixture.json` profiles the shared native stages
and a separate embedded completion session. The JSON fixture is an array of
`{path, source}` documents with a `main.tex` member. Row zero contains cold
queries; the remaining 100 rows alternate command-prefix replacements. Browser
visible latency must be measured separately in the embedding editor.

Full parsing skips capture probes for ordinary ASCII prose only when docstrip,
pending lexer modes and short-verbatim delimiters are absent. It still uses the
ordinary token routine. Within a non-statement text block, a word or a single
space/newline token immediately before a word emits the same token event without
command and comment probes. Block termination remains checked first; statement
bodies retain their semicolon handling. Neither shortcut changes tree shape or
extends incremental reparse admission.

### Completion queries and project namespaces

BibTeX completion uses the existing salsa-cached tree when it describes the
captured buffer. Only a value-macro context demands the cached semantic model;
entry types and field names use syntax and static data. A stale, missing or
cancelled snapshot falls back to parsing the captured buffer, with the same
context-dependent model demand. This adds no source store or new cache.

`Analysis::resolve_labels` and `resolve_citations` expose the existing tracked
queries separately. Consumers needing one model no longer demand both through
`resolve_project`; diagnostics that consume both retain the combined method.
In particular, label hover and document symbols no longer force bibliography
analysis merely to resolve a document namespace.

For a current tracked LaTeX buffer, label completion reads the distinct names
from its include-connected label namespace. It filters by prefix and sorts the
result without visiting each definer's tree again. Duplicate definitions produce
one candidate, and unrelated document namespaces stay separate. The stale-buffer
fallback retains local completion from the captured text. File changes and closes
invalidate membership and label facts through the existing salsa queries.


### Shared engine performance follow-up, September 2026

The LaTeX token tier admits an additional conservative control-word case: a
command with no children other than its head in a prose paragraph. Its ancestors
may only be paragraphs, environments and the root; a preceding sibling command
across trivia declines to exclude definition operands. Both spellings
must be lowercase ASCII and inert under the fixed parse context. Lexer pending
states, definition families, conditional flow and operands, aliases, delimiter
policies, verbatim signatures and argument-bearing built-ins all decline. This
excludes math, argument positions and `.dtx`.
The unchanged token sequence therefore cannot alter a scan, pairing or attachment
choice. The existing joins, diagnostic shift, length check and shared full-parse
oracle remain mandatory. This extends Token admission, not the lexer architecture.

BibTeX now has a leaf reparse entry point. Its lexer is context-free and its
parser reads token text only to classify entry-type names. Those positions are
excluded. A WORD or NUMBER must relex to exactly one token of the same kind;
complete bounded neighbours must retain the same token sequence. Parser errors
addressing that token expand or shrink with it, suffix errors shift, and partial
overlaps decline. Every debug splice compares both green tree and errors with a
full parse. The release benchmark repeats that comparison outside the timed
region and requires at least a fivefold speedup for each admitted workload.
Seeded BibTeX corpus edits record exact splice counts with per-file floors in
`tests/bib_reparse_baseline.txt`; LaTeX retains its pinned corpus ratchet.

The BibTeX base lives in the same bounded, hot-aware cache as LaTeX bases and uses
the existing staged edit chain. A single known edit or a locally composed batch can splice; unsupported batches
and unknown transforms full-parse. Eviction, disposal and stale edits preserve the
same fallback contract. Neither host acquires another persistent source store.

Diagnostic pulls and code actions demand label resolution only when the local
semantic model has labels or references, and citation resolution only when it has
citation uses. This is equivalent to eager resolution because the corresponding
rules iterate those local sets before consulting project context. New cross-file
rules must preserve that demand contract or extend it. Package-option extraction
rejects non-`.sty` paths before requesting their syntax and semantics, matching
the pure extractor's existing domain. Rule selection remains a post-filter.

The linter constructs shared mode and conditional indexes on first use. Mode
indexing walks child nodes and covers the token gaps between them as one range:
all direct tokens inherit the same mode, and the existing range coalescer already
merged them. A test compares the complete range partition against the previous
token-by-token algorithm over the checked-in LaTeX corpus. Positional argument
classification and math/unknown/text behavior remain unchanged.


### Touching edit composition and current Rust compiler

Development and release builds pin Rust 1.98.1, including its vtable-generation
correctness fix. Rust 2024 remains the edition and Rust 1.89 remains the supported
minimum, checked separately in CI. Citet reads the compiler pin from this checkout
when building the browser artifact instead of maintaining a second version string.

`coalesce_touching_edits` composes sequential edits when they touch the replacement
produced so far. Independent edits remain separate. There are no batch-size or
replacement-size tuning constants. Only newly touched original bytes join the
replacement; ordinary forward typing reuses its String allocation. Each edit must
fit the intermediate UTF-8 text. Exhaustive two-edit combinations compare accepted
compositions with independent sequential edits, including Unicode, CRLF, deletion
and overlap. This is text algebra before parsing, not a parser tier or lexer restart.

LaTeX tries the composed edit through the existing transform check, tier ladder,
length check and full-parse oracle. If composition or the final splice declines,
it retains individual replay. BibTeX uses the same composition and its existing
leaf proof, with a full-parse fallback. Neither path publishes an intermediate
tree or consumes staged edits early. The release batch benchmark compares exact
green trees and error vectors before timing the old replay and new composition
under the same compiler. Performance floors are regression checks, not runtime
limits.

### Derived lint findings shared by language requests

Following rust-analyzer's demand-driven query architecture, raw lint findings are
now tracked queries keyed by `SourceFile`. Diagnostic pulls and code actions read
the same owned findings and fixes. They no longer rerun the linter independently
for an unchanged analysis. Rule selection and protocol position conversion remain
outside these queries; selecting a different rule subset does not rerun dispatch.

The LaTeX query reads the existing syntax and semantic queries, package options,
and label/citation resolution only when local finding sites require them. The
BibTeX query currently reads only its local syntax and semantic model. Salsa
records these dependencies and invalidates findings after relevant input changes;
range-free project facts can stop unrelated edits from invalidating them. Tests
cover source, label, citation, membership and declaration changes, and verify
unchanged results against the eager linter. Diagnostic values contain no Salsa
handles, so the query's `unsafe(non_salsa_values)` annotation is valid; ordinary
value equality also allows result backdating.

This adds derived state to the existing analysis database, with no separate source
store, TTL, request-count limit or new scheduler. Code actions clone only findings
selected by the caller. CLI linting and cancellation/cache-miss fallbacks retain
the same pure linter. Cold analysis remains synchronous; sharing results avoids
repeat traversals but does not make a traversal preemptible.

Code-action requests demand only their requested kinds. Quick-fix requests read
lint findings; table refactorings use the current snapshot's parsed tree instead
of parsing the full source again. A missing, stale or cancelled snapshot uses the
existing pure parse fallback. The lint-findings request boundary also checks the
captured source against the snapshot before reuse. Tests compare table edits with
the pure implementation and verify that refactoring-only requests demand no lint.

### Citation syntax recovery by source range

Citation hover and completion details already select an entry from the cached
BibTeX model, which carries its complete syntax range. They now recover its node
with Rowan's range descent through `bib::ast::entry_at_range`, rather than walking
every preceding entry and field. This follows rust-analyzer's separation of
semantic facts from recoverable syntax locations. No new cache or index is added.

The caller supplies the model and tree from the same snapshot. The accessor checks
bounds and exact entry kind/range; a range alone does not establish revision
identity. Corpus tests compare every entry with ordinary traversal, including
malformed and Unicode input. Existing case-insensitive key selection and first
bibliography-definer precedence remain unchanged. The release benchmark compares
node identity and measures the last entry of a 20,000-entry bibliography.
