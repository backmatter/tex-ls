...
**Usage:** `meaning [OPTIONS] <COMMAND>`

## Options

`--config <PATH>`
:   Path to a `meaning.toml` to use instead of discovering one. Applies to `format` and `lint`; ignored by `parse`, `lsp`, and `init`

`--no-config`
:   Ignore any `meaning.toml` (project, `$MEANING_CONFIG`, or global) and use built-in defaults
...
**Usage:** `meaning format [OPTIONS] [PATHS]...`
...
`--exclude <PATTERN>`
:   Gitignore-style pattern to skip during directory discovery (repeatable). Added on top of any `exclude`/`extend-exclude` from `meaning.toml`
...
**Usage:** `meaning lint [OPTIONS] [PATHS]...`
...
`--select <RULE>`
:   Run only these rules (repeatable). Overrides `[lint] select` from `meaning.toml` when given

`--ignore <RULE>`
:   Disable these rules (repeatable). Overrides `[lint] ignore` from `meaning.toml` when given
...
`--ipc-dir <DIR>`
:   Directory holding the servers' IPC advertisements. Defaults to `$MEANING_IPC_DIR`, then a per-user directory under the runtime (or temporary) directory

## `meaning init`

Write a commented starter `meaning.toml` to the current directory

**Usage:** `meaning init [OPTIONS]`

### Options

`--force`
:   Overwrite an existing `meaning.toml`
