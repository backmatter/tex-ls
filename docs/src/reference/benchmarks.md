...
Wall-clock speed of `meaning` against comparable tools, measured with
...
- `tex-fmt` breaks overfull lines greedily but does not reflow: it won't rewrap
  lines that already fit, so it moves far less text than `meaning`, which
  reflows each paragraph to the target width.
- Among the linters, `lacheck` is a small classic checker, `chktex` is
  regex-driven, and `meaning lint` does a full CST parse plus its rule set.
...
Each tool is invoked exactly as a user would pipe a document through it:
  ...
  | Tool          | Invocation                                              |
  | ------------- | ------------------------------------------------------- |
  | `meaning`     | `meaning format --no-config --stdin-filepath bench.tex` |
...
`meaning` cannot yet format (parser diagnostics) are skipped, as are comparison
...
The harness starts three fresh processes each of `meaning lsp` and `texlab run`
