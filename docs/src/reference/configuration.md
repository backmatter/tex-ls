...
Meaning is configured through a `meaning.toml` file. All keys are optional and
spelled in kebab-case; an unknown key or section is a hard error, not a silent
no-op. Run `meaning init` to write a commented starter file showing every key at
its default.
...
For each input, Meaning walks from the file's directory upward and uses the
first `meaning.toml` it finds. The walk stops at a directory containing a `.git`
entry (the repository root), so a config file outside your repository is never
picked up.

If no project file is found, Meaning next checks the `MEANING_CONFIG`
...
drive and pointing every machine at it. A set `MEANING_CONFIG` shadows the
global config entirely.

If `MEANING_CONFIG` is unset, meaning falls back to a global user config: the
first existing file among

1. `$XDG_CONFIG_HOME/meaning/config.toml`
2. `~/.config/meaning/config.toml`
3. the platform config directory (`%APPDATA%\meaning\config.toml` on Windows,
   `~/Library/Application Support/meaning/config.toml` on macOS)

The `MEANING_CONFIG` and global files use the same schema as a project
`meaning.toml` and are whole-file fallbacks, never merged with a project config.
...
- `--no-config` ignores any project, `MEANING_CONFIG`, or global file and uses
  built-in defaults.
...
Meaning publishes a [JSON Schema](https://json-schema.org/) for `meaning.toml`
so editors with TOML support can provide key and value completion, hover
documentation, and validation.

**Schema URL:** <https://backmatter.github.io/meaning/meaning.schema.json>
...
```jsonc
{
  "evenBetterToml.schema.associations": {
    "^(.*/)?meaning\\.toml$": "https://backmatter.github.io/meaning/meaning.schema.json"
  }
}
```
...
```toml
#:schema https://backmatter.github.io/meaning/meaning.schema.json
```

This is also useful for the global user configuration, whose generic
`config.toml` file name should not be associated with Meaning automatically.
...
to the directory containing the `meaning.toml`. Excludes apply to both `format`
...
Options for `meaning format`. Each mirrors a CLI flag of the same name, which
...
Rule selection for `meaning lint`, shared by the [LaTeX](linter-rules.md) and
...
keys. This covers wrappers that Meaning cannot recognize without macro
...
Declares environments Meaning cannot recognize from the file alone: one that
...
Entries are keyed by the environment's own name, whether or not Meaning already
knows it:
...
environment Meaning already curates, so there is no way to spell out "this one
...
- `like` naming an environment Meaning does not know
...
- a spelling that is already a LaTeX command Meaning knows (`'\emph'`), which
...
The target is looked up among the environments Meaning curates by hand; a
...
Use this when the definition is somewhere Meaning cannot see: a sibling `.sty`,
...
A spelling must be a command of your own. Naming one Meaning already knows
...
> through your editor's LSP settings, not `meaning.toml`. Where a TeX
