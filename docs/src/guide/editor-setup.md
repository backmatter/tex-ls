...
Meaning ships a language server. Start it with:
...
editor's LSP client at the `meaning` binary with the `lsp` argument and
...
discovered `meaning.toml` always wins outright, and absent one, your editor's
...
`meaning.toml`, which locates the compile's `.aux` artifacts; see the
...
these settings come from the editor rather than `meaning.toml`, and they never
affect `meaning format` or `meaning lint`, whose output stays a pure function of
...
**Meaning never typesets, and it never reads a `.synctex.gz`.** Forward search
...
`-synctex=1` passed to `pdflatex`/`lualatex` directly. Meaning will not run that
...
like [TEXMF discovery](#texmf-discovery), and not from `meaning.toml`. Where the
...
```sh
meaning inverse-search --input "%f" --line "%l"
```

substituting the viewer's own placeholders. For zathura that is:

```sh
zathura --synctex-editor-command "meaning inverse-search --input %{input} --line %{line}"
```
...
Servers advertise themselves in `$MEANING_IPC_DIR`, else a per-user directory
...
bytes, and meaning says so explicitly in its log if yours does. On a system with
...
and meaning ignores any advertisement it does not own, so another user can
...
```lua
vim.lsp.config.meaning = {
  cmd = { "meaning", "lsp" },
  filetypes = { "tex", "latex", "plaintex", "bib" },
  root_markers = { "meaning.toml", ".git" },
  init_options = { lineWidth = 80, indentWidth = 2 },
}
vim.lsp.enable("meaning")
```
...
The VS Code extension is included as source under `editors/code`. Build it
locally and configure it to use your installed server:
...
Install the generated VSIX with VS Code's **Install from VSIX** command. Set
`meaning.executableStrategy` to `environment` to use `meaning` on your `PATH`,
or use `path` with `meaning.executablePath` for an explicit binary. There is no
Meaning marketplace release yet.
...
- `meaning.formatting.enable` — use Meaning as a formatter.
- `meaning.diagnostics.enable` — show Meaning diagnostics (the linter).
- `meaning.languageFeatures.enable` — hover, completion, navigation, symbols,
...
Turning off `meaning.diagnostics.enable` this way suppresses **every**
diagnostic, including the syntax/parse errors that a `meaning.toml` `[lint]`
selection [cannot silence](../reference/configuration.md#lint). The
`meaning.toml` route stays the right tool when you want to keep parse errors but
mute specific lint rules across every editor and the CLI.

### Using with LaTeX Workshop

Meaning works alongside [LaTeX
...
building, PDF preview, and SyncTeX, while meaning handles formatting, linting,
and navigation. Run both, and let each own its half.

**Formatting.** The meaning extension registers itself as the default formatter
...
built-in formatter, so pick meaning explicitly:

```json
{
  "[bibtex]": {
    "editor.defaultFormatter": "backmatter.meaning"
  }
}
```
...
alongside meaning produces overlapping diagnostics for many common issues.

Meaning completes label names across the document's connected include graph,
including labels defined in chapters. It keeps unrelated document namespaces
separate and deduplicates names. BibTeX completion offers entry types, field names
and string macros, reusing the current document's cached analysis.
...
Any LSP-capable editor can run meaning: configure a server whose command is
`meaning lsp`, communicating over stdio, for LaTeX documents. Consult your
