...
`meaning format` lays out LaTeX source deterministically. Output is decided
...
```sh
meaning format paper.tex          # rewrite the file in place
cat paper.tex | meaning format    # stdin → stdout
meaning format --check paper.tex  # diff, don't write; non-zero if unformatted
```
...
and `--wrap`---mirror the `[format]` section of `meaning.toml` and override it
...
For persistent settings, meaning reads a `meaning.toml` discovered from the
working directory upward; pass `--config <PATH>` to point at a specific file or
`--no-config` to ignore any discovered one. Run `meaning init` to write a
starter `meaning.toml`.
...
```tex
% meaning-format skip: hand-aligned by eye
\begin{tikzpicture}
...
```tex
% meaning-format skip-file: generated, do not edit
```
...
every lint rule over the same span: `% meaning skip`, `% meaning off` /
`% meaning on`, and `% meaning skip-file`. Use the `-format` spelling when you
want the linter to keep reporting.

To exclude whole files by path instead, use `exclude`/`extend-exclude` in
`meaning.toml`; see the [Configuration
...
The mirror of these is `% meaning-lint`, which suppresses diagnostics without
...
- **Protected regions**: verbatim-like content (`verbatim`, `lstlisting`,
  `\verb`, comments) is never altered. An environment meaning cannot tell is
...
therefore *lint fixes*, not formatting. Run `meaning lint --fix` for those.
