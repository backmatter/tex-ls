...
`meaning lint` parses each file and reports diagnostics, rendered with source
...
cat paper.tex | meaning lint   # stdin
```
...
single problem never aborts the parse—meaning anchors recovery on clean LaTeX
...
Beyond parse recovery, meaning ships a growing set of built-in rules
...
The verb carries the scope, and there are three:
  ...
  | Scope              | Directive                                                |
  | ------------------ | -------------------------------------------------------- |
  | The next construct | `% meaning-lint skip <rule>: <reason>`                   |
  | A region           | `% meaning-lint off <rule>` … `% meaning-lint on <rule>` |
  | The whole file     | `% meaning-lint skip-file <rule>: <reason>`              |
...
`% meaning skip`, `% meaning off` / `% meaning on`, and `% meaning skip-file`.
For layout only, use the `% meaning-format` spellings described in
...
```bib
@comment{meaning-lint skip missing-required-field: publisher long gone}
...
Some rules ship an **auto-fix**. `meaning lint --fix` applies the
...
`meaning lint --output json` emits the findings as a JSON array on **stdout**
