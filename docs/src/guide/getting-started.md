...
Meaning's main subcommands are `format`, `lint`, and `lsp` (with `parse` and
...
```sh
meaning format intro.tex methods.tex results.tex
```
...
```sh
cat paper.tex | meaning format -
```

A piped standard input is also read when you pass no paths at all, so the
shorter `cat paper.tex | meaning format` works too. At an interactive prompt,
though, where there is nothing to pipe, `meaning format` with no paths reports a
...
```sh
meaning format --check paper.tex
```
...
```sh
meaning format --line-width 100 --indent-width 4 --wrap preserve paper.tex
```
