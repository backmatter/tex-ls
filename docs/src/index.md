...
**Meaning** is a language server, formatter, and linter for LaTeX.

It parses LaTeX into a **lossless concrete syntax tree** and builds three tools
on top of it:

- a **formatter** (`meaning format`) that lays out source deterministically,
- a **linter** (`meaning lint`) that reports diagnostics, and
- a **language server** (`meaning lsp`) that brings both to your editor, as well
...
Meaning treats input as generic TeX surface syntax. It never *requires*
...
- [Installation](guide/installation.md): get the `meaning` binary.
