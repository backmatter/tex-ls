...
//! `crates/meaning-parser/build.rs`, next to the data it bakes.
...
fn generate_completions(outdir: &std::ffi::OsString) -> std::io::Result<()> {
    ...
    for shell in [
        ...
        generate_to(shell, &mut cmd, "meaning", outdir)?;
    }
    ...
    for (src, dst) in [
        ("meaning.bash", "meaning.bash"),
        ("meaning.fish", "meaning.fish"),
        ("_meaning", "_meaning"),
    ] {
...
/// Generate `target/man/meaning.1` plus a `meaning-<sub>.1` page per subcommand,
/// like git/cargo.
fn generate_man_pages() -> std::io::Result<()> {
    ...
    let subcommand_names: Vec<String> = cmd
        ...
        .map(|s| format!("meaning-{}", s.get_name()))
        .collect();
    ...
    std::fs::write(out_dir.join("meaning.1"), main_content.as_bytes())?;
    ...
    for subcommand in cmd.get_subcommands() {
        ...
        let name = format!("meaning-{}", subcommand_name);
        ...
        let fixed_content = content
            ...
            .replace(
                ...
                &format!("\\fBmeaning {}\\fR", subcommand_name),
            )
            .replace(
                &format!("{}\\-", subcommand_name),
                &format!("meaning\\-{}\\-", subcommand_name),
            );

        // SEE ALSO: meaning(1) plus sibling subcommand pages.
        let mut see_also_refs: Vec<String> = vec!["meaning".to_string()];
