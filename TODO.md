...
following **rust-analyzer's** architecture. See `docs/src/development/architecture.md` for
design decisions and invariants.

A cargo workspace with separate parser, formatter, analysis, shared protocol,
native host, and browser adapter responsibilities. See the
[ownership decision](docs/adr/0001-independent-language-service.md).
The formatter also exercises parser correctness.

Status: `[ ]` todo · `[~]` in progress · `[x]` done

## Next development pass after the architecture cutover

Recorded on 2026-09-11. The extraction is implemented and validated locally.
The work below is pending; audit items are questions to verify, not confirmed
defects. Start with incremental analysis and semantic queries after preserving
the architecture checkpoint.

### Preserve the baseline and verify host parity

- [ ] Commit the Meaning architecture rewrite before changing its internals.
  The pre-rewrite checkpoint is `bf5086d`.
  Keep commits local unless pushing is explicitly requested.
- [ ] Replay equivalent source changes through native and browser hosts and
  compare shared feature results, allowing only advertised host differences.

The recorded Rust baseline is 2,548 passing tests and three intentionally
ignored tests. Establish reproducible native and WASM performance baselines
using fixtures and measurement scripts owned by this repository. Record query
latency, allocations, retained memory, and artifact size with build details.

### 1. Incremental analysis and memory ownership

- [ ] Trace edits through `meaning-analysis/src/incremental`: record which
  inputs change, which queries recompute, and which source/tree/index values
  are copied or retained. Include prose edits, declaration changes, include
  graph changes, bibliography edits, and project removal.
- [ ] Profile memory retention. Separate live allocations from
  the WASM allocator's retained linear memory; check release after source
  removal, snapshot disposal, and repeated project switching.
- [ ] Audit custom caching around the existing Salsa database. Use its tracked
  dependencies where they simplify correctness and invalidate less work.
  Preserve ordered reparse staging and consumed-prefix handling wherever that
  side state remains necessary.
- [ ] Replace proven duplicate ownership and unnecessary cloning. Retain data
  according to source and snapshot lifetime. Do not reintroduce arbitrary
  file-count limits, edit windows, or user-tuned cache quotas.

Completion evidence: full and incremental results agree, unrelated facts remain
reusable, removed sources are no longer retained by live engine state, and
paired native/browser profiles explain changes in time and memory.

### 2. Shared semantic queries

- [ ] Map the facts used by diagnostics, completion, hover, navigation,
  references, and rename. Identify repeated tree walks and resolution logic.
- [ ] Consolidate repeated work into analysis-owned queries and stable per-file
  summaries. A prose edit should preserve project facts when those facts have
  not changed. Validate dependency edges for includes, labels, citations,
  declarations, and package information.
- [ ] Keep shared feature behavior consistent across hosts. Keep URI handling
  and LSP result conversion in protocol, and external discovery in hosts.

Completion evidence: affected features agree on resolution, existing LaTeX and
BibTeX behavior passes corpus comparisons, and query measurements show that
the extracted summaries prevent repeated work.

### 3. Scheduling, cancellation, and protocol correctness

- [ ] Audit native scheduling and the browser session/worker boundary for
  cancellation, superseded reads, and stale response publication. Verify that
  newer document state wins without dropping or reordering source writes.
- [ ] Check initialize/shutdown, document versions, sequential content changes,
  negotiated position encodings, and workspace-edit applicability against LSP.
- [ ] Verify that a fresh session reconstructs equivalent analysis from explicit
  source and configuration inputs without retaining state from prior sessions.

Completion evidence: deterministic race scenarios cover edits during requests,
document removal, cancellation, restart, Unicode positions, and CRLF. Each host
advertises only the operations it provides.

### 4. Parser and recovery evaluation

- [ ] Profile parsing and recovery on real projects and incomplete input before
  selecting a replacement algorithm or library. The existing parser already
  uses rowan; a library change needs a demonstrated benefit.
- [ ] Evaluate simpler parser event handling and recovery where current code is
  difficult to maintain. Record an architecture decision before changing the
  incremental parsing model or its locality assumptions.
- [ ] Compare any replacement against losslessness, full/incremental parse
  equivalence, malformed-input progress, formatter idempotence and token
  preservation, diagnostics, and shared language features. Preserve protected
  regions and supported LaTeX dialect behavior.

### Delivery rules and references

Replace one owned subsystem at a time, then remove its obsolete implementation
and update callers and documentation in the same change. Upstream merge and old
internal API compatibility are not requirements. Preserve attribution and
licenses. Keep native/browser parity and analysis independent of application
storage and network services. Runtime configuration for user behavior remains distinct from
arbitrary implementation tuning.

Evaluate maintained dependencies against correctness, complexity, WASM support,
supported Rust versions, licensing, and measured cost. Meaning already uses
Salsa and rowan; audit their use before adding competing infrastructure.
For each substantive replacement, run targeted correctness checks, the project
check workflow, affected corpus comparisons, and paired performance scenarios.
Validate the native executable and WASM binding when shared engine code changes.

- [rust-analyzer architecture](https://rust-analyzer.github.io/book/contributing/architecture.html):
  explicit analysis inputs, immutable snapshots, and stable semantic summaries.
- [clangd code walkthrough](https://clangd.llvm.org/design/code): language
  operations and host scheduling responsibilities.
- [LSP specification](https://microsoft.github.io/language-server-protocol/specifications/lsp/3.17/specification/):
  synchronization, cancellation, capabilities, and workspace edits.
...
- [ ] **Make aligned math layout respect the configured line width.** Alignment
  ...
  a width ceiling or layout-meaning assertion using a long aligned display from
...
- [ ] **Key-value continuation indent in an expl3 fallback statement (open scope
  ...
  Meaning neither produces nor preserves this: it emits the continuation *level
  ...
  so this is meaning inventing layout for a dialect it cannot name. Upstream's
...
- [ ] Profile large BibTeX edits with a repository-owned 20,000-entry fixture.
  Measure edited-field, unchanged-field, and string-macro completion requests
  separately in native and WASM builds. Identify repeated parsing and model
  construction before selecting optimizations.
- [ ] Measure batch project registration and workspace-symbol query costs.
  Each new path currently clones/sorts the tracked path list; each workspace
  symbol request rebuilds outlines and line indexes. Preserve ordered upserts,
  reparse staging and snapshot membership when reducing this work.

- [ ] Profile completion on a repository-owned 1,000-file fixture with a 1 MB
  active file. Compare command-name edits and prose edits, and keep cold-query
  latency separate from warm latency in native and WASM builds. Investigate a
  locality proof for control-word edits under the existing reparse contract.
...
(latexindent/tex-fmt, ChkTeX/lacheck), so meaning already leads on language
...
meaning lacks and wants are filed in the sections below, tagged *(LW)*: command
...
Meaning offers command, environment, label, cite-key, bib field/type, and file
...
- [ ] *(Design decision)* **Package-scoped command completion.** texlab suggests
  ...
  model). Meaning's signature DB is flat (curated + CWL + scanned); scoping
...
- [ ] *(Design decision)* **Math preview on hover *(LW)*.** LaTeX Workshop's
  ...
  eventual rationale in the architecture documentation.

### Code actions

- [ ] **Surround selection with environment/command *(LW)*.** LaTeX Workshop
  ships these as client-side commands; meaning can host them editor-agnostically
...
- [ ] **Split `crates/meaning-parser/tests/parser.rs` by area.** It is now 2,970
...
- [ ] **Mine the `latexindent` corpus for construct coverage** (human-in-the-loop,
  ongoing). See `CONTRIBUTING.md` for fixture authoring. The corpus is read as a
...
meaning) is admissible; the rest are explicit non-goals recorded here so they are
not re-proposed. Forward and inverse search ship today
(`lsp/forward_search.rs`, `ipc.rs`, `meaning inverse-search`) without meaning
...
- [ ] **`unexplained-suppression`.** Report a suppression directive with no
  ...
  while meaning currently enables every registered rule by default; either
  accept the default-on policy or first add explicit opt-in rule metadata.

- [ ] **Format suppression in `.bib`.** The `% meaning-format` axis parses in a
...
- [ ] **`task bib-error-compat`: biber as a `.bib` *error* oracle.** The gap the
  ...
  tally. Cross-tabulate per corpus file, `meaning has diagnostics` × `biber has
  ERRORS`: agreement on the diagonal, **meaning dirty + biber clean = over-strict**
  (this bug's class), meaning clean + biber dirty = under-strict (we would format
...
- [ ] **Central-bib fallback via the texmf index *(LW)*.** LaTeX Workshop
  ...
  Use this fallback for LSP completion, hover, and go-to-definition. The
  `undefined-citation` lint and CLI remain hermetic and project-local.

## CST / AST / trivia

- [ ] **[low, latent] No `SyntaxNodePtr`/`AstPtr`.** RA stashes stable node
  pointers in salsa data to re-resolve across reparses; meaning sidesteps this by
