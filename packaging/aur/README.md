...
[`meaning-bin`](https://aur.archlinux.org/packages/meaning-bin) AUR package.
There is no separate AUR mirror repo: the AUR git remote
(`ssh://aur@aur.archlinux.org/meaning-bin.git`) is a pure deploy target.
...
The first workflow run claims the `meaning-bin` name by pushing to the empty AUR
