...
const REPO: &str = "backmatter/meaning";

struct MeaningBinary {
    path: String,
    args: Option<Vec<String>>,
}

struct MeaningExtension {
...
impl MeaningExtension {
    fn language_server_binary(
        &mut self,
        language_server_id: &LanguageServerId,
        worktree: &zed::Worktree,
    ) -> Result<MeaningBinary> {
        ...
        if let Some(path) = worktree.which("meaning") {
            return Ok(MeaningBinary {
                path,
                args: binary_args,
            });
        ...
        let asset = details
            ...
            .ok_or_else(|| {
                format!(
                    "Meaning release {} has no asset matching any of {:?}",
        ...
        if !fs::metadata(&details.downloaded_binary_path).is_ok_and(|metadata| metadata.is_file()) {
            ...
            .map_err(|error| format!("Failed to download Meaning: {error}"))?;
        }
    ...
    fn new(platform: zed::Os, architecture: zed::Architecture, version: String) -> Result<Self> {
        let architecture = match architecture {
            ...
            zed::Architecture::X86 => {
                return Err("Meaning does not publish binaries for 32-bit x86".into())
            }
        };

        let asset_names = match platform {
            zed::Os::Mac => vec![format!("meaning-{architecture}-apple-darwin.tar.gz")],
            zed::Os::Linux => vec![
                format!("meaning-{architecture}-unknown-linux-gnu.tar.gz"),
                format!("meaning-{architecture}-unknown-linux-musl.tar.gz"),
            ],
            zed::Os::Windows => vec![format!("meaning-{architecture}-pc-windows-msvc.zip")],
        };
        let downloaded_file_type = match platform {
            zed::Os::Mac | zed::Os::Linux => zed::DownloadedFileType::GzipTar,
            zed::Os::Windows => zed::DownloadedFileType::Zip,
        };
        let downloaded_directory = format!("meaning-{version}");
        let binary_name = match platform {
            zed::Os::Mac | zed::Os::Linux => "meaning",
            zed::Os::Windows => "meaning.exe",
        };
...
impl zed::Extension for MeaningExtension {
    fn new() -> Self {
        Self {
...
zed::register_extension!(MeaningExtension);
...
mod tests {
    ...
    fn resolves_macos_release() {
        assert_eq!(
            ...
            Ok(GithubReleaseDetails {
                asset_names: vec!["meaning-aarch64-apple-darwin.tar.gz".into()],
                downloaded_file_type: DownloadedFileType::GzipTar,
                downloaded_directory: "meaning-0.20.0".into(),
                downloaded_binary_path: "meaning-0.20.0/meaning".into(),
            })
        );
    }
    ...
    fn resolves_linux_release_with_musl_fallback() {
        assert_eq!(
            ...
            Ok(GithubReleaseDetails {
                asset_names: vec![
                    "meaning-x86_64-unknown-linux-gnu.tar.gz".into(),
                    "meaning-x86_64-unknown-linux-musl.tar.gz".into(),
                ],
                downloaded_file_type: DownloadedFileType::GzipTar,
                downloaded_directory: "meaning-0.20.0".into(),
                downloaded_binary_path: "meaning-0.20.0/meaning".into(),
    ...
    fn resolves_windows_release() {
        assert_eq!(
            ...
            Ok(GithubReleaseDetails {
                asset_names: vec!["meaning-aarch64-pc-windows-msvc.zip".into()],
                downloaded_file_type: DownloadedFileType::Zip,
                downloaded_directory: "meaning-0.20.0".into(),
                downloaded_binary_path: "meaning-0.20.0/meaning.exe".into(),
            })
        );
    }
