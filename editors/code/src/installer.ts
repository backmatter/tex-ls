...
function isCandidateMeaningCliReleaseTag(tag: string): boolean {
...
function detectTargetAsset(): TargetAsset {
  ...
  if (process.platform === "darwin" && process.arch === "arm64") {
    return { archiveNames: ["meaning-aarch64-apple-darwin.tar.gz"], binaryName };
  }
  if (process.platform === "darwin" && process.arch === "x64") {
    return { archiveNames: ["meaning-x86_64-apple-darwin.tar.gz"], binaryName };
  }
  if (process.platform === "linux" && process.arch === "arm64") {
    return {
      archiveNames: [
        "meaning-aarch64-unknown-linux-gnu.tar.gz",
        "meaning-aarch64-unknown-linux-musl.tar.gz",
      ],
      binaryName,
    };
  }
  if (process.platform === "linux" && process.arch === "x64") {
    return {
      archiveNames: [
        "meaning-x86_64-unknown-linux-gnu.tar.gz",
        "meaning-x86_64-unknown-linux-musl.tar.gz",
      ],
      binaryName,
    };
  }
  if (process.platform === "win32" && process.arch === "x64") {
    return { archiveNames: ["meaning-x86_64-pc-windows-msvc.zip"], binaryName };
  }
  if (process.platform === "win32" && process.arch === "arm64") {
    return { archiveNames: ["meaning-aarch64-pc-windows-msvc.zip"], binaryName };
  }
...
export async function resolveMeaningBinary(
        ...
        async () => {
          ...
          if (candidate.kind === "latest-list") {
            ...
            for (const release of releases) {
              ...
              if (!isCandidateMeaningCliReleaseTag(release.tag_name)) {
                continue;
              }
            ...
            throw new Error(
              `No stable Meaning CLI release asset '${target.archiveNames.join("' or '")}' found for ${repo}@latest`,
            );
          }
          ...
          if (
            candidate.kind === "latest-pointer" &&
            !isCandidateMeaningCliReleaseTag(release.tag_name)
          ) {
            throw new Error(
              `${repo}/releases/latest points at '${release.tag_name}', which is not a Meaning CLI release`,
            );
          }
  ...
  if (asset.name.endsWith(".zip")) {
    await fs.writeFile(installedBinary, binaryFromZip(archive, target.binaryName));
